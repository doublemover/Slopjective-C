...
Current hard-cutover overlay: this v0.11 plan is historical planning input, not
the current public command or compatibility model. Active repo docs must route
commands through `npm run objc3c -- <action>`, treat direct helper paths as
implementation evidence only, and keep retired spellings, shims, fallback paths,
compatibility aliases, and migration-lane behavior out of accepted support.
...
| Part 3 `3.9` (issue 1) | Canonical value-optional source spelling and rejection policy.                         | `A-05`, `A-06`, `B-05`, `B-06`, `D-06`                 |
...
| ------ | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `A-01` | Define and publish machine-readable schema for `objc3-runtime-2025Q4` and `objc3-abi-2025Q4` manifests. | `scripts/objc3c_shared/schema_registry.py`, `docs/support/capability_matrix.md`, `docs/support/evidence_map.md`     | Manifest schema ownership resolves through the shared registry and support/evidence rows; direct planning-ledger mirrors are not support truth. | None                           |
| `A-02` | Define manifest schema versioning and rejection policy.                                                 | `scripts/objc3c_shared/schema_registry.py`, `docs/support/capability_matrix.md`, `docs/support/evidence_map.md`     | Version transitions, unsupported inputs, and retired manifest surfaces fail closed unless the capability/evidence owners record implemented support. | `A-01`                         |
| `A-03` | Define cross-part abstract-machine synchronization protocol for Parts 0/3/6/7/8.                        | `spec/process/ABSTRACT_MACHINE_SYNC_PROTOCOL.md`, update proposal for `spec/ABSTRACT_MACHINE_AND_SEMANTIC_CORE.md` | Protocol assigns owners, required cross-links, and change-trigger rules for all listed parts.            | None                           |
...
| `A-05` | Decide canonical value-optional spelling policy (`Optional<T>` as the future spelling, lowercase rejected). | Part 3 patch draft for `3.9`, `spec/DECISIONS_LOG.md` entry                                                        | Canonical spelling and rejection policy are ratified and remove ambiguity from open issue 3.9.1.          | None                           |
| `A-06` | Specify diagnostics and canonicalization fix-it behavior for noncanonical optional spelling.            | Part 3 diagnostics patch draft, `spec/EDGE_CASE_EXAMPLES_AND_FIXITS.md` updates                                    | Error matrix and fix-it behavior are defined without creating an accepted compatibility alias.            | `A-05`                         |
...
| `B-05` | Implement parser behavior for canonical and rejected optional spellings.           | Parser patch plan, `tests/parse/OPT-SPELL-*`                                                                     | Parser accepts only canonical enabled spellings and rejects lowercase retired spellings per Lane A. | `A-05`                         |
| `B-06` | Implement diagnostics/fix-its for noncanonical optional spelling paths.            | Diagnostics patch plan, `tests/diag/OPT-DIAG-*`                                                                  | Diagnostic messages and fix-it edits match Lane A canonicalization rules with profile-sensitive severity. | `A-06`, `B-05`                 |
...
| `C-11` | Stand up public registry format for extension capabilities and lifecycle state.    | Schema IDs owned by `scripts/objc3c_shared/schema_registry.py`; support rows in `docs/support/capability_matrix.md` and `docs/support/evidence_map.md`; governance projection README     | Registry payload claims resolve through shared schema ownership and the capability/evidence matrix; direct registry-file mirrors, fallback aliases, and compatibility-only publication paths are rejected. | `C-02`, `C-08`, `C-10`         |
...
| Task   | Objective                                                                   | Artifacts                                                                                  | Done criteria                                                                                   | Depends on                                     |
| ------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `D-01` | Produce integrated dependency map across all lane tasks.                    | Historical planning ledger only: `docs/reference/legacy_spec_anchor_index.md`              | All 56 tasks have predecessors/successors and a single critical path is identified.             | `A-01`, `A-03`, `A-05`, `A-07`, `A-09`, `C-01` |
| `D-02` | Define cycle milestones, gate dates, and review checkpoints.                | Historical planning ledger only: `docs/reference/legacy_spec_anchor_index.md#planning-future-work-v011-milestones` | Milestones map to task IDs and include objective entry/exit criteria.                           | `D-01`                                         |
| `D-03` | Assign primary and backup owners for every task.                            | Historical planning ledger only: `docs/reference/legacy_spec_anchor_index.md`              | Each task has accountable owner, backup owner, and review SLA.                                  | `D-02`                                         |
| `D-04` | Build risk register linked to unresolved or high-volatility decisions.      | Historical planning ledger only: `docs/reference/legacy_spec_anchor_index.md`              | Every risk has likelihood, impact, mitigation task links, and trigger metrics.                  | `D-01`, `A-05`, `A-07`, `A-09`, `C-01`         |
| `D-05` | Define quality gates and pass thresholds for cycle exit.                    | `docs/support/capability_matrix.md`, `docs/support/evidence_map.md`, `scripts/objc3c_shared/schema_registry.py`; historical planning ledger: `docs/reference/legacy_spec_anchor_index.md#planning-future-work-v011-quality-gates` | Conformance, diagnostics stability, and reproducibility thresholds resolve through current support/evidence/schema owners. | `B-11`, `B-13`                                 |
| `D-06` | Produce canonicalization guidance for optional, mangling, and reification changes. | `docs/migration/v011_language_changes.md`                                                  | Guide includes rejected-source diagnostics, fix-it usage, and canonical rewrite recommendations without promising a migration lane. | `A-12`, `B-06`, `B-08`, `B-09`                 |
| `D-07` | Define docs synchronization workflow for normative and tooling artifacts.   | `docs/support/capability_matrix.md`, `docs/support/evidence_map.md`, `scripts/objc3c_shared/schema_registry.py`     | Workflow defines trigger conditions, required reviewers, publication cadence, and canonical support/schema owners without fallback doc mirrors. | `A-14`, `B-13`, `C-14`                         |
| `D-08` | Define discrepancy triage process for spec/toolchain divergence.            | `docs/support/capability_matrix.md`, `docs/support/evidence_map.md`, issue template; historical planning ledger: `docs/reference/legacy_spec_anchor_index.md` | Severity matrix and response SLA are adopted without treating planning-ledger rows as support truth. | `D-07`                                         |
...
| `D-13` | Decide carryover/de-scope list for unfinished tasks.                        | Historical planning ledger only: `docs/reference/legacy_spec_anchor_index.md#planning-future-work-v011-carryover` | Each incomplete task is classified as carryover, defer, or drop with rationale.                 | `D-12`                                         |
...
| `4`   | `A-05` | Resolves optional spelling ambiguity that directly impacts parser rejection behavior.          | No prerequisite.                                         |
