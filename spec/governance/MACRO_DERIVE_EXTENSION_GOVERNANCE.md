...
- vendor extension version and rejection rules,
...
- Stable change-control is required (Section 7).
- Unprefixed canonical names are allowed only at this stage.
- Any incompatible change requires major-version change control and explicit transition/rejection policy.
...
## 3. Vendor extension version and rejection rules {#mdx-gov-3}
...
### 3.2 Macro and derive surface transition rules {#mdx-gov-3-2}
...
- Vendor aliases to Stable names are not support surfaces. Transition records may name old IDs only as deprecation/tombstone evidence and shall emit diagnostics when encountered.

### 3.3 Feature-test and metadata support {#mdx-gov-3-3}
...
- transition evidence: replacement notes, breakage analysis, and strict unsupported-toolchain behavior for unsupported toolchains,
...
| Provisional -> Stable                  | Yes         | Full dossier; independent implementations (2+); interop proof; replacement guidance | Next major release by default (minor only with explicit waiver) |
| Stable -> Deprecated                   | Yes         | Replacement plan; deprecation diagnostics/fix-its; sunset schedule                 | Next minor or major release                                     |
...
- Deprecated extensions may publish maintenance patches only; no new functionality.
...
- release-note and replacement-guidance entries,
