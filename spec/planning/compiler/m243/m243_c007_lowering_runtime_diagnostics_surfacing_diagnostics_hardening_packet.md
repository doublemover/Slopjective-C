# M243-C007 Lowering/Runtime Diagnostics Surfacing Diagnostics Hardening Packet

Packet: `M243-C007`
Milestone: `M243`
Lane: `C`
Dependencies: `M243-C006`

## Scope

Expand lane-C lowering/runtime diagnostics surfacing closure with explicit
diagnostics hardening consistency/readiness and replay-key guardrails so
diagnostics hardening evidence remains deterministic and fail-closed.
Code/spec anchors are mandatory scope inputs.

## Anchors

- Contract: `docs/contracts/m243_lowering_runtime_diagnostics_surfacing_diagnostics_hardening_c007_expectations.md`
- Checker: `scripts/check_m243_c007_lowering_runtime_diagnostics_surfacing_diagnostics_hardening_contract.py`
- Tooling tests: `tests/tooling/test_check_m243_c007_lowering_runtime_diagnostics_surfacing_diagnostics_hardening_contract.py`
- Surface type anchor:
  `native/objc3c/src/pipeline/objc3_frontend_types.h`
- Surface derivation anchor:
  `native/objc3c/src/pipeline/objc3_lowering_runtime_diagnostics_surfacing_diagnostics_hardening_surface.h`
- Pipeline wiring: `native/objc3c/src/pipeline/objc3_frontend_pipeline.cpp`
- Artifact fail-closed wiring: `native/objc3c/src/pipeline/objc3_frontend_artifacts.cpp`
- Dependency anchors from `M243-C006`:
  - `docs/contracts/m243_lowering_runtime_diagnostics_surfacing_edge_case_expansion_and_robustness_c006_expectations.md`
  - `spec/planning/compiler/m243/m243_c006_lowering_runtime_diagnostics_surfacing_edge_case_expansion_and_robustness_packet.md`
  - `scripts/check_m243_c006_lowering_runtime_diagnostics_surfacing_edge_case_expansion_and_robustness_contract.py`
  - `tests/tooling/test_check_m243_c006_lowering_runtime_diagnostics_surfacing_edge_case_expansion_and_robustness_contract.py`
- Architecture/spec anchors:
  - `native/objc3c/src/ARCHITECTURE.md`
  - `spec/LOWERING_AND_RUNTIME_CONTRACTS.md`
  - `spec/MODULE_METADATA_AND_ABI_TABLES.md`
- Build/readiness scripts (`package.json`):
  - `check:objc3c:m243-c007-lane-c-readiness`

## Required Evidence

- `tmp/reports/m243/M243-C007/lowering_runtime_diagnostics_surfacing_diagnostics_hardening_contract_summary.json`

## Determinism Criteria

- C007 readiness remains fail-closed when diagnostics hardening consistency,
  readiness, replay-key continuity, or lowering pipeline diagnostics hardening
  prerequisites drift.
- C006 edge-case expansion and robustness remains a strict prerequisite for
  C007.
