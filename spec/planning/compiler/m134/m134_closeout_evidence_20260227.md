...
- `test:objc3c:lane-e` recovered to green after stale check-chain removal and nil-receiver lowering expectation refresh.
...
- [ ] `TODO-M134-VAL-PYTEST`: restore full `python -m pytest tests/tooling -q` baseline after planning-corpus reduction side effects.
...
| `python -m pytest tests/tooling -q` | FAIL | Current baseline fails in planning/seed-era suites (for example: missing `spec/planning/v013_*` seed assets and planning-checkbox fixture contract drift). |

## Remaining Blockers

| ID | Owner | Blocker | Unblock criteria |
| --- | --- | --- | --- |
| `TODO-M134-VAL-PYTEST` | INT/tooling owners | `python -m pytest tests/tooling -q` remains red after planning-corpus reduction (seed-era and planning-checkbox suites still depend on removed/changed v013 assets/behaviors). | Land follow-on tooling baseline restoration so full `tests/tooling` suite returns green deterministically. |

## INT-RG Closeout Recommendation

`M134-INT-RG-01` should remain open. As of `2026-02-28`, milestone lane issues are closed and lane-e is green, but full `tests/tooling` baseline remains unresolved (`TODO-M134-VAL-PYTEST`).
