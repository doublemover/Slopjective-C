...
M270-C002 lowering implementation note:

- actor lowering now emits live helper-backed rewrites for:
...
- lane D now proves the supported actor mailbox slice through the private
  helper cluster and `objc3_runtime_copy_actor_runtime_state_for_testing`
- runnable mailbox helper traffic still remains bound to
...
- feature-specific fix-it synthesis now publishes
