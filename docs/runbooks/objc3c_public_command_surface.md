...
| `check:stdlib:surface` | `check-stdlib-surface` | `repo` | `stdlib roots, canonical module inventory, and package alias mapping stay checked in and coherent` | `fixed-shape` | `python:scripts/check_stdlib_surface.py` |
...
| `test:stdlib:e2e` | `validate-runnable-stdlib-foundation` | `full` | `packaged stdlib boundary contracts, module smoke compilation, and runtime-archive linkage stay reproducible from the staged runnable toolchain bundle` | `fixed-shape` | `python:scripts/check_objc3c_runnable_stdlib_foundation_end_to_end.py` |
