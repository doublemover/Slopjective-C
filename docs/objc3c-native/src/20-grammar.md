...
## M272 direct, final, and sealed dispatch lowering

- effective `objc_direct` sends on concrete `self` and known class receivers now
  lower as exact LLVM direct calls to the bound `@objc3_method_*`
  implementation symbols
- `objc_dynamic` opt-out sites still lower through `@objc3_runtime_dispatch_i32`
...
- this remains a scoped Part 9 lowering slice; broad dynamic-receiver direct
  dispatch and optimizer-driven devirtualization are still out of scope

## M272 dispatch metadata and interface preservation
...
- emitted `module.runtime-import-surface.json` artifacts now carry one dedicated
...
- imported runtime surfaces now reload the same Part 9 preservation packet and
  preserve the widened class/method source-record fields for separate
  compilation replay
...
## M273 macro safety, sandbox, and determinism semantics

- the emitted frontend manifest now publishes `frontend.pipeline.semantic_surface.objc_part10_macro_safety_sandbox_and_determinism_semantics`
- macro callable admission is currently limited to pure, body-backed free functions
- macro callables must carry all three markers:
  - `objc_macro`
  - `objc_macro_package`
...
- live fail-closed diagnostics in this slice are:
  - `O3S320` incomplete macro metadata
  - `O3S321` orphan package/provenance metadata
  - `O3S322` sandbox-rejected package
  - `O3S323` non-deterministic provenance token
