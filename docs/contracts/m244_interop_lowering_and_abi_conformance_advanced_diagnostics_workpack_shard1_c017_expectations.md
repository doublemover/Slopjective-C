# M244 Interop Lowering and ABI Conformance advanced diagnostics workpack (shard 1) Expectations (C017)
...
Scope: lane-C interop lowering/ABI advanced diagnostics workpack (shard 1) governance with deterministic dependency continuity and fail-closed readiness integration.

## Objective

Execute lane-C advanced diagnostics workpack (shard 1) governance for interop
lowering and ABI conformance on top of C016 docs/runbook synchronization assets.
...
- `m244-c016` assets remain mandatory prerequisites:
...
2. Readiness command chain enforces `m244-c016` before `m244-c017`
   evidence checks run.
...
- `package.json` includes `check:objc3c:m244-c017-lane-c-readiness`.
