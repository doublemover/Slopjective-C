# M228 IR Emission Completeness Advanced Conformance Workpack (Shard 4) Expectations (C036)

Contract ID: `objc3c-ir-emission-completeness-advanced-conformance-workpack-shard4/m228-c036-v1`
Status: Accepted
Scope: lane-C IR-emission advanced conformance workpack (shard 4) closure on top of C035 advanced-conformance-shard1 governance.

## Objective

Execute issue `#5262` by locking deterministic lane-C advanced-integration-shard1
...
- `M228-C035` remains a mandatory prerequisite:
...
- Packet/checker/test assets for C036 remain mandatory:
...
1. `Objc3IREmissionCoreFeatureImplementationSurface` carries explicit C036
...
2. `BuildObjc3IREmissionCoreFeatureAdvancedIntegrationShard1Key(...)`
   remains deterministic and keyed by C035 closure continuity plus parse/typed
   advanced-integration shard1 alignment.
3. `BuildObjc3IREmissionCoreFeatureImplementationSurface(...)` computes
   advanced-integration shard1 fail-closed from C035 continuity and
...
5. `BuildObjc3FrontendArtifacts(...)` remains fail-closed for lane-C C036
   through `IsObjc3IREmissionCoreFeatureAdvancedIntegrationShard1Ready(...)`
   with deterministic diagnostic code `O3L337`.
6. IR metadata transport includes C036 readiness/key anchors:
...
- `package.json`
  - add/check `check:objc3c:m228-c036-ir-emission-completeness-advanced-conformance-workpack-shard4-contract`
  - add/check `test:tooling:m228-c036-ir-emission-completeness-advanced-conformance-workpack-shard4-contract`
  - add/check `check:objc3c:m228-c036-lane-c-readiness` chained from C035 -> C036
