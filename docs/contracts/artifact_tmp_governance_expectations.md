...
M143 standardizes deterministic artifact generation and replay-safe scratch handling under `tmp/` for CLI and C API parity workflows, including parser/AST-facing lane-A, sema/type-system lane-B, and lowering/LLVM IR/runtime-ABI lane-C closeout coverage.
...
| `M143-C001` | Lowering/LLVM IR/runtime-ABI lane-C replay artifacts are fail-closed under `tmp/artifacts/objc3c-native/{lowering-regression,typed-abi-replay-proof,lowering-replay-proof}/`; default run ids are deterministic (`m143-lane-c-lowering-regression-default`, `m143-lane-c-typed-abi-default`, `m143-lane-c-lowering-replay-proof-default`) when `OBJC3C_NATIVE_LOWERING_RUN_ID`, `OBJC3C_TYPED_ABI_REPLAY_PROOF_RUN_ID`, and `OBJC3C_NATIVE_LOWERING_REPLAY_PROOF_RUN_ID` are unset. |
