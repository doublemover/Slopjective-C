...
| `throws(E)` typed throws and `Optional<T>` value optionals                                                                                            | `reserved` / strict-error rejection              | Cite only the parser diagnostics, reserved capability rows, and checked negative/contract fixtures; typed throws are not erased to bare `throws`, lowercase `optional<T>` is not an alias, and neither feature is preview, partial, or accepted syntax.                                                               |
