...
| `throws(E)` typed throws and `Optional<T>` value optionals                                                                                            | typed throws single-payload error-out ABI; value optionals checked lowering contract only | For typed throws, claim only single-payload source/interface preservation, exact sema effect identity, and hidden error-out ABI lowering without erasure to bare `throws`; malformed and multi-payload forms remain rejected. For value optionals, cite only semantic type-signature carrier, stable `has_value`/`payload` layout identity, explicit absent/present construction contracts, checked unwrap/binding diagnostics, reserved IR/runtime diagnostics, and contract fixtures; lowercase `optional<T>` is not an alias. |
