...
| `throws(E)` typed throws, `Optional<T>` value optionals, expression-form `match`, and guarded match patterns                                         | `reserved` / strict-error rejection              | Cite only the parser diagnostics and negative fixtures; do not describe these as preview, partial, or accepted syntax. |
