...
| `throws(E)` typed throws and `Optional<T>` value optionals                                                                                            | typed throws source/sema metadata only; value optionals semantic carrier only | For typed throws, claim only single-payload source/interface preservation, exact sema effect identity, and fail-closed ABI/runtime/lowering boundaries; malformed payloads are not erased to bare `throws`. For value optionals, cite only semantic type-signature carrier, stable `has_value`/`payload` layout identity, reserved executable/runtime diagnostics, and contract fixtures; lowercase `optional<T>` is not an alias. |
