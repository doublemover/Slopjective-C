...
| --- | --- | --- | --- |
| #8153 | `docs/support/capability_matrix.json`, `docs/support/evidence_map.json`, `tests/conformance/support_claim_runnable_evidence_catalog.json`, `scripts/check_objc3c_post_cutover_index_closure.py` | `npm run objc3c -- validate-post-cutover-issue-evidence` | Index evidence is source-backed support truth only; this page does not close GitHub issues by itself. |
...
| #8161 | `tests/tooling/fixtures/stdlib_collections/runtime_backed_collection_claims_contract.json`, `tests/tooling/runtime/stdlib_foundation_next_runtime_probe.cpp`, `native/objc3c/src/runtime/stdlib/collections_runtime_contract.h` | `npm run objc3c -- validate-stdlib-foundation`; `npm run objc3c -- test-runtime-acceptance-fast` | Collections support covers concrete i32 arrays, slices, aggregation, mutable map insert/update, sets, and deterministic iterator guards; collection literals, dictionary literals, set literals, generic element/key/value typing, owned arbitrary-length array storage, array mutation, syntax-level for-in integration, map iteration, non-i32 hashing, and set deletion remain outside the claim. |
| #8162 | `tests/tooling/fixtures/stdlib_text/runtime_backed_text_claims_contract.json`, `tests/tooling/runtime/stdlib_foundation_next_runtime_probe.cpp`, `tests/tooling/fixtures/native/execution/positive/source_string_literal_text_shape_handle.objc3`, `native/objc3c/src/runtime/stdlib/text_runtime_contract.h` | `npm run objc3c -- validate-stdlib-foundation`; `npm run objc3c -- validate-conformance-corpus`; `npm run objc3c -- test-runtime-acceptance-fast` | Text support covers runtime-owned UTF-8 records, source-literal storage-backed byte retrieval, byte/unit counts, concatenation, prefix helpers, and fail-closed status reporting; Unicode scalar iteration, normalization, formatting, interpolation, source-level mutation, and Foundation bridging remain outside the claim. |
| #8163 | `tests/tooling/fixtures/module_cache/`, `tests/tooling/fixtures/module_interop_contracts/foundation_next_visibility_bridge_contract.json`, `native/objc3c/src/pipeline/objc3_module_interop_contract_surface.cpp`, `scripts/check_objc3c_incremental_module_cache_consistency.py` | `npm run objc3c -- test-mixed-module-differential`; `npm run objc3c -- test-runtime-acceptance-cross-module`; `npm run objc3c -- validate-module-interop-contracts` | Module support covers deterministic public import lookup, visibility/reexport checks, dependency-graph diagnostics, and rebuild metadata; hidden declarations, stale cache inputs, missing modules, cycles, duplicate exports, and ABI-incompatible imports fail closed. |
| #8164 | `tests/tooling/test_protocol_existential_support_claim_groundwork.py`, `tests/tooling/fixtures/native/protocol_composition_existential_value_flow.objc3`, `tests/conformance/semantic/TYP-8013-25.json` | `npm run objc3c -- validate-conformance-corpus`; `npm run objc3c -- validate-object-model-conformance`; `npm run objc3c -- test-runtime-acceptance-fast` | Protocol existential evidence covers checked `id<Protocol>` and comma-style `id<Protocol, Protocol>` value flow plus cross-module protocol metadata preservation; associated types, dynamic existential invocation ABI, ampersand-style composition syntax, Swift bridging, and Objective-C 2 compatibility remain out of scope. |
...
- `language.text.source-string-literal-text-shape-handle`
