...
- `npm run objc3c -- <action>` invokes `python -m scripts.objc3c_workflow`.
- `scripts/objc3c_workflow/registry.py` owns action names, tiers, pass-through
...
aliases, `public_scripts` tables, direct helper invocations, native build
commands, and implementation-only script names are not public command surface.
