...
### 0.3.6 Normative keywords and specification terms
This specification uses the following terms with their usual standards meaning:

- **shall**: a mandatory requirement for conforming implementations.
- **should**: a strong recommendation; deviations are permitted only with a clear reason.
- **may**: an optional capability; implementations are permitted but not required to provide it.

It also uses these technical terms:

- **ill‑formed**: a program that must be rejected with at least one diagnostic.
- **diagnostic**: a compiler message at warning or error level (as controlled by strictness; Part 1).
- **undefined behavior (UB)**: no requirements are imposed; implementations may do anything.
- **implementation‑defined behavior**: the implementation must choose a behavior and document it.
- **unspecified behavior**: the implementation may choose any behavior from an allowed set; it need not document its choice.

Unless explicitly stated otherwise, new ObjC 3.0 constructs are specified to avoid undefined behavior in favor of diagnosable ill‑formed programs.
