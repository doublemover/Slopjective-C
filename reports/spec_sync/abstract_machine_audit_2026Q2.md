...
| AM matrix rows                                                 | `rg -n "AM-T0[1-9]      | AM-T1[0-9]      | Part 3            | Part 6                          | Part 7                          | Part 8" docs/reference/legacy_spec_anchor_index.md#abstract-machine-and-semantic-core` | `PASS rows AM-T01..AM-T19 and cross-part links present` |
...
- Evidence:
  - `docs/reference/legacy_spec_anchor_index.md#part-6-errors-results-throws:294`:
    `T?`: unwrap or early-return `nil` (or throw) depending on surrounding function's declared return/effects.
  - `docs/reference/legacy_spec_anchor_index.md#part-6-errors-results-throws:337`:
    If the enclosing function is `throws` or returns `Result<…>`, use of `e?` is
    `ill-formed` in v1.
  - `docs/reference/legacy_spec_anchor_index.md#abstract-machine-and-semantic-core:242`:
...
- Evidence:
  - `docs/reference/legacy_spec_anchor_index.md#part-7-concurrency-async-await-actors:402`:
    `defer` handlers shall execute exactly once, in LIFO order, when scope exit
    occurs due to cancellation-driven unwind.
  - `docs/reference/legacy_spec_anchor_index.md#part-7-concurrency-async-await-actors:404`:
  ...
  - `docs/reference/legacy_spec_anchor_index.md#part-8-system-programming-extensions:38` and `:291`:
...
- Evidence:
  - `docs/reference/legacy_spec_anchor_index.md#part-0-baseline-and-normative-references:96-97`:
    preferred exact phrase is `ill-formed; diagnostic required`.
  - `docs/reference/legacy_spec_anchor_index.md#part-3-types-nullability-optionals-generics-keypaths:391` and `:444`:
