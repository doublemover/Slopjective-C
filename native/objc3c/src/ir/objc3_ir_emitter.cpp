...
class Objc3IREmitter {
 ...
 private:
  ...
  void EmitFrontendMetadata(std::ostringstream &out) const {
    ...
    // source-closure proof surface only. Later realization work must
    ...
    // object-model semantic-rule freeze anchor: IR stays evidence-only
    ...
    // evidence-only consumer of the sema-owned protocol conformance result while
    ...
    out << "!objc3.objc_runtime_link_host_link = !{!13}\n";
    ...
    out << "!45 = !{!\"" << EscapeCStringLiteral(frontend_metadata_.runtime_metadata_source_ownership_contract_id)
        ...
        << (frontend_metadata_.runtime_link_test_only ? 1 : 0) << ", i1 "
    ...
    out << "!12 = !{i64 "
        ...
        << "}\n\n";
    out << "!13 = !{i64 "
        << static_cast<unsigned long long>(frontend_metadata_.runtime_link_host_link_message_send_sites)
        << ", i64 " << static_cast<unsigned long long>(frontend_metadata_.runtime_link_host_link_required_sites)
        << ", i64 " << static_cast<unsigned long long>(frontend_metadata_.runtime_link_host_link_elided_sites)
        << ", i64 "
        << static_cast<unsigned long long>(frontend_metadata_.runtime_link_host_link_runtime_dispatch_arg_slots)
        << ", i64 "
        << static_cast<unsigned long long>(
               frontend_metadata_.runtime_link_host_link_runtime_dispatch_declaration_parameter_count)
        << ", !\"" << EscapeCStringLiteral(frontend_metadata_.runtime_link_host_link_runtime_dispatch_symbol)
        << "\", i1 "
        << (frontend_metadata_.runtime_link_host_link_default_runtime_dispatch_symbol_binding ? 1 : 0)
        << ", i64 "
        << static_cast<unsigned long long>(frontend_metadata_.runtime_link_host_link_contract_violation_sites)
        << ", i1 " << (frontend_metadata_.deterministic_runtime_link_host_link_handoff ? 1 : 0)
        << "}\n\n";
    out << "!14 = !{i64 "
        << static_cast<unsigned long long>(
               frontend_metadata_.ownership_qualifier_lowering_ownership_qualifier_sites)
    ...
    if (synthesized_property_accessor_count_ > 0u ||
      ...
      emit_declaration_once(kObjc3RuntimeActorRecordRaceGuardI32Symbol,
                            "declare i32 @" +
                                ...
                                "(i32)\n");
      emit_declaration_once(kObjc3RuntimeActorBindExecutorI32Symbol,
                            "declare i32 @" +
                                std::string(
                                    kObjc3RuntimeActorBindExecutorI32Symbol) +
                                "(i32, i32)\n");
      emit_declaration_once(kObjc3RuntimeActorMailboxEnqueueI32Symbol,
                            "declare i32 @" +
                                std::string(
                                    kObjc3RuntimeActorMailboxEnqueueI32Symbol) +
                                "(i32, i32, i32)\n");
      emit_declaration_once(kObjc3RuntimeActorMailboxDrainNextI32Symbol,
                            "declare i32 @" +
                                std::string(
