...
inline Objc3ParseLoweringReadinessSurface BuildObjc3ParseLoweringReadinessSurface(
  ...
  if (!toolchain_runtime_ga_operations_conformance_corpus_consistent) {
    surface.failure_reason =
        "toolchain/runtime GA operations conformance corpus is inconsistent";
    return surface;
  }

  if (!toolchain_runtime_ga_operations_conformance_corpus_ready) {
    surface.failure_reason =
  ...
  if (!toolchain_runtime_ga_operations_performance_quality_guardrails_consistent) {
    surface.failure_reason =
        "toolchain/runtime GA operations performance quality guardrails are inconsistent";
    return surface;
  }

  if (!toolchain_runtime_ga_operations_performance_quality_guardrails_ready) {
    surface.failure_reason =
