...
inline void CollectTypeSystemTypeSourceClosureExprSites(
...
inline void CollectTypeSystemTypeSourceClosureStmtSites(
...
inline Objc3FrontendTypeSystemTypeSourceClosureSummary
BuildTypeSystemTypeSourceClosureSummary(
  ...
  for (const auto &fn : program.functions) {
    for (const auto &stmt : fn.body) {
      detail::CollectTypeSystemTypeSourceClosureStmtSites(stmt.get(), summary);
    }
  }
  for (const auto &implementation : program.implementations) {
    for (const auto &method : implementation.methods) {
      for (const auto &stmt : method.body) {
        detail::CollectTypeSystemTypeSourceClosureStmtSites(
            stmt.get(), summary);
      }
    }
  }
