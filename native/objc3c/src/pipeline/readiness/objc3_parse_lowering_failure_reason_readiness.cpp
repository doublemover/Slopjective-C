...
BuildObjc3ParseLoweringFailureReasonReadiness(
  ...
  if (!surface.semantic_integration_surface_built) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "semantic integration surface not built");
  }

  if (!surface.semantic_diagnostics_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "semantic diagnostics handoff is not deterministic");
  }
  ...
  if (!surface.protocol_category_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "protocol/category handoff is not deterministic");
  }
  ...
  if (!surface.symbol_graph_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "symbol graph handoff is not deterministic");
  }

  if (!surface.scope_resolution_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "scope resolution handoff is not deterministic");
  }

  if (!surface.object_pointer_type_handoff_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "object pointer/nullability handoff is not deterministic");
  }

  if (!surface.typed_handoff_key_deterministic) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering handoff key is not deterministic");
  }

  if (!surface.typed_sema_core_feature_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering core feature contract is inconsistent");
  }

  if (!surface.typed_sema_core_feature_expansion_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering core feature expansion is inconsistent");
  }

  if (surface.typed_sema_core_feature_expansion_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering core feature expansion key is empty");
  }

  if (!surface.typed_sema_edge_case_compatibility_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case compatibility is inconsistent");
  }

  if (!surface.typed_sema_edge_case_compatibility_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case compatibility is not ready");
  }

  if (surface.typed_sema_edge_case_compatibility_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case compatibility key is empty");
  }

  if (!surface.typed_sema_edge_case_expansion_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case expansion is inconsistent");
  }

  if (!surface.typed_sema_edge_case_robustness_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case robustness is not ready");
  }

  if (surface.typed_sema_edge_case_robustness_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case robustness key is empty");
  }

  if (!surface.typed_sema_diagnostics_hardening_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering diagnostics hardening is inconsistent");
  }

  if (!surface.typed_sema_diagnostics_hardening_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering diagnostics hardening is not ready");
  }

  if (surface.typed_sema_diagnostics_hardening_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering diagnostics hardening key is empty");
  }

  if (!surface.typed_sema_recovery_determinism_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering recovery/determinism is inconsistent");
  }

  if (!surface.typed_sema_recovery_determinism_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering recovery/determinism is not ready");
  }

  if (surface.typed_sema_recovery_determinism_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering recovery/determinism key is empty");
  }

  if (!surface.typed_sema_conformance_matrix_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance matrix is inconsistent");
  }

  if (!surface.typed_sema_conformance_matrix_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance matrix is not ready");
  }

  if (surface.typed_sema_conformance_matrix_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance matrix key is empty");
  }

  if (!surface.typed_sema_conformance_corpus_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance corpus is inconsistent");
  }

  if (!surface.typed_sema_conformance_corpus_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance corpus is not ready");
  }

  if (surface.typed_sema_conformance_corpus_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance corpus key is empty");
  }
  ...
  if (!surface.typed_sema_cross_lane_integration_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering cross-lane integration is inconsistent");
  }

  if (!surface.typed_sema_cross_lane_integration_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering cross-lane integration is not ready");
  }

  if (surface.typed_sema_cross_lane_integration_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering cross-lane integration key is empty");
  }
  ...
  if (!surface.typed_sema_advanced_core_shard1_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 1 is inconsistent");
  }

  if (!surface.typed_sema_advanced_core_shard1_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 1 is not ready");
  }

  if (surface.typed_sema_advanced_core_shard1_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 1 key is empty");
  }
  ...
  if (!surface.typed_sema_advanced_diagnostics_shard1_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 1 is inconsistent");
  }

  if (!surface.typed_sema_advanced_diagnostics_shard1_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 1 is not ready");
  }

  if (surface.typed_sema_advanced_diagnostics_shard1_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 1 key is empty");
  }

  if (!surface.typed_sema_advanced_conformance_shard1_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 1 is inconsistent");
  }

  if (!surface.typed_sema_advanced_conformance_shard1_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 1 is not ready");
  }

  if (surface.typed_sema_advanced_conformance_shard1_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 1 key is empty");
  }

  if (!surface.typed_sema_advanced_integration_shard1_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 1 is inconsistent");
  }

  if (!surface.typed_sema_advanced_integration_shard1_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 1 is not ready");
  }

  if (surface.typed_sema_advanced_integration_shard1_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 1 key is empty");
  }

  if (!surface.typed_sema_advanced_performance_shard1_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced performance shard 1 is inconsistent");
  }

  if (!surface.typed_sema_advanced_performance_shard1_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced performance shard 1 is not ready");
  }

  if (surface.typed_sema_advanced_performance_shard1_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced performance shard 1 key is empty");
  }

  if (!surface.typed_sema_advanced_core_shard2_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 2 is inconsistent");
  }

  if (!surface.typed_sema_advanced_core_shard2_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 2 is not ready");
  }

  if (surface.typed_sema_advanced_core_shard2_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced core shard 2 key is empty");
  }
  ...
  if (!surface.typed_sema_advanced_diagnostics_shard2_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 2 is inconsistent");
  }

  if (!surface.typed_sema_advanced_diagnostics_shard2_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 2 is not ready");
  }

  if (surface.typed_sema_advanced_diagnostics_shard2_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced diagnostics shard 2 key is empty");
  }

  if (!surface.typed_sema_advanced_conformance_shard2_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 2 is inconsistent");
  }

  if (!surface.typed_sema_advanced_conformance_shard2_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 2 is not ready");
  }

  if (surface.typed_sema_advanced_conformance_shard2_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced conformance shard 2 key is empty");
  }

  if (!surface.typed_sema_advanced_integration_shard2_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 2 is inconsistent");
  }

  if (!surface.typed_sema_advanced_integration_shard2_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 2 is not ready");
  }

  if (surface.typed_sema_advanced_integration_shard2_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering advanced integration shard 2 key is empty");
  }

  if (!surface.typed_sema_integration_closeout_signoff_consistent) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering integration closeout/sign-off is inconsistent");
  }

  if (!surface.typed_sema_integration_closeout_signoff_ready) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering integration closeout/sign-off is not ready");
  }

  if (surface.typed_sema_integration_closeout_signoff_key.empty()) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering integration closeout/sign-off key is empty");
  }

  if (!typed_sema_lowering_readiness.typed_edge_case_compatibility_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case compatibility drifted from parse/lowering readiness");
  }

  if (!typed_sema_lowering_readiness.typed_edge_case_robustness_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering edge-case robustness drifted from parse/lowering readiness");
  }

  if (!typed_sema_lowering_readiness.typed_diagnostics_hardening_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering diagnostics hardening drifted from parse/lowering readiness");
  }

  if (!typed_sema_lowering_readiness.typed_recovery_determinism_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering recovery/determinism drifted from parse/lowering readiness");
  }

  if (!typed_sema_lowering_readiness.typed_conformance_matrix_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance matrix drifted from parse/lowering readiness");
  }

  if (!typed_sema_lowering_readiness.typed_conformance_corpus_alignment) {
    return Objc3ParseLoweringFailureReasonReadinessFailure(
        "typed sema-to-lowering conformance corpus drifted from parse/lowering readiness");
  }
