...
static bool IsRecognizedCoffMachine(std::uint16_t machine) {
  switch (machine) {
...
static void NormalizeCoffTimestamp(const std::filesystem::path &object_out) {
...
static int RunIRCompile(const std::filesystem::path &clang_path,
  ...
  const int compile_status = RunProcess(
      clang_path.string(), {"-x", "ir", "-c", ir_path.string(), "-o", object_out.string(), "-fno-color-diagnostics"});
  if (compile_status == 0) {
