...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m250_d006_toolchain_runtime_ga_operations_readiness_edge_case_expansion_and_robustness_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_d006_toolchain_runtime_ga_operations_readiness_edge_case_expansion_and_robustness_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m250-toolchain-runtime-ga-operations-readiness-edge-case-expansion-and-robustness-contract-d006-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-toolchain-runtime-ga-operations-readiness-edge-case-expansion-and-robustness/m250-d006-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-D006-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_edge_case_robustness_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-D006-SUR-06", "M250-D006-FORB-01"}
