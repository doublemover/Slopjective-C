...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m250_c011_lowering_runtime_stability_performance_quality_guardrails_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_c011_lowering_runtime_stability_performance_quality_guardrails_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m250-lowering-runtime-stability-performance-quality-guardrails-contract-c011-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc = tmp_path / "m250_lowering_runtime_stability_performance_quality_guardrails_c011_expectations.md"
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-lowering-runtime-stability-performance-quality-guardrails/m250-c011-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-C011-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_guardrails_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-C011-SUR-02", "M250-C011-FORB-01"}
...
def test_contract_fails_closed_when_expansion_failure_reason_is_missing(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["core_surface"].read_text(encoding="utf-8").replace(
            ...
            "lowering/runtime performance quality guardrails expansion drift marker",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M250-C011-SUR-13" for failure in payload["failures"])
