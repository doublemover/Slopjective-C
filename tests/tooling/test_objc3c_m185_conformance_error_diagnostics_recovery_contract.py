...
CONFORMANCE_FIXTURE = FIXTURE_ROOT / "M185-D001.json"
...
def test_m185_conformance_fixture_fields_are_canonical() -> None:
    fixture = json.loads(_read(CONFORMANCE_FIXTURE))

    assert fixture["id"] == "M185-D001"
    assert fixture["bucket"] == "lowering_abi"
    assert fixture["profile"] == "core"
    assert fixture["tracking"]["milestone"] == "M185"
    assert fixture["tracking"]["milestone_number"] == 270
    assert fixture["tracking"]["lane"] == "D"
    assert fixture["tracking"]["issue"] == 4517
    assert fixture["tracking"]["task"] == "M185-D001"
    assert fixture["tracking"]["sequence"] == 1


def test_m185_conformance_fixture_references_runbook_and_lane_d_contract_tests() -> None:
    fixture = json.loads(_read(CONFORMANCE_FIXTURE))
    ...
    assert "// EXPECT: error_diagnostics_recovery_lowering" in source
    assert "// EXPECT: deterministic_error_diagnostics_recovery_lowering_handoff=true" in source

    assert expect["parse"] == "accept"
    assert expect["diagnostics"] == []


def test_m185_conformance_fixture_replay_anchor_matches_validation_packets() -> None:
    ...
    assert "lane_contract=m185-error-diagnostics-ux-recovery-lowering-v1" in run1_replay_key

    assert f"; error_diagnostics_recovery_lowering = {run1_replay_key}" in run1_ir
    assert f"; error_diagnostics_recovery_lowering = {run2_replay_key}" in run2_ir
    assert "!objc3.objc_error_diagnostics_recovery_lowering = !{!45}" in run1_ir
    assert "!objc3.objc_error_diagnostics_recovery_lowering = !{!45}" in run2_ir
