...
SCRIPT_PATH = ROOT / "scripts" / "check_m227_b005_type_system_objc3_forms_edge_compat_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m227_b005_type_system_objc3_forms_edge_compat_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m227-type-system-objc3-forms-edge-compat-b005-v1"
    assert payload["ok"] is True
    assert payload["checks_total"] >= 10
    assert payload["checks_total"] == payload["checks_passed"]


def test_contract_fails_closed_when_sel_exclusion_is_removed(
    tmp_path: Path,
    ...
    assert any(failure["check_id"] == "M227-B005-SRC-04" for failure in payload["failures"])


def test_contract_fails_closed_when_disjoint_flag_is_removed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-B005-HDR-01" for failure in payload["failures"])
