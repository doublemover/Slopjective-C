...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m244_e002_lane_e_interop_conformance_gate_and_operations_modular_split_scaffolding_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m244_e002_lane_e_interop_conformance_gate_and_operations_modular_split_scaffolding_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m244_e002_lane_e_interop_conformance_gate_and_operations_modular_split_scaffolding_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m244-e002-lane-e-interop-conformance-gate-operations-modular-split-scaffolding-contract-v1"
    assert payload["ok"] is True
    assert payload["checks_total"] >= 50
    assert payload["checks_total"] == payload["checks_passed"]


def test_contract_default_summary_out_is_under_tmp_reports_m244_e002() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m244/M244-E002/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m244_e002_expectations.md"
    ...
    assert any(failure["check_id"] == "M244-E002-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_reference_drifts(
    ...
    drift_packet = tmp_path / "m244_e002_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E002-DOC-PKT-10" for failure in payload["failures"])
...
def test_contract_fails_closed_when_readiness_command_drops_dependency_references(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E002-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_drops_diagnostics_replay_optimization_input(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E002-PKG-04" for failure in payload["failures"])
