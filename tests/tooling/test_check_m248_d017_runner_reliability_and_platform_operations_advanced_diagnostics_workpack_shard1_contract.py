...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m248_d017_runner_reliability_and_platform_operations_advanced_diagnostics_workpack_shard1_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m248_d017_runner_reliability_and_platform_operations_advanced_diagnostics_workpack_shard1_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m248-runner-reliability-platform-operations-advanced-diagnostics-workpack-shard1-contract-d017-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-runner-reliability-platform-operations-advanced-diagnostics-workpack-shard1/m248-d017-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M248-D017-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_advanced_diagnostics_consistency(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D017-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_advanced_diagnostics_failure_reason_is_missing(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D017-SUR-08" for failure in payload["failures"])
