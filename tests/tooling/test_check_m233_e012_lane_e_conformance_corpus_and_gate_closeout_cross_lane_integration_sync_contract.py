...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m233_e012_lane_e_conformance_corpus_and_gate_closeout_cross_lane_integration_sync_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m233_e012_lane_e_conformance_corpus_and_gate_closeout_cross_lane_integration_sync_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m233-e012-lane-e-conformance-corpus-gate-closeout-cross-lane-integration-sync-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m233_e012() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m233/M233-E012/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m233_e012_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_b012_dependency_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m233_lane_e_conformance_corpus_and_gate_closeout_cross_lane_integration_sync_e012_expectations.md"
    drift_doc.write_text(
        replace_once(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M233-E011`, `M233-A008`, `M233-B012`, `M233-C016`, `M233-D020`",
            "Dependencies: `M233-E011`, `M233-A008`, `M233-B099`, `M233-C016`, `M233-D020`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M233-E012-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m233_e012_lane_e_conformance_corpus_and_gate_closeout_cross_lane_integration_sync_packet.md"
    drift_doc.write_text(
        replace_once(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M233-E012-DOC-PKT-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_missing_perf_budget_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M233-E012-PKG-04" for failure in payload["failures"])


def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    drift_expectations = tmp_path / "m233_e012_expectations.md"
    drift_packet = tmp_path / "m233_e012_packet.md"
    ...
    drift_expectations.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-lane-e-conformance-corpus-gate-closeout-cross-lane-integration-sync/m233-e012-drift`",
        ),
        encoding="utf-8",
    )
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M233-E011`, `M233-A008`, `M233-B012`, `M233-C016`, `M233-D020`",
            "Dependencies: `M233-E011`, `M233-A008`, `M233-B012`, `M233-C016`, `M233-D099`",
        ),
        encoding="utf-8",
    )
