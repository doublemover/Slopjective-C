...
SCRIPT_PATH = ROOT / "scripts" / "check_m241_d018_build_system_and_artifact_orchestration_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m241_d018_build_system_and_artifact_orchestration_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m241_d018_build_system_and_artifact_orchestration_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m241-d018-build-system-and-artifact-orchestration-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m241_d018() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m241/M241-D018/")
...
def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M241-D018-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_token_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m241_d018_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M241-D018-DOC-PKT-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M241-D018-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_metadata_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M241-D018-META-01" for failure in payload["failures"])
