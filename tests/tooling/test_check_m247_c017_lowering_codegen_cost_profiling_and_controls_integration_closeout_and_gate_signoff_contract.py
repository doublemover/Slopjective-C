...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m247_c017_lowering_codegen_cost_profiling_and_controls_integration_closeout_and_gate_signoff_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m247_c017_lowering_codegen_cost_profiling_and_controls_integration_closeout_and_gate_signoff_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m247_c017_lowering_codegen_cost_profiling_and_controls_integration_closeout_and_gate_signoff_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m247-c017-lowering-codegen-cost-profiling-controls-integration-closeout-and-gate-signoff-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m247_c017() -> None:
...
def test_contract_fails_closed_when_c016_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m247_c016_expectations.md"
    drift_doc.write_text(
        replace_all_occurrences(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C017-C016-EXP-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_c016_checker_dependency_path_is_missing(tmp_path: Path) -> None:
    summary_out = tmp_path / "summary.json"
    exit_code = contract.run(
        [
            "--c016-checker",
...
def test_contract_fails_closed_when_package_loses_perf_budget_anchor(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C017-PKG-04" for failure in payload["failures"])
