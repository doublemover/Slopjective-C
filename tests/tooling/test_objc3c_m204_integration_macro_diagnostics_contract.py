...
def test_m204_integration_macro_diagnostics_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m204-macro-diagnostics"' in package_json
    assert (
        "npm run check:objc3c:m205-macro-security && python -m pytest "
        "tests/tooling/test_objc3c_m204_frontend_macro_diagnostics_contract.py "
        "tests/tooling/test_objc3c_m204_sema_macro_diagnostics_contract.py "
        "tests/tooling/test_objc3c_m204_lowering_macro_diagnostics_contract.py "
        "tests/tooling/test_objc3c_m204_validation_macro_diagnostics_contract.py "
        "tests/tooling/test_objc3c_m204_integration_macro_diagnostics_contract.py -q"
    ) in package_json

    assert "Run M204 macro-diagnostics integration gate" in workflow
    assert "npm run check:objc3c:m204-macro-diagnostics" in workflow
