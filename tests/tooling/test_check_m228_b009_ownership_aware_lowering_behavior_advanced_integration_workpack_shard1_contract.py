...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_b019_ownership_aware_lowering_behavior_advanced_integration_workpack_shard1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_b019_ownership_aware_lowering_behavior_advanced_integration_workpack_shard1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-b019-ownership-aware-lowering-behavior-advanced-integration-workpack-shard1-contract-v1"
    )
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M228-B019-DOC-03" for failure in payload["failures"])


def test_contract_fails_closed_when_lane_readiness_drops_b018_dependency(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            '"check:objc3c:m228-b019-lane-b-readiness": "npm run check:objc3c:m228-b018-lane-b-readiness',
            '"check:objc3c:m228-b019-lane-b-readiness": "npm run check:objc3c:m228-b099-lane-b-readiness',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-B019-PKG-05" for failure in payload["failures"])
...
def test_contract_fails_closed_when_runbook_anchor_drifts(
    ...
    drift_runbook.write_text(
        contract.ARTIFACTS["runbook_doc"].read_text(encoding="utf-8").replace(
            ...
            "objc3c-ownership-aware-lowering-behavior-advanced-integration-workpack-shard1/m228-b019-drift",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-B019-RUN-02" for failure in payload["failures"])
