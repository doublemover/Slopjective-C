...
SCRIPT_PATH = ROOT / "scripts" / "check_m228_c003_ir_emission_completeness_core_feature_implementation_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m228_c003_ir_emission_completeness_core_feature_implementation_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_c003_ir_emission_completeness_core_feature_implementation_contract.py"
    )
...
def test_contract_fails_closed_when_artifact_diag_code_drifts(
    ...
    assert any(failure["check_id"] == "M228-C003-ART-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_ir_metadata_line_drifts(
    ...
    drift_ir_source.write_text(
        contract.ARTIFACTS["ir_source"].read_text(encoding="utf-8").replace(
            ...
            'out << "; ir_emission_core_feature_impl_ready_drift = "',
            1,
        ),
        encoding="utf-8",
    )
