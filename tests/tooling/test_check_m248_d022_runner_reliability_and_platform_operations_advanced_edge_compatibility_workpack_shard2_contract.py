...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m248_d022_runner_reliability_and_platform_operations_advanced_edge_compatibility_workpack_shard2_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m248_d022_runner_reliability_and_platform_operations_advanced_edge_compatibility_workpack_shard2_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m248-runner-reliability-platform-operations-advanced-edge-compatibility-workpack-shard2-contract-d022-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    assert any(failure["check_id"] == "M248-D022-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_advanced_edge_compatibility_consistency(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D022-FORB-01" for failure in payload["failures"])


def test_contract_fails_closed_when_advanced_edge_compatibility_failure_reason_is_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    drift_surface = tmp_path / "objc3_parse_lowering_readiness_surface.h"
    drift_surface.write_text(
        contract.ARTIFACTS["parse_surface_header"].read_text(encoding="utf-8").replace(
            ...
            "toolchain/runtime GA operations advanced edge compatibility shard2 drift marker",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D022-SUR-08" for failure in payload["failures"])
