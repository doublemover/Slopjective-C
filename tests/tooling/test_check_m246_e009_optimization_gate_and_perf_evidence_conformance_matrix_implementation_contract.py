...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_e009_optimization_gate_and_perf_evidence_conformance_matrix_implementation_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_e009_optimization_gate_and_perf_evidence_conformance_matrix_implementation_contract.py"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_e009() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-E009/")
...
def test_contract_passes_on_repository_sources_and_summary_contract(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-e009-optimization-gate-perf-evidence-conformance-matrix-implementation-contract-v1"
...
def test_contract_fails_closed_when_prerequisite_dependency_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m246_e009_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_pending_b010_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_e009_expectations.md"
    drift_doc.write_text(
        replace_once(
            ...
            "| `M246-B099` | Dependency token `M246-B099` is mandatory as pending seeded lane-B conformance matrix implementation assets. |",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E009-DOC-EXP-05" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_line_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_e009_packet.md"
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M246-E008`, `M246-A007`, `M246-B010`, `M246-C016`, `M246-D007`",
            "Dependencies: `M246-E008`, `M246-A007`, `M246-B010`, `M246-C016`, `M246-D099`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E009-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_chain_drops_real_d007_anchor(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m246_e009_lane_e_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E009-RUN-08" for failure in payload["failures"])
...
def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-optimization-gate-perf-evidence-conformance-matrix-implementation/m246-e009-drift`",
        ),
        encoding="utf-8",
    )
    drift_runner.write_text(
