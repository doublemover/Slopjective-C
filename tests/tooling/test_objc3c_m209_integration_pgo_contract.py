...
def test_m209_integration_pgo_is_documented() -> None:
...
def test_m209_integration_pgo_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m209-pgo-hooks"' in package_json
    assert (
        "npm run check:objc3c:m210-performance-regression && python -m pytest "
        "tests/tooling/test_objc3c_m209_frontend_pgo_contract.py "
        "tests/tooling/test_objc3c_m209_sema_pgo_contract.py "
        "tests/tooling/test_objc3c_m209_lowering_pgo_contract.py "
        "tests/tooling/test_objc3c_m209_validation_pgo_contract.py "
        "tests/tooling/test_objc3c_m209_integration_pgo_contract.py -q"
    ) in package_json

    assert "Run M209 profile-guided optimization integration gate" in workflow
    assert "npm run check:objc3c:m209-pgo-hooks" in workflow
