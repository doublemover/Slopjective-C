...
from governance_sustainability_schema_surface_assertions import (
    assert_fail_closed_without_summary,
    assert_summary_matches_registered_schemas,
)
from governance_sustainability_schema_surface_sources import (
    load_checker,
    load_json,
...
def assert_governance_sustainability_schema_surface_uses_registered_schemas() -> None:
...
def assert_governance_sustainability_schema_surface_rejects_unregistered_surface_path(
    tmp_path: Path,
) -> None:
    checker = load_checker()
    surface = load_json(checker.SCHEMA_SURFACE)
    surface["budget_summary_schema"] = (
        "schemas/unregistered-governance-budget.schema.json"
    )
...
def assert_governance_sustainability_schema_surface_rejects_broken_registered_draft(
...
def assert_governance_sustainability_schema_surface_rejects_broken_registered_schema_id(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    checker = checker_with_broken_schema(
        ...
        lambda payload: payload.__setitem__(
            "$id",
            "schemas/objc3c-governance-sustainability-evidence-broken.schema.json",
        ),
    )
...
def assert_governance_sustainability_schema_surface_rejects_broken_registered_contract(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
