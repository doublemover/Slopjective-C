...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m172_validation_nullability_flow_warning_precision_contract"
)
...
LANE_CONTRACT = "m172-nullability-flow-warning-precision-lowering-v1"
LOWERING_PREFIX = "; nullability_flow_warning_precision_lowering = "
PROFILE_PREFIX = "; frontend_objc_nullability_flow_warning_precision_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_nullability_flow_warning_precision_lowering_profile = "
    r"nullability_flow_sites=(\d+), object_pointer_type_sites=(\d+), nullability_suffix_sites=(\d+), "
    r"nullable_suffix_sites=(\d+), nonnull_suffix_sites=(\d+), normalized_sites=(\d+), "
    r"contract_violation_sites=(\d+), "
    r"deterministic_nullability_flow_warning_precision_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!25 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    nullability_flow_sites: int,
    object_pointer_type_sites: int,
    nullability_suffix_sites: int,
    nullable_suffix_sites: int,
    nonnull_suffix_sites: int,
    ...
    return (
        f"nullability_flow_sites={nullability_flow_sites};"
        f"object_pointer_type_sites={object_pointer_type_sites};"
        f"nullability_suffix_sites={nullability_suffix_sites};"
        f"nullable_suffix_sites={nullable_suffix_sites};"
        f"nonnull_suffix_sites={nonnull_suffix_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int]:
    ...
    return (
        sema["nullability_flow_warning_precision_lowering_sites"],
        sema["nullability_flow_warning_precision_lowering_object_pointer_type_sites"],
        sema["nullability_flow_warning_precision_lowering_nullability_suffix_sites"],
        sema["nullability_flow_warning_precision_lowering_nullable_suffix_sites"],
        sema["nullability_flow_warning_precision_lowering_nonnull_suffix_sites"],
        sema["nullability_flow_warning_precision_lowering_normalized_sites"],
        sema["nullability_flow_warning_precision_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !25 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_nullability_flow_warning_precision_lowering_surface"
    ]
    lowering = manifest["lowering_nullability_flow_warning_precision"]

    nullability_flow_sites = sema["nullability_flow_warning_precision_lowering_sites"]
    object_pointer_type_sites = sema[
        "nullability_flow_warning_precision_lowering_object_pointer_type_sites"
    ]
    nullability_suffix_sites = sema[
        "nullability_flow_warning_precision_lowering_nullability_suffix_sites"
    ]
    nullable_suffix_sites = sema[
        "nullability_flow_warning_precision_lowering_nullable_suffix_sites"
    ]
    nonnull_suffix_sites = sema[
        "nullability_flow_warning_precision_lowering_nonnull_suffix_sites"
    ]
    normalized_sites = sema["nullability_flow_warning_precision_lowering_normalized_sites"]
    ...
    expected_replay_key = _expected_replay_key(
        nullability_flow_sites=nullability_flow_sites,
        object_pointer_type_sites=object_pointer_type_sites,
        nullability_suffix_sites=nullability_suffix_sites,
        nullable_suffix_sites=nullable_suffix_sites,
        nonnull_suffix_sites=nonnull_suffix_sites,
        normalized_sites=normalized_sites,
        contract_violation_sites=contract_violation_sites,
        deterministic=deterministic_handoff,
    )

    assert nullability_suffix_sites <= nullability_flow_sites
    assert nullable_suffix_sites <= nullability_suffix_sites
    assert nonnull_suffix_sites <= nullability_suffix_sites
    assert nullability_suffix_sites == nullable_suffix_sites + nonnull_suffix_sites
    assert normalized_sites <= nullability_flow_sites
    assert contract_violation_sites <= nullability_flow_sites
    assert object_pointer_type_sites >= nullability_suffix_sites

    assert deterministic_handoff is True
    assert sema["lowering_nullability_flow_warning_precision_replay_key"] == expected_replay_key

    assert surface["nullability_flow_sites"] == nullability_flow_sites
    assert surface["object_pointer_type_sites"] == object_pointer_type_sites
    assert surface["nullability_suffix_sites"] == nullability_suffix_sites
    assert surface["nullable_suffix_sites"] == nullable_suffix_sites
    assert surface["nonnull_suffix_sites"] == nonnull_suffix_sites
...
def test_m172_validation_runbook_section_exists() -> None:
...
def test_m172_validation_manifest_packets_hold_deterministic_nullability_flow_warning_precision_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m172_validation_ir_markers_match_manifest_nullability_flow_warning_precision_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_nullability_flow_warning_precision_lowering = !{!25}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_nullability_flow_warning_precision"]["replay_key"]


def test_m172_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_nullability_flow_warning_precision"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_nullability_flow_warning_precision"]["replay_key"]
