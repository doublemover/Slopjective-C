...
SPEC = importlib.util.spec_from_file_location(
    "check_m249_d019_installer_runtime_operations_and_support_tooling_advanced_integration_workpack_shard1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m249_d019_installer_runtime_operations_and_support_tooling_advanced_integration_workpack_shard1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == (
        "m249-d019-installer-runtime-operations-support-tooling-advanced-integration-workpack-shard1-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m249_d019() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m249/M249-D019/")


def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m249_d019_expectations.md"
    ...
    assert any(failure["check_id"] == "M249-D019-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_mandatory_scope_wording_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m249_d019_packet.md"
...
def test_contract_fails_closed_when_readiness_runner_no_longer_chains_d017(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M249-D019-RUN-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifted(
    ...
    payload["scripts"]["check:objc3c:m249-d019-lane-d-readiness"] = (
        "python scripts/run_m249_d019_lane_d_readiness_drift.py"
    )
