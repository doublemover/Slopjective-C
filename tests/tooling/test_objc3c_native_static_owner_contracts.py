...
from objc3c_native_static_owner_contracts_behavior import (
    ...
    assert_native_inc_units_remain_private_to_owner_aggregators,
    assert_native_owner_modules_are_registered_in_cmake_without_inc_units,
    assert_runtime_public_result_abi_uses_split_status_and_payload_headers,
)


def test_native_owner_modules_are_registered_in_cmake_without_inc_units() -> None:
    assert_native_owner_modules_are_registered_in_cmake_without_inc_units()


def test_native_inc_units_remain_private_to_owner_aggregators() -> None:
    assert_native_inc_units_remain_private_to_owner_aggregators()
