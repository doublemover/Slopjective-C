...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m227-typed-sema-to-lowering-core-feature-c003-v1"
    assert payload["ok"] is True
    assert payload["checks_total"] >= 20
    assert payload["checks_total"] == payload["checks_passed"]


def test_contract_fails_closed_when_case_count_constant_drifts(
    tmp_path: Path,
...
def test_contract_fails_closed_when_readiness_surface_drops_typed_core_feature_key(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_readiness.write_text(
        (
            ...
            "surface.typed_sema_core_feature_removed =",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-C003-REA-04" for failure in payload["failures"])
