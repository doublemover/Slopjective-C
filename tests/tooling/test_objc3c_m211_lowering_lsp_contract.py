...
def test_m211_lowering_runtime_lsp_semantic_profile_section_present() -> None:
...
def test_m211_lowering_runtime_lsp_semantic_markers_align_to_source_surfaces() -> None:
    ...
    assert 'manifest << "      \\"semantic_surface\\": {\\"declared_globals\\":" << program.globals.size()' in (
        artifacts_source
    )
    assert '<< ",\\"declared_functions\\":" << manifest_functions.size()' in artifacts_source
    assert '<< ",\\"resolved_global_symbols\\":" << pipeline_result.integration_surface.globals.size()' in artifacts_source
    assert '<< ",\\"resolved_function_symbols\\":" << pipeline_result.integration_surface.functions.size()' in (
        artifacts_source
    )
    ...
    assert 'manifest << "    {\\"name\\":\\"" << fn.name << "\\",\\"params\\":" << fn.params.size() << ",\\"param_types\\":[";' in (
        artifacts_source
    )
