...
def test_m213_sema_debug_info_fidelity_profile_section_exists() -> None:
    fragment = _read(SEMA_DOC_FRAGMENT)
    for text in (
        "## M213 sema/type debug-info fidelity profile",
        "stable debug format order: `dwarf`, `pdb`",
        "debug format 1.1 deterministic sema diagnostics",
        "m213_dwarf_sema_diagnostics_packet",
        "m213_pdb_sema_diagnostics_packet",
        ...
        "debug format 1.2 deterministic type-metadata + debug stepping fidelity",
        "m213_dwarf_type_metadata_stepping_packet",
        "m213_pdb_type_metadata_stepping_packet",
        ...
        "fn.line",
        "fn.column",
        "python -m pytest tests/tooling/test_objc3c_m213_sema_debug_fidelity_contract.py -q",
    ):
        assert text in fragment


def test_m213_sema_debug_info_fidelity_profile_maps_to_source_anchors() -> None:
    ...
    for marker in ("program.globals[i].line", "program.globals[i].column", "fn.line", "fn.column"):
        assert marker in artifacts_source
