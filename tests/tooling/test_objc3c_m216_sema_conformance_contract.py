...
def test_m216_sema_conformance_suite_profile_section_exists() -> None:
    fragment = _read(SEMA_DOC_FRAGMENT)
    for text in (
        "## M216 sema/type conformance suite profile",
        "deterministic sema/type conformance suite v1 replay",
        "Suite v1 spec-section packet map:",
        "spec section 1.1 deterministic sema diagnostics",
        "m216_v1_sema_diagnostics_packet",
        "spec section 1.2 deterministic type-metadata handoff",
        "m216_v1_type_metadata_handoff_packet",
        ...
        "scalar_param_i32",
        "scalar_param_bool",
        "python -m pytest tests/tooling/test_objc3c_m216_sema_conformance_contract.py -q",
    ):
        assert text in fragment


def test_m216_sema_conformance_suite_profile_maps_to_source_anchors() -> None:
