...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m244_c014_interop_lowering_and_abi_conformance_release_candidate_and_replay_dry_run_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m244_c014_interop_lowering_and_abi_conformance_release_candidate_and_replay_dry_run_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m244_c014_interop_lowering_and_abi_conformance_release_candidate_and_replay_dry_run_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m244-c014-interop-lowering-and-abi-conformance-release-candidate-and-replay-dry-run-contract-v1"
    )
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("docs/contracts/does_not_exist_m244_c014.md"),
    )
...
def test_contract_fails_closed_when_package_chain_drops_c013_readiness(tmp_path: Path) -> None:
    ...
    scripts["check:objc3c:m244-c014-lane-c-readiness"] = scripts[
