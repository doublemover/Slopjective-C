...
def test_m219_sema_cross_platform_parity_profile_section_exists() -> None:
    fragment = _read(SEMA_DOC_FRAGMENT)
    for text in (
        "## M219 sema/type cross-platform parity profile",
        "stable platform order: `windows-msvc-x64`, `linux-clang-x64`, `macos-clang-arm64`",
        ...
        "python -m pytest tests/tooling/test_objc3c_m219_sema_cross_platform_contract.py -q",
    ):
        assert text in fragment


def test_m219_sema_cross_platform_parity_profile_maps_to_source_anchors() -> None:
