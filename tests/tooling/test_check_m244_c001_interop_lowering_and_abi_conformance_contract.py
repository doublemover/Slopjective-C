...
SCRIPT_PATH = ROOT / "scripts" / "check_m244_c001_interop_lowering_and_abi_conformance_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m244_c001_interop_lowering_and_abi_conformance_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_m244_c001_interop_lowering_and_abi_conformance_contract.py")
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m244-c001-interop-lowering-and-abi-conformance-contract-v1"
    assert payload["ok"] is True
    assert payload["checks_total"] >= 30
    assert payload["checks_total"] == payload["checks_passed"]


def test_contract_default_summary_out_is_under_tmp_reports_m244_c001() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m244/M244-C001/")
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M244-C001-DOC-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_architecture_deterministic_anchor_drifts(
    ...
    drift_architecture.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "deterministic lowering/ABI anchors and dependency tokens remain frozen",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-C001-ARCH-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_readiness_drops_tooling_test(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M244-C001-PKG-03", "M244-C001-FORB-01"}
