...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m241_e018_module_gate_and_reproducibility_evidence_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m241-e018-module-gate-and-reproducibility-evidence-advanced-conformance-workpack-shard-1-v1"
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = (
        tmp_path / "m241_module_gate_and_reproducibility_evidence_advanced_conformance_workpack_shard_1_e018_expectations.md"
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M241-E018-DOC-EXP-03" for failure in payload["failures"])
