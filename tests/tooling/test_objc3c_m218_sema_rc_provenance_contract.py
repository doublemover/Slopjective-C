...
def test_m218_sema_rc_provenance_profile_section_exists() -> None:
    fragment = _read(SEMA_DOC_FRAGMENT)
    for text in (
        "## M218 sema/type RC provenance profile",
        "release-candidate automation and provenance attestation on the sema/type lane",
        ...
        "scalar_param_i32",
        "scalar_param_bool",
        "rc_type_metadata_handoff_packet",
        "python -m pytest tests/tooling/test_objc3c_m218_sema_rc_provenance_contract.py -q",
    ):
        assert text in fragment


def test_m218_sema_rc_provenance_profile_maps_to_source_anchors() -> None:
