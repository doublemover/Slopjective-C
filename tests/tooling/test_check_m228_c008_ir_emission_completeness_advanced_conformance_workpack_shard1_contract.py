...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m228_c018_ir_emission_completeness_advanced_conformance_workpack_shard1_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m228_c018_ir_emission_completeness_advanced_conformance_workpack_shard1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_c018_ir_emission_completeness_advanced_conformance_workpack_shard1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-c018-ir-emission-completeness-advanced-conformance-workpack-shard1-contract-v1"
    )
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M228-C018-DOC-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_anchor_drifts(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["core_surface_header"].read_text(encoding="utf-8").replace(
            ...
            "IsObjc3IREmissionCoreFeatureAdvancedConformanceShard1ReadyDrift(",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C018-SUR-22" for failure in payload["failures"])


def test_contract_fails_closed_when_lane_readiness_drops_c017_dependency(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            ...
            '"check:objc3c:m228-c018-lane-c-readiness": "npm run check:objc3c:m228-c099-lane-c-readiness',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C018-PKG-05" for failure in payload["failures"])
