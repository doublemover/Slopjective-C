...
1. Backward/forward change-rejection taxonomy matrix for the extension registry contract.
...
| `schema.json` | Fixture-local machine-checkable schema with canonical ID `https://objc3c.dev/schemas/extension-registry-governance-contract-v1.schema.json`; historical `compatibility_*` keys are taxonomy fixture keys only, not compatibility support fields. | Read-only fixture artifact for this issue. |
...
3. Backward and forward transition outcomes are independently classified as fixture evidence states: `pass`, `conditional`, or `fail`.
...
| `WVR-RC-02` | Toolchain environment defect with confirmed isolation note and replay date. | `medium` | owner, rationale, expiry date, isolation note, replay command |
...
1. Unknown-major taxonomy (`CM-RC-07`) failures.
...
| `ESC-RC-02` (`E2`) | Repeated validator failure or unknown-major taxonomy failure. | Governance lane lead | `T+48h` | Freeze merge, open incident note, assign remediation owner. |
...
| `ESC-RC-04` (`E4`) | Integrity-risk or policy breach with no permitted corrective bypass. | Steering owner + security owner | Immediate | Apply emergency hold and require superseding corrective artifact set. |
...
| --- | --- | --- | --- |
| `AC-V013-GOV-02-01` | Compatibility taxonomy matrix covers schema versioning and backward/forward rejection policy. | Section `3.2` contains matrix rows `CM-RC-01`..`CM-RC-08` with backward and forward evidence outcomes. | This package Section `3`; schema `governance_contract.compatibility_matrix`. |
| `AC-V013-GOV-02-02` | Required-field rejection behavior is explicit. | Section `3.2` and Section `5.2` define blocking behavior for required-field changes. | This package Sections `3` and `5`; schema `required_field_policy`. |
...
| --- | --- | --- | --- |
| [x] | `CL-V013-GOV-02-01` | `AC-V013-GOV-02-01` | Compatibility taxonomy matrix includes backward/forward evidence outcomes and schema-versioning rules. |
...
- [x] Compatibility taxonomy matrix covers schema versioning, required fields, and forward/backward rejection policy.
...
| --- | --- | --- | --- | --- | --- | --- |
| `ERC-DEP-M15-01` | `Hard` | This package is canonical authority for M15 W1 compatibility taxonomy semantics (`CM-RC-*`), validator contract (`VAL-RC-*`), and escalation policy (`ESC-RC-*`). | M15 artifacts omit this package as authority, remove required row IDs, or introduce contradictory taxonomy/validator/escalation rejection semantics. | `Lane A M15 owner (#894)` | Restore canonical references + deterministic IDs, rerun M15 command anchors, and republish clean evidence. | `AC-V014-M15-01` |
...
| `ERC-DEP-M15-04` | `Hard` | Breaking compatibility taxonomy classes (`CM-RC-04`, `CM-RC-06`, `CM-RC-07`, `CM-RC-08`) remain non-waiverable fail-closed controls. | Any artifact permits waiver, `HOLD`, or alternate-success bypass for these breaking classes. | `Lane A M15 owner (#894)` | Restore non-waiverable fail-closed language and rerun disposition command anchor with exit `0`. | `AC-V014-M15-02` |
| `ERC-DEP-M15-05` | `Soft` | Conditional taxonomy classes (`CM-RC-02`, `CM-RC-03`, `CM-RC-05`) may enter `HOLD` only as evidence with explicit owner, ETA, and replay command; they cannot override hard gates or authorize support. | Conditional drift lacks owner/ETA/replay command, or `HOLD` is used to bypass any hard gate failure. | `Lane A M15 owner (#894)` | Record owner + ETA + replay command, remediate drift, then rerun affected command anchors. | `AC-V014-M15-02` |
...
| --- | --- | --- | --- | --- | --- | --- |
| `ERC-DEP-M16-01` | `Hard` | This package is canonical W2 authority for strict validator parity, deterministic failure taxonomy, and publication-gate rejection controls for fixture-local compatibility labels. | W2 artifacts omit this package as authority, remove `ERC-DEP-M16-*` rows, or redefine W2 semantics with contradictory support language. | `Lane A M16 owner (#899)` | Restore canonical references and W2 row set, rerun matrix command anchors, and republish clean evidence. | `AC-V014-M16-01` |
...
| `ERC-DEP-M16-06` | `Soft` | Conditional compatibility taxonomy drift (`CM-RC-02`, `CM-RC-03`, `CM-RC-05`) may remain only as evidence-only `HOLD` with explicit owner + ETA + replay command and no support authority. | Missing owner/ETA/replay metadata for conditional drift, or any attempt to use conditional drift to bypass hard gates or authorize support. | `Lane A M16 owner (#899)` | Record owner + ETA + replay command or resolve drift, then rerun impacted command anchors. | `AC-V014-M16-07` |
...
| `FTX-RC-M16-06` | `Soft` | Conditional compatibility taxonomy drift with hard gates still passing and explicit owner + ETA + replay command present. | `HOLD` | `ESC-RC-01` (`E1`) | Waiver not used; evidence-only `HOLD`, no support |
...
| --- | --- | --- | --- |
| `PUB-RC-M16-01` | All hard dependencies (`ERC-DEP-M16-01`, `ERC-DEP-M16-02`, `ERC-DEP-M16-03`, `ERC-DEP-M16-04`, `ERC-DEP-M16-05`, `ERC-DEP-M16-07`, `ERC-DEP-M16-08`) pass with linked evidence anchors. | Fixture evidence remains `PUBLISH`-eligible; no compatibility support is implied. | `REJECT`. |
...
| `PUB-RC-M16-03` | Acceptance/evidence schema remains complete: stable `DEP/CMD/EVID/AC` IDs plus explicit exit codes for every replay command. | Fixture-evidence `PUBLISH` or evidence-only `HOLD` (if only `PUB-RC-M16-02` applies). | `REJECT`. |
...
1. `PUBLISH`: fixture evidence only; every hard dependency passes and no hard taxonomy class (`FTX-RC-M16-01`, `FTX-RC-M16-02`, `FTX-RC-M16-03`, `FTX-RC-M16-04`, `FTX-RC-M16-05`, `FTX-RC-M16-07`) is active.
2. `HOLD`: evidence-only state; only soft class `FTX-RC-M16-06` is active with required owner + ETA + replay command, and all hard dependencies remain `PASS`.
