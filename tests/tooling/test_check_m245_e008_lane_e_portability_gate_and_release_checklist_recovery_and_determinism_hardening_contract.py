...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m245_e008_lane_e_portability_gate_and_release_checklist_recovery_and_determinism_hardening_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m245_e008_lane_e_portability_gate_and_release_checklist_recovery_and_determinism_hardening_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m245-e008-lane-e-portability-gate-release-checklist-recovery-and-determinism-hardening-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m245_e008() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m245/M245-E008/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m245_e008_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_b004_dependency_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m245_lane_e_portability_gate_and_release_checklist_recovery_and_determinism_hardening_e008_expectations.md"
    drift_doc.write_text(
        replace_once(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M245-E007`, `M245-A003`, `M245-B004`, `M245-C004`, `M245-D006`",
            "Dependencies: `M245-E007`, `M245-A003`, `M245-B099`, `M245-C004`, `M245-D006`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E008-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m245_e008_lane_e_portability_gate_and_release_checklist_recovery_and_determinism_hardening_packet.md"
    drift_doc.write_text(
        replace_once(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E008-DOC-PKT-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_missing_diagnostics_replay_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E008-PKG-03" for failure in payload["failures"])


def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    drift_expectations = tmp_path / "m245_e008_expectations.md"
    drift_packet = tmp_path / "m245_e008_packet.md"
    ...
    drift_expectations.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-lane-e-portability-gate-release-checklist-recovery-and-determinism-hardening/m245-e008-drift`",
        ),
        encoding="utf-8",
    )
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M245-E007`, `M245-A003`, `M245-B004`, `M245-C004`, `M245-D006`",
            "Dependencies: `M245-E007`, `M245-A003`, `M245-B004`, `M245-C004`, `M245-D099`",
        ),
        encoding="utf-8",
    )
