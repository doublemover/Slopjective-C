...
def test_m212_integration_code_action_is_documented() -> None:
    library_api_doc = _read(LIBRARY_API_DOC)

    for text in (
        "## M212 integration refactor/code-action engine",
        "npm run check:objc3c:m212-code-action",
        "check:objc3c:m213-debug-fidelity",
...
def test_m212_integration_code_action_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m212-code-action"' in package_json
    assert (
        "npm run check:objc3c:m213-debug-fidelity && python -m pytest "
        "tests/tooling/test_objc3c_m212_frontend_code_action_contract.py "
        "tests/tooling/test_objc3c_m212_sema_code_action_contract.py "
        "tests/tooling/test_objc3c_m212_lowering_code_action_contract.py "
        "tests/tooling/test_objc3c_m212_validation_code_action_contract.py "
        "tests/tooling/test_objc3c_m212_integration_code_action_contract.py -q"
    ) in package_json

    assert "Run M212 refactor/code-action integration gate" in workflow
    assert "npm run check:objc3c:m212-code-action" in workflow
