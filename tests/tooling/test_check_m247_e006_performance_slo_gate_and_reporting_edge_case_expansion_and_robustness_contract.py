...
SPEC = importlib.util.spec_from_file_location(
    "check_m247_e006_performance_slo_gate_and_reporting_edge_case_expansion_and_robustness_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m247_e006_performance_slo_gate_and_reporting_edge_case_expansion_and_robustness_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m247-e006-performance-slo-gate-reporting-edge-case-expansion-and-robustness-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m247_e006() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m247/M247-E006/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("docs/contracts/does_not_exist_m247_e006.md"),
    )
...
def test_contract_fails_closed_when_expectations_drop_dependency_token(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Dependencies: `M247-E005`, `M247-A006`, `M247-B099`, `M247-C006`, `M247-D005`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-E006-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_chain_drops_b007_gate(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m247_e006_lane_e_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-E006-RUN-06" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_lane_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    scripts["check:objc3c:m247-e006-lane-e-readiness"] = "npm run check:objc3c:m247-e005-lane-e-readiness"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-E006-CFG-07" for failure in payload["failures"])
