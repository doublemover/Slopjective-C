...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_c017_ir_emission_completeness_advanced_diagnostics_workpack_shard1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_c017_ir_emission_completeness_advanced_diagnostics_workpack_shard1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-c017-ir-emission-completeness-advanced-diagnostics-workpack-shard1-contract-v1"
    )
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M228-C017-DOC-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_anchor_drifts(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["core_surface_header"].read_text(encoding="utf-8").replace(
            ...
            "IsObjc3IREmissionCoreFeatureAdvancedDiagnosticsShard1ReadyDrift(",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C017-SUR-22" for failure in payload["failures"])


def test_contract_fails_closed_when_lane_readiness_drops_c016_dependency(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            ...
            '"check:objc3c:m228-c017-lane-c-readiness": "npm run check:objc3c:m228-c099-lane-c-readiness',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C017-PKG-05" for failure in payload["failures"])
