...
SPEC = importlib.util.spec_from_file_location(
    "check_m249_d014_installer_runtime_operations_and_support_tooling_release_candidate_and_replay_dry_run_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m249_d014_installer_runtime_operations_and_support_tooling_release_candidate_and_replay_dry_run_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m249-d014-installer-runtime-operations-support-tooling-release-candidate-replay-dry-run-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m249_d014() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m249/M249-D014/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M249-D014-DOC-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_chain_drops_runner(
    ...
    payload["scripts"]["check:objc3c:m249-d014-lane-d-readiness"] = (
        "python scripts/run_m249_d014_lane_d_readiness_drift.py"
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M249-D014-PKG-05" for failure in payload["failures"])
...
def test_contract_fails_closed_when_architecture_anchor_drifts(
    ...
    drift_architecture.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "M249 lane-D D014 replay dry-run placeholder anchor",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M249-D014-ARC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_d013_checker_dependency_missing(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] == "M249-D014-MISSING-D013_CHECKER" for failure in payload["failures"]
    )
