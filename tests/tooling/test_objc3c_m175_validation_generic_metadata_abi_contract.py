...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m175_validation_generic_metadata_abi_contract"
)
...
LANE_CONTRACT = "m175-generic-metadata-abi-lowering-v1"
LOWERING_PREFIX = "; generic_metadata_abi_lowering = "
PROFILE_PREFIX = "; frontend_objc_generic_metadata_abi_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_generic_metadata_abi_lowering_profile = "
    r"generic_metadata_abi_sites=(\d+), generic_suffix_sites=(\d+), protocol_composition_sites=(\d+), "
    r"ownership_qualifier_sites=(\d+), object_pointer_type_sites=(\d+), pointer_declarator_sites=(\d+), "
    r"normalized_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_generic_metadata_abi_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!28 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    generic_metadata_abi_sites: int,
    ...
    return (
        f"generic_metadata_abi_sites={generic_metadata_abi_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["generic_metadata_abi_lowering_sites"],
        sema["generic_metadata_abi_lowering_generic_suffix_sites"],
        sema["generic_metadata_abi_lowering_protocol_composition_sites"],
        sema["generic_metadata_abi_lowering_ownership_qualifier_sites"],
        sema["generic_metadata_abi_lowering_object_pointer_type_sites"],
        sema["generic_metadata_abi_lowering_pointer_declarator_sites"],
        sema["generic_metadata_abi_lowering_normalized_sites"],
        sema["generic_metadata_abi_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !28 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_generic_metadata_abi_lowering_surface"
    ]
    lowering = manifest["lowering_generic_metadata_abi"]

    generic_metadata_abi_sites = sema["generic_metadata_abi_lowering_sites"]
    generic_suffix_sites = sema["generic_metadata_abi_lowering_generic_suffix_sites"]
    protocol_composition_sites = sema["generic_metadata_abi_lowering_protocol_composition_sites"]
    ownership_qualifier_sites = sema["generic_metadata_abi_lowering_ownership_qualifier_sites"]
    object_pointer_type_sites = sema["generic_metadata_abi_lowering_object_pointer_type_sites"]
    pointer_declarator_sites = sema["generic_metadata_abi_lowering_pointer_declarator_sites"]
    normalized_sites = sema["generic_metadata_abi_lowering_normalized_sites"]
    contract_violation_sites = sema["generic_metadata_abi_lowering_contract_violation_sites"]
    deterministic_handoff = sema["deterministic_generic_metadata_abi_lowering_handoff"]

    expected_replay_key = _expected_replay_key(
        generic_metadata_abi_sites=generic_metadata_abi_sites,
    ...
    assert generic_suffix_sites <= generic_metadata_abi_sites
    assert protocol_composition_sites <= generic_metadata_abi_sites
    assert ownership_qualifier_sites <= generic_metadata_abi_sites
    assert object_pointer_type_sites >= protocol_composition_sites
    assert pointer_declarator_sites <= generic_metadata_abi_sites
    assert normalized_sites <= generic_metadata_abi_sites
    assert contract_violation_sites <= generic_metadata_abi_sites

    assert deterministic_handoff is True
    assert sema["lowering_generic_metadata_abi_replay_key"] == expected_replay_key

    assert surface["generic_metadata_abi_sites"] == generic_metadata_abi_sites
...
def test_m175_validation_runbook_section_exists() -> None:
...
def test_m175_validation_manifest_packets_hold_deterministic_generic_metadata_abi_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m175_validation_ir_markers_match_manifest_generic_metadata_abi_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_generic_metadata_abi_lowering = !{!28}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_generic_metadata_abi"]["replay_key"]


def test_m175_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_generic_metadata_abi"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_generic_metadata_abi"]["replay_key"]
