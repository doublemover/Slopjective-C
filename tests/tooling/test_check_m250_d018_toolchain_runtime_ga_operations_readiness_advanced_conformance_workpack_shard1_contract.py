...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m250_d018_toolchain_runtime_ga_operations_readiness_advanced_conformance_workpack_shard1_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_d018_toolchain_runtime_ga_operations_readiness_advanced_conformance_workpack_shard1_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m250-toolchain-runtime-ga-operations-readiness-advanced-conformance-workpack-shard1-contract-d018-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-toolchain-runtime-ga-operations-readiness-advanced-conformance-workpack-shard1/m250-d018-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-D018-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_advanced_conformance_consistency(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M250-D018-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_advanced_conformance_failure_reason_is_missing(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M250-D018-SUR-08" for failure in payload["failures"])
