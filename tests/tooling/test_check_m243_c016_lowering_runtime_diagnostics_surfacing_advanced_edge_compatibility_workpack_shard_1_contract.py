...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_c016_lowering_runtime_diagnostics_surfacing_advanced_edge_compatibility_workpack_shard_1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-c016-lowering-runtime-diagnostics-surfacing-advanced-edge-compatibility-workpack-shard-1-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m243_c016() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m243/M243-C016/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m243_c016_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-C016-DOC-02" for failure in payload["failures"])


def test_contract_fails_closed_when_package_readiness_chain_drops_c015(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C016-PKG-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_architecture_anchor_drifts(
    ...
    drift_architecture.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "M243 lane-C C016 guardrails placeholder anchor",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C016-ARC-01" for failure in payload["failures"])
