...
def test_contract_default_summary_out_is_under_tmp_reports_m246_e005() -> None:
...
def test_contract_fails_closed_when_expectations_drop_dependency_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E005-DOC-EXP-08" for failure in payload["failures"])
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E005-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_on_read_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E005-DOC-EXP-EXISTS-READ" for failure in payload["failures"])
...
def test_contract_failure_ordering_is_deterministic(tmp_path: Path) -> None:
    ...
    missing_run_script = tmp_path / "missing_run.py"
    ...
    assert any(failure["check_id"] == "M246-E005-DOC-EXP-EXISTS" for failure in failures)
    assert any(failure["check_id"] == "M246-E005-DOC-PKT-EXISTS" for failure in failures)
    assert any(failure["check_id"] == "M246-E005-RUN-EXISTS" for failure in failures)
