...
SPEC = importlib.util.spec_from_file_location(
    "check_m243_b006_semantic_diagnostic_taxonomy_and_fix_it_synthesis_edge_case_expansion_and_robustness_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_b006_semantic_diagnostic_taxonomy_and_fix_it_synthesis_edge_case_expansion_and_robustness_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-b006-semantic-diagnostic-taxonomy-and-fixit-synthesis-edge-case-expansion-and-robustness-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m243_b006() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m243/M243-B006/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m243_b006_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-B006-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_package_readiness_chain_drops_b005(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-B006-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_edge_case_robustness_ready(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["robustness_surface_header"].read_text(encoding="utf-8")
        + "\nsurface.edge_case_robustness_ready = true;\n",
        encoding="utf-8",
    )

    artifacts = dict(contract.ARTIFACTS)
    artifacts["robustness_surface_header"] = drift_surface
    monkeypatch.setattr(contract, "ARTIFACTS", artifacts)
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-B006-FORB-01" for failure in payload["failures"])
