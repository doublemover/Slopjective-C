...
CONFORMANCE_FIXTURE = FIXTURE_ROOT / "M184-D001.json"
...
def test_m184_conformance_fixture_fields_are_canonical() -> None:
    fixture = json.loads(_read(CONFORMANCE_FIXTURE))

    assert fixture["id"] == "M184-D001"
    assert fixture["bucket"] == "lowering_abi"
    assert fixture["profile"] == "core"
    assert fixture["tracking"]["milestone"] == "M184"
    assert fixture["tracking"]["milestone_number"] == 269
    assert fixture["tracking"]["lane"] == "D"
    assert fixture["tracking"]["issue"] == 4512
    assert fixture["tracking"]["task"] == "M184-D001"
    assert fixture["tracking"]["sequence"] == 1


def test_m184_conformance_fixture_references_runbook_and_lane_d_contract_tests() -> None:
    fixture = json.loads(_read(CONFORMANCE_FIXTURE))
    ...
    assert "// EXPECT: unwind_cleanup_lowering" in source
    assert "// EXPECT: deterministic_unwind_cleanup_lowering_handoff=true" in source

    assert expect["parse"] == "accept"
    assert expect["diagnostics"] == []


def test_m184_conformance_fixture_replay_anchor_matches_validation_packets() -> None:
    ...
    assert "lane_contract=m184-unwind-safety-cleanup-emission-lowering-v1" in run1_replay_key

    assert f"; unwind_cleanup_lowering = {run1_replay_key}" in run1_ir
    assert f"; unwind_cleanup_lowering = {run2_replay_key}" in run2_ir
    assert "!objc3.objc_unwind_cleanup_lowering = !{!44}" in run1_ir
    assert "!objc3.objc_unwind_cleanup_lowering = !{!44}" in run2_ir
