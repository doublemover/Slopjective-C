...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m245_e022_lane_e_portability_gate_and_release_checklist_advanced_edge_compatibility_workpack_shard2_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m245_e022_lane_e_portability_gate_and_release_checklist_advanced_edge_compatibility_workpack_shard2_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m245-e022-lane-e-portability-gate-release-checklist-advanced-edge-compatibility-workpack-shard2-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m245_e022() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m245/M245-E022/")
...
def test_contract_fails_closed_when_expectations_dependency_drifts(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Dependencies: `M245-E021`, `M245-A008`, `M245-B999`, `M245-C011`, `M245-D014`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E022-DOC-EXP-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E022-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_issue_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E022-DOC-PKT-03" for failure in payload["failures"])
