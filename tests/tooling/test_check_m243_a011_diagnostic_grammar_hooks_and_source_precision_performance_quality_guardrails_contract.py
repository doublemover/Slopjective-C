...
SPEC = importlib.util.spec_from_file_location(
    "check_m243_a011_diagnostic_grammar_hooks_and_source_precision_performance_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_a011_diagnostic_grammar_hooks_and_source_precision_performance_quality_guardrails_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-a011-diagnostic-grammar-hooks-and-source-precision-performance-quality-guardrails-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m243_a011() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m243/M243-A011/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m243_a011_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-A011-DOC-03" for failure in payload["failures"])


def test_contract_fails_closed_when_package_dependency_anchor_drops_a010(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M243-A011-PKG-03", "M243-A011-FORB-01"}
...
def test_contract_fails_closed_when_architecture_anchor_drifts(
    ...
    drift_architecture.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "M243 lane-A A011 performance and quality guardrails anchors parser diagnostic grammar-hook readiness mismatch",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-A011-ARCH-01" for failure in payload["failures"])
