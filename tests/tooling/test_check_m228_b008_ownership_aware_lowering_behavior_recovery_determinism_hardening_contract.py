...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_b008_ownership_aware_lowering_behavior_recovery_determinism_hardening_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_b008_ownership_aware_lowering_behavior_recovery_determinism_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-b008-ownership-aware-lowering-behavior-recovery-determinism-hardening-contract-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-ownership-aware-lowering-behavior-recovery-determinism-hardening/m228-b008-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M228-B008-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_forbidden_ready_shortcut_appears(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-B008-FORB-01" for failure in payload["failures"])
