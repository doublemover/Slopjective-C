...
SCRIPT = ROOT / "scripts" / "check_m260_b002_retain_release_weak_and_unowned_legality_for_runtime_backed_storage_core_feature_implementation.py"
...
SPEC = importlib.util.spec_from_file_location(
    "check_m260_b002_retain_release_weak_and_unowned_legality_for_runtime_backed_storage_core_feature_implementation",
    SCRIPT,
)
...
def test_checker_passes() -> None:
    summary_path = ROOT / "tmp" / "reports" / "m260" / "M260-B002" / "pytest_runtime_backed_storage_ownership_legality_summary.json"
    result = run_checker(summary_path=summary_path)
    assert result.returncode == 0, result.stderr
    payload = json.loads(summary_path.read_text(encoding="utf-8"))
    assert payload["mode"] == "m260-b002-runtime-backed-storage-ownership-legality-v1"
    assert payload["contract_id"] == "objc3c-runtime-backed-storage-ownership-legality/m260-b002-v1"
    assert payload["next_issue"] == "M260-B003"
...
def test_checker_default_summary_out_is_under_tmp_reports() -> None:
    ...
    assert summary_path.relative_to(ROOT).as_posix().startswith("tmp/reports/m260/M260-B002/")
...
def test_checker_fails_closed_when_expectations_contract_drifts(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-runtime-backed-storage-ownership-legality/m260-b002-drifted`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M260-B002-DOC-01" for failure in payload["failures"])
