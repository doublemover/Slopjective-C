...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m227_b035_type_system_objc3_forms_advanced_diagnostics_workpack_shard4_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m227_b035_type_system_objc3_forms_advanced_diagnostics_workpack_shard4_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m227_b035_type_system_objc3_forms_advanced_diagnostics_workpack_shard4_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m227-type-system-objc3-forms-advanced-diagnostics-workpack-shard4-b035-v1"
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M227-B035-DOC-03" for failure in payload["failures"])


def test_contract_fails_closed_when_lane_readiness_drops_b034_dependency(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            ...
            '"check:objc3c:m227-b035-lane-b-readiness": "npm run check:objc3c:m227-b099-lane-b-readiness',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-B035-PKG-05" for failure in payload["failures"])
...
def test_contract_fails_closed_when_runbook_anchor_drifts(
    ...
    drift_runbook.write_text(
        contract.ARTIFACTS["runbook_doc"].read_text(encoding="utf-8").replace(
            ...
            "objc3c-type-system-objc3-forms-advanced-diagnostics-workpack-shard4/m227-b035-drift",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-B035-RUN-02" for failure in payload["failures"])
