...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "check_activation_seed_hardening_w2_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_activation_seed_hardening_w2_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_activation_seed_hardening_w2_contract.py")
...
def test_happy_path_is_deterministic() -> None:
    ...
    assert first_stdout == (
        "activation-seed-hardening-w2-contract: OK\n"
...
def test_drift_path_returns_exit_1_with_stable_report(tmp_path: Path) -> None:
    ...
    assert "activation-seed-hardening-w2-contract: contract drift detected" in first_stderr
    assert "- W2C-01\n" in first_stderr
    assert "zero_open/json output drift against" in first_stderr
    assert "remediation:\n" in first_stderr
    assert "python scripts/check_activation_seed_hardening_w2_contract.py" in first_stderr
...
def test_hard_fail_returns_exit_2_for_missing_checker_script() -> None:
    ...
    assert stdout == ""
    assert (
        "activation-seed-hardening-w2-contract: error: checker script does not exist:"
        in stderr
    )
