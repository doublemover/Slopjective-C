...
PIPELINE_ORCHESTRATION = ROOT / "native" / "objc3c" / "src" / "pipeline" / "frontend_pipeline_orchestration.cpp"
...
def test_sema_module_exists_and_pipeline_uses_api() -> None:
    ...
    assert PIPELINE_ORCHESTRATION.exists()
    assert PIPELINE_SEMA_STAGE_RUNNER.exists()

    sema_stage_runner = _read(PIPELINE_SEMA_STAGE_RUNNER)
    pipeline_orchestration = _read(PIPELINE_ORCHESTRATION)
    assert '#include "sema/objc3_sema_pass_manager.h"' in sema_stage_runner
    assert "RunObjc3SemaPassManager(sema_input)" in sema_stage_runner
...
def test_cmake_registers_sema_target() -> None:
    ...
    assert "objc3_semantic_passes.cpp" in cmake
