...
SPEC = importlib.util.spec_from_file_location(
    "check_m245_d011_build_link_runtime_reproducibility_operations_performance_and_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m245_d011_build_link_runtime_reproducibility_operations_performance_and_quality_guardrails_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m245-d011-build-link-runtime-reproducibility-operations-performance-and-quality-guardrails-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m245_d011() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m245/M245-D011/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m245_d011_expectations.md"
    ...
    assert any(failure["check_id"] == "M245-D011-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_package_readiness_chain_drops_d009(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-D011-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_d010_checker_dependency_path_missing(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-D011-DEP-D010-ARG-01" for failure in payload["failures"])
