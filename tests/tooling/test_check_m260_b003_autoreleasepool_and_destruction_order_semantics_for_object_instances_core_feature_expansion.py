...
SCRIPT = ROOT / "scripts" / "check_m260_b003_autoreleasepool_and_destruction_order_semantics_for_object_instances_core_feature_expansion.py"
...
SPEC = importlib.util.spec_from_file_location(
    "check_m260_b003_autoreleasepool_and_destruction_order_semantics_for_object_instances_core_feature_expansion",
    SCRIPT,
)
...
def test_checker_passes() -> None:
    summary_path = ROOT / "tmp" / "reports" / "m260" / "M260-B003" / "pytest_autoreleasepool_destruction_order_semantics_summary.json"
    result = run_checker(summary_path=summary_path)
    assert result.returncode == 0, result.stderr
    payload = json.loads(summary_path.read_text(encoding="utf-8"))
    assert payload["mode"] == "m260-b003-autoreleasepool-destruction-order-semantics-v1"
    assert payload["contract_id"] == "objc3c-runtime-backed-autoreleasepool-destruction-order-semantics/m260-b003-v1"
    assert payload["next_issue"] == "M260-C001"
...
def test_checker_default_summary_out_is_under_tmp_reports() -> None:
    ...
    assert summary_path.relative_to(ROOT).as_posix().startswith("tmp/reports/m260/M260-B003/")
...
def test_checker_fails_closed_when_expectations_contract_drifts(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-runtime-backed-autoreleasepool-destruction-order-semantics/m260-b003-drifted`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M260-B003-DOC-01" for failure in payload["failures"])
