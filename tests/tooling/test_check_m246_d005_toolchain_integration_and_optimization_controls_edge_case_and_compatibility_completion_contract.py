...
def test_contract_failures_are_sorted_deterministically(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M246-D005-DOC-EXP-04" for failure in failures)
    assert any(failure["check_id"] == "M246-D005-DOC-EXP-05" for failure in failures)
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M246-D005-DOC-EXP-05" for failure in payload["failures"])
...
def test_contract_fails_closed_when_expectations_doc_is_not_utf8(tmp_path: Path) -> None:
    bad_doc = tmp_path / "m246_d005_expectations.md"
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] == "M246-D005-DOC-EXP-EXISTS"
