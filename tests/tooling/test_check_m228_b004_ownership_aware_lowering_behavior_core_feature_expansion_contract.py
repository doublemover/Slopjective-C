...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_b004_ownership_aware_lowering_behavior_core_feature_expansion_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_b004_ownership_aware_lowering_behavior_core_feature_expansion_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m228-b004-ownership-aware-lowering-behavior-core-feature-expansion-contract-v1"
...
def test_contract_fails_closed_when_expansion_gate_diagnostic_drifts(
    ...
    assert any(failure["check_id"] == "M228-B004-ART-05" for failure in payload["failures"])


def test_contract_fails_closed_when_forbidden_expansion_shortcut_appears(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-B004-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_planning_packet_dependency_drifts(
    ...
    drift_packet = tmp_path / "m228_b004_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-B004-PKT-04" for failure in payload["failures"])
