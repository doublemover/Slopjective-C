...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_c007_ir_emission_completeness_diagnostics_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-c007-ir-emission-completeness-diagnostics-hardening-contract-v1"
    )
...
def test_contract_fails_closed_when_diag_code_drifts(
    ...
    assert any(failure["check_id"] == "M228-C007-ART-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_ir_metadata_line_drifts(
    ...
    drift_ir_source.write_text(
        contract.ARTIFACTS["ir_source"].read_text(encoding="utf-8").replace(
            ...
            'out << "; ir_emission_core_feature_diagnostics_hardening_ready_drift = "',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C007-IRS-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_drifts(
    ...
    drift_packet = (
        tmp_path / "m228_c007_ir_emission_completeness_diagnostics_hardening_packet.md"
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C007-PKT-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_forbidden_shortcut_appears(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C007-FORB-01" for failure in payload["failures"])
