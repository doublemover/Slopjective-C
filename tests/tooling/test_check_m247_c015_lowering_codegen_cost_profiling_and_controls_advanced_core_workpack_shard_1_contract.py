...
SPEC = importlib.util.spec_from_file_location(
    "check_m247_c015_lowering_codegen_cost_profiling_and_controls_advanced_core_workpack_shard_1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m247_c015_lowering_codegen_cost_profiling_and_controls_advanced_core_workpack_shard_1_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m247-c015-lowering-codegen-cost-profiling-controls-advanced-core-workpack-shard-1-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m247_c015() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m247/M247-C015/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m247_c015_dependency.py"),
    )
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m247_c015_expectations_issue_drift.md"
    drift_doc.write_text(
        replace_all_occurrences(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "Issue `#6756` defines canonical lane-C advanced core workpack (shard 1) scope.",
            "Issue `#6999` defines canonical lane-C advanced core workpack (shard 1) scope.",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_issue_metadata_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-DOC-PKT-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_c014_dependency_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-C014-EXP-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_c015_readiness_drops_c014_chain(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-RUN-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_drops_perf_budget_input(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M247-C015-PKG-05" for failure in payload["failures"])
