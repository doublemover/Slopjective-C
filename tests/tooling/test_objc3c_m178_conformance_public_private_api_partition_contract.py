...
FIXTURE = ROOT / "tests" / "conformance" / "lowering_abi" / "M178-D001.json"
...
def test_m178_conformance_mapping_is_documented() -> None:
    coverage_map = _read(COVERAGE_MAP)
    readme = _read(LOWERING_ABI_README)

    assert "| `M178-D001` | `#4482` public/private API partition lowering replay contract | `lowering_abi` |" in coverage_map
    assert "M178-D001.json" in readme


def test_m178_conformance_fixture_is_registered() -> None:
    ...
    group = next(
        item
        for item in groups
        if item["name"] == "m178_lane_d_issue_4482_public_private_api_partition_lowering_replay_contract"
    )
    assert group["issue"] == 4482
    assert group["files"] == ["M178-D001.json"]


def test_m178_conformance_fixture_fields_are_canonical() -> None:
    fixture = json.loads(_read(FIXTURE))

    assert fixture["id"] == "M178-D001"
