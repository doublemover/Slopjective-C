...
SCRIPT_PATH = ROOT / "scripts" / "check_m237_d007_block_runtime_abi_wiring_and_linkage_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m237_d007_block_runtime_abi_wiring_and_linkage_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m237_d007_block_runtime_abi_wiring_and_linkage_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m237-d007-block-runtime-abi-wiring-and-linkage-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m237_d007() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m237/M237-D007/")


def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    drift_doc = (
        tmp_path
        / "m237_block_runtime_abi_wiring_and_linkage_diagnostics_hardening_d007_expectations.md"
    )
    drift_doc.write_text(
        contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            "Issue `#5927` defines canonical lane-D diagnostics hardening scope.",
    ...
    assert any(failure["check_id"] == "M237-D007-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_token_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m237_d007_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M237-D007-DOC-PKT-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M237-D007-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_metadata_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M237-D007-META-01" for failure in payload["failures"])
