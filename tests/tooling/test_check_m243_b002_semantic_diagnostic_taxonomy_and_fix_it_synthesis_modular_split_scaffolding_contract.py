...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m243_b002_semantic_diagnostic_taxonomy_and_fix_it_synthesis_modular_split_scaffolding_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m243_b002_semantic_diagnostic_taxonomy_and_fix_it_synthesis_modular_split_scaffolding_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-semantic-diagnostic-taxonomy-and-fixit-synthesis-modular-split-scaffolding-contract-b002-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-semantic-diagnostic-taxonomy-and-fixit-synthesis-modular-split-scaffolding/m243-b002-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M243-B002-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_scaffold_forces_modular_split_ready(
    ...
    drift_scaffold = tmp_path / "objc3_semantic_diagnostic_taxonomy_and_fix_it_synthesis_scaffold.h"
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M243-B002-SCA-06", "M243-B002-FORB-01"}
