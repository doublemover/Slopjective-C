...
SCRIPT_PATH = (
    Path(__file__).resolve().parents[2] / "scripts" / "check_m08_fixture_manifest_parity.py"
)
SPEC = importlib.util.spec_from_file_location("check_m08_fixture_manifest_parity", SCRIPT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_m08_fixture_manifest_parity.py")
check_m08_fixture_manifest_parity = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = check_m08_fixture_manifest_parity
SPEC.loader.exec_module(check_m08_fixture_manifest_parity)
...
def lane_spec_map() -> dict[str, object]:
    return {spec.lane: spec for spec in check_m08_fixture_manifest_parity.LANE_SPECS}
...
class CheckM08FixtureManifestParityTests(unittest.TestCase):
    def run_main(self, argv: list[str]) -> tuple[int, str]:
        stdout = io.StringIO()
        with redirect_stdout(stdout):
            code = check_m08_fixture_manifest_parity.main(argv)
    ...
    def test_fails_when_fixture_and_manifest_drift_exists(self) -> None:
        ...
        self.assertIn("drift: semantic: manifest missing issue 1547", output)
