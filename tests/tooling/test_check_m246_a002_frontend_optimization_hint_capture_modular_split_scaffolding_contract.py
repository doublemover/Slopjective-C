...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_a002_frontend_optimization_hint_capture_modular_split_scaffolding_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_a002_frontend_optimization_hint_capture_modular_split_scaffolding_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-a002-frontend-optimization-hint-capture-modular-split-scaffolding-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_a002() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-A002/")
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M246-A002-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A002-DOC-EXP-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_chain_drops_a001(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A002-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_a001_checker_dependency_path_missing(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A002-DEP-A001-ARG-01" for failure in payload["failures"])
