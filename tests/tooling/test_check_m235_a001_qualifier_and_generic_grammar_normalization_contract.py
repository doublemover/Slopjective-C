...
SPEC = importlib.util.spec_from_file_location(
    "check_m235_a001_qualifier_and_generic_grammar_normalization_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m235_a001_qualifier_and_generic_grammar_normalization_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m235-a001-qualifier-and-generic-grammar-normalization-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m235_a001() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m235/M235-A001/")
...
def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M235-A001-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M235-A001-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_lowering_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M235-A001-SPC-01" for failure in payload["failures"])
