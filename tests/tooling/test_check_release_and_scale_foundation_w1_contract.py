...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "check_release_and_scale_foundation_w1_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_release_and_scale_foundation_w1_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_release_and_scale_foundation_w1_contract.py"
    )
...
def test_happy_path_is_deterministic() -> None:
    ...
    assert first_stdout == (
        "release-scale-foundation-w1-contract: OK\n"
        "- mode=release-scale-foundation-w1-contract-v1\n"
...
def test_drift_path_returns_exit_1_with_stable_report(tmp_path: Path) -> None:
    ...
    assert "release-scale-foundation-w1-contract: contract drift detected" in first_stderr
    assert "- M37C-01\n" in first_stderr
    assert "zero_open/json output drift against" in first_stderr
    assert "remediation:\n" in first_stderr
    assert "python scripts/check_release_and_scale_foundation_w1_contract.py" in first_stderr
...
def test_hard_fail_returns_exit_2_for_missing_checker_script() -> None:
    ...
    assert stdout == ""
    assert (
        "release-scale-foundation-w1-contract: error: checker script does not exist:"
        in stderr
    )
