...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_b003_ownership_aware_lowering_behavior_core_feature_implementation_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_b003_ownership_aware_lowering_behavior_core_feature_implementation_contract.py"
    )
...
def test_contract_fails_closed_when_artifact_gate_diagnostic_drifts(
    tmp_path: Path,
    ...
    assert any(failure["check_id"] == "M228-B003-ART-03" for failure in payload["failures"])


def test_contract_fails_closed_when_forbidden_scaffold_shortcut_appears(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
