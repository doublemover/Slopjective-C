...
SCRIPT_PATH = ROOT / "scripts" / "check_m226_c003_parse_lowering_core_readiness_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m226_c003_parse_lowering_core_readiness_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m226-c003-parse-lowering-core-readiness-contract-v1"
...
def test_contract_fails_closed_when_readiness_drops_parse_artifact_handoff_gate(
    tmp_path: Path,
    ...
    assert any(failure["check_id"] == "M226-C003-RDY-07" for failure in payload["failures"])


def test_contract_fails_closed_when_manifest_drops_parse_artifact_handoff_key_projection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    artifacts_drift.write_text(
        ...
        .replace(
            ...
            '\\"parse_artifact_handoff_removed\\":\\"',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M226-C003-ART-03" for failure in payload["failures"])
