...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m169_validation_block_copy_dispose_contract"
)
...
LOWERING_PREFIX = "; block_copy_dispose_lowering = "
PROFILE_PREFIX = "; frontend_objc_block_copy_dispose_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_block_copy_dispose_lowering_profile = "
    ...
    r"copy_helper_required_sites=(\d+), dispose_helper_required_sites=(\d+), "
    r"profile_normalized_sites=(\d+), copy_helper_symbolized_sites=(\d+), "
    r"dispose_helper_symbolized_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_block_copy_dispose_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!22 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    ...
    profile_normalized_sites: int,
    copy_helper_symbolized_sites: int,
    dispose_helper_symbolized_sites: int,
    ...
    return (
        ...
        f"copy_helper_required_sites={copy_helper_required_sites};"
        f"dispose_helper_required_sites={dispose_helper_required_sites};"
        f"profile_normalized_sites={profile_normalized_sites};"
        f"copy_helper_symbolized_sites={copy_helper_symbolized_sites};"
        f"dispose_helper_symbolized_sites={dispose_helper_symbolized_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["block_copy_dispose_lowering_sites"],
        sema["block_copy_dispose_lowering_mutable_capture_count"],
        sema["block_copy_dispose_lowering_byref_slot_count"],
        sema["block_copy_dispose_lowering_parameter_entries"],
        sema["block_copy_dispose_lowering_capture_entries"],
        sema["block_copy_dispose_lowering_body_statement_entries"],
        sema["block_copy_dispose_lowering_copy_helper_required_sites"],
        sema["block_copy_dispose_lowering_dispose_helper_required_sites"],
        sema["block_copy_dispose_lowering_profile_normalized_sites"],
        sema["block_copy_dispose_lowering_copy_helper_symbolized_sites"],
        sema["block_copy_dispose_lowering_dispose_helper_symbolized_sites"],
        sema["block_copy_dispose_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !22 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"]["objc_block_copy_dispose_lowering_surface"]
    lowering = manifest["lowering_block_copy_dispose"]

    block_literal_sites = sema["block_copy_dispose_lowering_sites"]
    mutable_capture_count_total = sema["block_copy_dispose_lowering_mutable_capture_count"]
    byref_slot_count_total = sema["block_copy_dispose_lowering_byref_slot_count"]
    parameter_entries_total = sema["block_copy_dispose_lowering_parameter_entries"]
    capture_entries_total = sema["block_copy_dispose_lowering_capture_entries"]
    body_statement_entries_total = sema["block_copy_dispose_lowering_body_statement_entries"]
    copy_helper_required_sites = sema["block_copy_dispose_lowering_copy_helper_required_sites"]
    dispose_helper_required_sites = sema["block_copy_dispose_lowering_dispose_helper_required_sites"]
    profile_normalized_sites = sema["block_copy_dispose_lowering_profile_normalized_sites"]
    copy_helper_symbolized_sites = sema["block_copy_dispose_lowering_copy_helper_symbolized_sites"]
    dispose_helper_symbolized_sites = sema["block_copy_dispose_lowering_dispose_helper_symbolized_sites"]
    contract_violation_sites = sema["block_copy_dispose_lowering_contract_violation_sites"]
    deterministic_handoff = sema["deterministic_block_copy_dispose_lowering_handoff"]
    ...
    expected_replay_key = _expected_replay_key(
        ...
        copy_helper_required_sites=copy_helper_required_sites,
        dispose_helper_required_sites=dispose_helper_required_sites,
        profile_normalized_sites=profile_normalized_sites,
        copy_helper_symbolized_sites=copy_helper_symbolized_sites,
        dispose_helper_symbolized_sites=dispose_helper_symbolized_sites,
        contract_violation_sites=contract_violation_sites,
        deterministic=deterministic_handoff,
    )

    assert copy_helper_required_sites <= block_literal_sites
    assert dispose_helper_required_sites <= block_literal_sites
    assert profile_normalized_sites <= block_literal_sites
    assert copy_helper_symbolized_sites <= block_literal_sites
    assert dispose_helper_symbolized_sites <= block_literal_sites
    ...
    assert copy_helper_required_sites == dispose_helper_required_sites

    assert deterministic_handoff is True
    assert sema["lowering_block_copy_dispose_replay_key"] == expected_replay_key
    ...
    assert surface["copy_helper_required_sites"] == copy_helper_required_sites
    assert surface["dispose_helper_required_sites"] == dispose_helper_required_sites
    assert surface["profile_normalized_sites"] == profile_normalized_sites
    assert surface["copy_helper_symbolized_sites"] == copy_helper_symbolized_sites
    assert surface["dispose_helper_symbolized_sites"] == dispose_helper_symbolized_sites
...
def test_m169_validation_runbook_section_exists() -> None:
...
def test_m169_validation_manifest_packets_hold_deterministic_block_copy_dispose_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m169_validation_ir_markers_match_manifest_block_copy_dispose_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_block_copy_dispose_lowering = !{!22}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_block_copy_dispose"]["replay_key"]


def test_m169_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_block_copy_dispose"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_block_copy_dispose"]["replay_key"]
