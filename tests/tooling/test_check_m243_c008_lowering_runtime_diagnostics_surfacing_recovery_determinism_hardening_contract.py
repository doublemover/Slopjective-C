...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_c008_lowering_runtime_diagnostics_surfacing_recovery_determinism_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-c008-lowering-runtime-diagnostics-surfacing-recovery-determinism-hardening-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m243_c008() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m243/M243-C008/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m243_c008_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-C008-DOC-02" for failure in payload["failures"])


def test_contract_fails_closed_when_package_readiness_chain_drops_c007(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            ...
            "&& npm run check:objc3c:m243-c008-lowering-runtime-diagnostics-surfacing-recovery-determinism-hardening-contract "
            '&& npm run test:tooling:m243-c008-lowering-runtime-diagnostics-surfacing-recovery-determinism-hardening-contract"',
            '"check:objc3c:m243-c008-lane-c-readiness": '
            '"npm run check:objc3c:m243-c008-lowering-runtime-diagnostics-surfacing-recovery-determinism-hardening-contract '
            '&& npm run test:tooling:m243-c008-lowering-runtime-diagnostics-surfacing-recovery-determinism-hardening-contract"',
            1,
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C008-CFG-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_recovery_determinism_ready(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C008-FORB-01" for failure in payload["failures"])
