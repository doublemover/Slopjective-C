...
SPEC = importlib.util.spec_from_file_location(
    "check_m245_c011_lowering_ir_portability_contracts_performance_and_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m245_c011_lowering_ir_portability_contracts_performance_and_quality_guardrails_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m245-c011-lowering-ir-portability-contracts-performance-and-quality-guardrails-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m245_c011() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m245/M245-C011/")
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_all_occurrences(
            ...
            "Issue `#6999` defines canonical lane-C performance and quality guardrails scope.",
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M245-C011-DOC-EXP-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_token_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-C011-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_c010_checker_mode_drifts(tmp_path: Path) -> None:
    drift_checker = (
        tmp_path
    ...
    drift_checker.write_text(
        replace_all_occurrences(
            contract.DEFAULT_C010_CHECKER.read_text(encoding="utf-8"),
            'MODE = "m245-c010-lowering-ir-portability-contracts-conformance-corpus-expansion-contract-v1"',
            'MODE = "m245-c010-lowering-ir-portability-contracts-conformance-corpus-expansion-contract-drift"',
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-C011-C010-CHK-01" for failure in payload["failures"])
