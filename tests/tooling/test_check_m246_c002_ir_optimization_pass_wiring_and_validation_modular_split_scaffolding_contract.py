...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-c002-ir-optimization-pass-wiring-validation-modular-split-scaffolding-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_c002() -> None:
...
def test_contract_fails_closed_when_expectations_drop_dependency_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-C002-DOC-EXP-04" for failure in payload["failures"])
