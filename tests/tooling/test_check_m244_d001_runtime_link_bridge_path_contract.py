...
SCRIPT_PATH = ROOT / "scripts" / "check_m244_d001_runtime_link_bridge_path_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m244_d001_runtime_link_bridge_path_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_m244_d001_runtime_link_bridge_path_contract.py")
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m244-d001-runtime-link-bridge-path-contract-v1"
    assert payload["ok"] is True
    assert payload["checks_total"] >= 38
    assert payload["checks_total"] == payload["checks_passed"]


def test_contract_default_summary_out_is_under_tmp_reports_m244_d001() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m244/M244-D001/")
...
def test_contract_fails_closed_when_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m244_d001_expectations.md"
    ...
    assert any(failure["check_id"] == "M244-D001-DOC-03" for failure in payload["failures"])


def test_contract_fails_closed_when_architecture_fail_closed_anchor_drifts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drift_architecture.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "runtime/link bridge-path dependency continuity remains deterministic",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-D001-ARCH-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_readiness_drops_tooling_test(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M244-D001-PKG-03", "M244-D001-FORB-01"}
