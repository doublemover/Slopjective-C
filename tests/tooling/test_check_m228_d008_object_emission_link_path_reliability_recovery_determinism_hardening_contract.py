...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_d008_object_emission_link_path_reliability_recovery_determinism_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-d008-object-emission-link-path-reliability-recovery-determinism-hardening-contract-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-object-emission-link-path-reliability-recovery-determinism-hardening/m228-d008-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M228-D008-DOC-01" for failure in payload["failures"])


def test_contract_fails_closed_when_forbidden_recovery_ready_shortcut_appears(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D008-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_planning_packet_dependency_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D008-PKT-04" for failure in payload["failures"])
