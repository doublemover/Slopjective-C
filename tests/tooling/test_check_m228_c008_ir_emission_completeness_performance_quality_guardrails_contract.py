...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m228_c011_ir_emission_completeness_performance_quality_guardrails_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m228_c011_ir_emission_completeness_performance_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_c011_ir_emission_completeness_performance_quality_guardrails_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-c011-ir-emission-completeness-performance-quality-guardrails-contract-v1"
    )
...
def test_contract_fails_closed_when_diag_code_drifts(
    ...
    assert any(failure["check_id"] == "M228-C011-ART-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_ir_metadata_line_drifts(
    ...
    drift_ir_source.write_text(
        contract.ARTIFACTS["ir_source"].read_text(encoding="utf-8").replace(
            ...
            'out << "; ir_emission_core_feature_performance_quality_guardrails_ready_drift = "',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C011-IRS-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C011-PKT-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_forbidden_shortcut_appears(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-C011-FORB-01" for failure in payload["failures"])
