...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m248_d024_runner_reliability_and_platform_operations_advanced_conformance_workpack_shard2_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m248_d024_runner_reliability_and_platform_operations_advanced_conformance_workpack_shard2_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m248-runner-reliability-platform-operations-advanced-conformance-workpack-shard2-contract-d024-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    assert any(failure["check_id"] == "M248-D024-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_dependency_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["d023_contract_doc"].read_text(encoding="utf-8").replace(
    ...
    artifact_overrides = dict(contract.ARTIFACTS)
    artifact_overrides["d023_contract_doc"] = drift_doc
    monkeypatch.setattr(contract, "ARTIFACTS", artifact_overrides)
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D024-DEP-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_advanced_conformance_consistency(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D024-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_advanced_conformance_failure_reason_is_missing(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["parse_surface_header"].read_text(encoding="utf-8").replace(
            ...
            "toolchain/runtime GA operations advanced conformance shard2 drift marker",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-D024-SUR-08" for failure in payload["failures"])
