...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m178_validation_public_private_api_partition_contract"
)
...
LANE_CONTRACT = "m178-public-private-api-partition-lowering-v1"
LOWERING_PREFIX = "; public_private_api_partition_lowering = "
PROFILE_PREFIX = "; frontend_objc_public_private_api_partition_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_public_private_api_partition_lowering_profile = "
    r"public_private_api_partition_sites=(\d+), namespace_segment_sites=(\d+), import_edge_candidate_sites=(\d+), "
    r"object_pointer_type_sites=(\d+), pointer_declarator_sites=(\d+), normalized_sites=(\d+), "
    r"contract_violation_sites=(\d+), deterministic_public_private_api_partition_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!31 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    public_private_api_partition_sites: int,
    ...
    return (
        f"public_private_api_partition_sites={public_private_api_partition_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int]:
    ...
    return (
        sema["public_private_api_partition_lowering_sites"],
        sema["public_private_api_partition_lowering_namespace_segment_sites"],
        sema["public_private_api_partition_lowering_import_edge_candidate_sites"],
        sema["public_private_api_partition_lowering_object_pointer_type_sites"],
        sema["public_private_api_partition_lowering_pointer_declarator_sites"],
        sema["public_private_api_partition_lowering_normalized_sites"],
        sema["public_private_api_partition_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !31 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_public_private_api_partition_lowering_surface"
    ]
    lowering = manifest["lowering_public_private_api_partition"]

    public_private_api_partition_sites = sema["public_private_api_partition_lowering_sites"]
    namespace_segment_sites = sema["public_private_api_partition_lowering_namespace_segment_sites"]
    import_edge_candidate_sites = sema["public_private_api_partition_lowering_import_edge_candidate_sites"]
    object_pointer_type_sites = sema["public_private_api_partition_lowering_object_pointer_type_sites"]
    pointer_declarator_sites = sema["public_private_api_partition_lowering_pointer_declarator_sites"]
    normalized_sites = sema["public_private_api_partition_lowering_normalized_sites"]
    contract_violation_sites = sema["public_private_api_partition_lowering_contract_violation_sites"]
    deterministic_handoff = sema["deterministic_public_private_api_partition_lowering_handoff"]

    expected_replay_key = _expected_replay_key(
        public_private_api_partition_sites=public_private_api_partition_sites,
    ...
    assert namespace_segment_sites <= public_private_api_partition_sites
    assert import_edge_candidate_sites <= public_private_api_partition_sites
    assert object_pointer_type_sites >= import_edge_candidate_sites
    assert pointer_declarator_sites <= public_private_api_partition_sites
    assert normalized_sites <= public_private_api_partition_sites
    assert contract_violation_sites <= public_private_api_partition_sites

    assert deterministic_handoff is True
    assert sema["lowering_public_private_api_partition_replay_key"] == expected_replay_key

    assert surface["public_private_api_partition_sites"] == public_private_api_partition_sites
...
def test_m178_validation_runbook_section_exists() -> None:
    ...
    for text in (
        ...
        "Current gap: dedicated M178-C IR emitter markers are not yet source-emitted",
    ):
        assert text in fragment


def test_m178_validation_manifest_packets_hold_deterministic_public_private_api_partition_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m178_validation_ir_markers_match_manifest_public_private_api_partition_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_public_private_api_partition_lowering = !{!31}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_public_private_api_partition"]["replay_key"]


def test_m178_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_public_private_api_partition"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_public_private_api_partition"]["replay_key"]
