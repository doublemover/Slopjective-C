...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m240_a007_metadata_declaration_source_modeling_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m240_a007_metadata_declaration_source_modeling_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m240_a007_metadata_declaration_source_modeling_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m240-a007-metadata-declaration-source-modeling-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m240_a007() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m240/M240-A007/")
...
def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    ...
    assert any(failure["check_id"] == "M240-A007-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M240-A007-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_lowering_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M240-A007-SPC-01" for failure in payload["failures"])
