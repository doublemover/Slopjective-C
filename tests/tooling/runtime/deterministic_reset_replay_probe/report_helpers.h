...
inline void PrintPostResetReportFields(const PostResetState &post_reset) {
  ...
  PrintUint64Field(
      ...
      JsonU64(
          post_reset.reset_replay.snapshot.retained_bootstrap_image_count));
  PrintUint64Field("post_reset_reset_generation",
                   JsonU64(post_reset.reset_replay.snapshot.reset_generation));
}

inline void PrintPostReplayReportFields(
    const ResetReplayCycleResult &reset_replay) {
  using objc3c::runtime::probe::PrintIntField;
  ...
  PrintStringField(
      ...
      post_replay.image_walk.snapshot.last_walked_translation_unit_identity_key);
  PrintStringField(
