...
int main() {
  ...
  const int nonisolated = objc3_runtime_actor_enter_nonisolated_i32(11, 0);
  const int hopped = objc3_runtime_actor_hop_to_executor_i32(13, 1);
  ...
  return (copy_status == 0 && replay == 1 && guard == 1 && isolation == 1 &&
          ...
          snapshot.last_nonisolated_value == 11 &&
          snapshot.last_nonisolated_executor_tag == 0 &&
          snapshot.last_hop_value == 13 &&
          snapshot.last_hop_executor_tag == 1 &&
          snapshot.last_hop_result == 13)
             ? 0
             : 1;
}
