#ifndef OBJC3C_TESTS_TOOLING_RUNTIME_SYNTHESIZED_ACCESSOR_PROBE_ACCESSOR_FIXTURE_DEFINITIONS_H_
#define OBJC3C_TESTS_TOOLING_RUNTIME_SYNTHESIZED_ACCESSOR_PROBE_ACCESSOR_FIXTURE_DEFINITIONS_H_

namespace objc3c::runtime::probe::synthesized_accessor {

inline constexpr int kWidgetClassReceiver = 1024;
inline constexpr int kCountSetterValue = 37;
inline constexpr int kEnabledSetterValue = 1;
inline constexpr int kValueSetterValue = 55;
inline constexpr const char *kAllocSelector = "alloc";
inline constexpr const char *kCountGetterSelector = "count";
inline constexpr const char *kCountSetterSelector = "setCount:";
inline constexpr const char *kEnabledGetterSelector = "enabled";
inline constexpr const char *kEnabledSetterSelector = "setEnabled:";
inline constexpr const char *kValueGetterSelector = "value";
inline constexpr const char *kValueSetterSelector = "setValue:";

}  // namespace objc3c::runtime::probe::synthesized_accessor

#endif  // OBJC3C_TESTS_TOOLING_RUNTIME_SYNTHESIZED_ACCESSOR_PROBE_ACCESSOR_FIXTURE_DEFINITIONS_H_
