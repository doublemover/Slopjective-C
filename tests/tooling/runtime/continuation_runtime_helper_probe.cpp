...
int main() {
  ...
  const int unknown_resume = objc3_runtime_resume_async_continuation_i32(2, 102);
  ...
  return (handle == 1 && handed_off == 1 && resumed == 77 &&
          double_resumed == 0 && double_resume_failure_code == 4 &&
          missing_payload_handle == 65537 && missing_payload_resumed == 0 &&
          missing_payload_failure_code == 6 &&
          cancelled_handle == 131073 && cancelled == 131073 &&
          resume_after_cancel == 0 && resume_after_cancel_failure_code == 5 &&
          recycled_handle == 196609 && stale_generation_resume == 0 &&
          ...
          snapshot.allocation_call_count == 5 &&
          snapshot.handoff_call_count == 1 && snapshot.resume_call_count == 7 &&
          snapshot.cancel_call_count == 1 &&
          snapshot.rejected_operation_count == 7 &&
          snapshot.live_continuation_handle_count == 1 &&
          snapshot.completed_continuation_count == 1 &&
          snapshot.cancelled_continuation_count == 1 &&
