...
int main() {
  ...
  const int bridged_foreign_missing_payload =
      objc3_runtime_bridge_foreign_exception_error_i32(3, 0, 0);
  const int bridged_foreign_invalid_kind =
  ...
  std::printf("bridged_foreign_missing_payload=%d\n",
              bridged_foreign_missing_payload);
  std::printf("bridged_foreign_invalid_kind=%d\n",
              bridged_foreign_invalid_kind);
  ...
  std::printf("foreign_exception_bridge_call_count=%llu\n",
  ...
  std::printf("last_foreign_exception_kind=%d\n",
              snapshot.last_foreign_exception_kind);
  ...
  std::printf("last_foreign_exception_kind_name=%s\n",
              snapshot.last_foreign_exception_kind_name != nullptr
  ...
  if (status != OBJC3_RUNTIME_REGISTRATION_STATUS_OK || loaded != 34 ||
      ...
      match_nserror != 1 || match_protocol != 1 || match_foreign != 1 ||
      match_catch_all != 1 ||
      ...
      snapshot.foreign_exception_bridge_call_count != 3 ||
      snapshot.catch_match_call_count != 4 ||
      ...
      snapshot.last_foreign_exception_kind != 9 ||
      snapshot.last_foreign_exception_payload_value != 88 ||
      snapshot.last_foreign_exception_mapped_error_value != 99 ||
      snapshot.last_foreign_exception_bridge_result !=
          OBJC3_RUNTIME_FOREIGN_EXCEPTION_BRIDGE_INVALID_KIND ||
