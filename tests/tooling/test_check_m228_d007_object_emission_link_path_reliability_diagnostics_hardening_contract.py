...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_d007_object_emission_link_path_reliability_diagnostics_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-d007-object-emission-link-path-reliability-diagnostics-hardening-contract-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-object-emission-link-path-reliability-diagnostics-hardening/m228-d007-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M228-D007-DOC-01" for failure in payload["failures"])


def test_contract_fails_closed_when_forbidden_diagnostics_key_shortcut_appears(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D007-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_planning_packet_dependency_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D007-PKT-04" for failure in payload["failures"])
