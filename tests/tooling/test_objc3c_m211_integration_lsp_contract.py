...
def test_m211_integration_lsp_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m211-lsp-semantics"' in package_json
    assert (
        "npm run check:objc3c:m212-code-action && python -m pytest "
        "tests/tooling/test_objc3c_m211_frontend_lsp_contract.py "
        "tests/tooling/test_objc3c_m211_sema_lsp_contract.py "
        "tests/tooling/test_objc3c_m211_lowering_lsp_contract.py "
        "tests/tooling/test_objc3c_m211_validation_lsp_contract.py "
        "tests/tooling/test_objc3c_m211_integration_lsp_contract.py -q"
    ) in package_json

    assert "Run M211 LSP semantic integration gate" in workflow
    assert "npm run check:objc3c:m211-lsp-semantics" in workflow
