...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m244_e009_lane_e_interop_conformance_gate_and_operations_conformance_matrix_implementation_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m244_e009_lane_e_interop_conformance_gate_and_operations_conformance_matrix_implementation_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m244_e009_lane_e_interop_conformance_gate_and_operations_conformance_matrix_implementation_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m244-e009-lane-e-interop-conformance-gate-operations-conformance-matrix-implementation-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m244_e009() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m244/M244-E009/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    assert any(failure["check_id"] == "M244-E009-DOC-EXP-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_reference_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E009-DOC-PKT-10" for failure in payload["failures"])
...
def test_contract_fails_closed_when_readiness_command_drops_dependency_references(
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            ...
            '&& npm run check:objc3c:m244-e009-lane-e-interop-conformance-gate-operations-conformance-matrix-implementation-contract '
            '&& npm run test:tooling:m244-e009-lane-e-interop-conformance-gate-operations-conformance-matrix-implementation-contract"',
            '"check:objc3c:m244-e009-lane-e-readiness": '
            '"npm run check:objc3c:m244-e008-lane-e-readiness '
            '&& npm run check:objc3c:m244-a007-lane-a-readiness '
            '&& npm run --if-present check:objc3c:m244-b010-lane-b-readiness '
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E009-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_drops_perf_budget_optimization_input(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M244-E009-PKG-08" for failure in payload["failures"])
