...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_a010_frontend_optimization_hint_capture_conformance_corpus_expansion_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_a010_frontend_optimization_hint_capture_conformance_corpus_expansion_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-a010-frontend-optimization-hint-capture-conformance-corpus-expansion-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_a010() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-A010/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_a010_expectations.md"
    ...
    assert any(failure["check_id"] == "M246-A010-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_token_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_a010_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A010-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_expectations_issue_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_a010_expectations_issue.md"
    drift_doc.write_text(
        replace_first(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "- Issue: `#5057`",
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A010-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_runner_chain_drops_a009_prerequisite(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m246_a010_lane_a_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A010-RUN-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_key_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-A010-PKG-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("docs/contracts/does_not_exist_m246_a010.md"),
    )
