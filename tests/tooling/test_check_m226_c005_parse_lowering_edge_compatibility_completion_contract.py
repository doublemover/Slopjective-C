...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m226_c005_parse_lowering_edge_compatibility_completion_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m226_c005_parse_lowering_edge_compatibility_completion_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m226-c005-parse-lowering-edge-compatibility-completion-contract-v1"
...
def test_contract_fails_closed_when_readiness_drops_compatibility_handoff_gate(
    tmp_path: Path,
    ...
    assert any(failure["check_id"] == "M226-C005-RDY-13" for failure in payload["failures"])


def test_contract_fails_closed_when_manifest_drops_compatibility_handoff_projection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M226-C005-ART-05" for failure in payload["failures"])
