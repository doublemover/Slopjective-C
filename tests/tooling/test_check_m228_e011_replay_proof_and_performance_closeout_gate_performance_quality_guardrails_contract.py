...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m228_e011_replay_proof_and_performance_closeout_gate_performance_quality_guardrails_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m228_e011_replay_proof_and_performance_closeout_gate_performance_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_e011_replay_proof_and_performance_closeout_gate_performance_quality_guardrails_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-e011-replay-proof-performance-closeout-gate-performance-quality-guardrails-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m228_e011() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m228/M228-E011/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("docs/contracts/does_not_exist_m228_e011.md"),
    )
...
def test_contract_fails_closed_when_expectations_drop_pending_c017_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-E011-DOC-EXP-08" for failure in payload["failures"])
...
def test_contract_fails_closed_when_packet_dependency_anchor_drifts(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8").replace(
            ...
            "Dependencies: `M228-E010`, `M228-A009`, `M228-B099`, `M228-C008`, `M228-D009`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-E011-DOC-PKT-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_chain_drops_c008_readiness(tmp_path: Path) -> None:
    ...
    scripts["check:objc3c:m228-e011-lane-e-readiness"] = scripts[
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-E011-CFG-06" for failure in payload["failures"])
