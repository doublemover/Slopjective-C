...
SCRIPT_PATH = ROOT / "scripts" / "check_m250_c004_lowering_runtime_stability_core_feature_expansion_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m250_c004_lowering_runtime_stability_core_feature_expansion_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m250-lowering-runtime-stability-core-feature-expansion-contract-c004-v1"
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc = tmp_path / "m250_lowering_runtime_stability_core_feature_expansion_c004_expectations.md"
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-lowering-runtime-stability-core-feature-expansion/m250-c004-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-C004-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_expansion_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-C004-SUR-04", "M250-C004-FORB-01"}
