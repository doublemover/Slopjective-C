...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m170_validation_block_determinism_perf_baseline_contract"
)
...
LANE_CONTRACT = "m170-block-determinism-perf-baseline-lowering-v1"
LOWERING_PREFIX = "; block_determinism_perf_baseline_lowering = "
PROFILE_PREFIX = "; frontend_objc_block_determinism_perf_baseline_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_block_determinism_perf_baseline_lowering_profile = "
    r"block_literal_sites=(\d+), baseline_weight_total=(\d+), parameter_entries_total=(\d+), "
    r"capture_entries_total=(\d+), body_statement_entries_total=(\d+), "
    r"deterministic_capture_sites=(\d+), heavy_tier_sites=(\d+), "
    r"normalized_profile_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_block_determinism_perf_baseline_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!23 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    ...
    baseline_weight_total: int,
    parameter_entries_total: int,
    capture_entries_total: int,
    body_statement_entries_total: int,
    deterministic_capture_sites: int,
    heavy_tier_sites: int,
    normalized_profile_sites: int,
    ...
    return (
        ...
        f"baseline_weight_total={baseline_weight_total};"
        ...
        f"deterministic_capture_sites={deterministic_capture_sites};"
        f"heavy_tier_sites={heavy_tier_sites};"
        f"normalized_profile_sites={normalized_profile_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["block_determinism_perf_baseline_lowering_sites"],
        sema["block_determinism_perf_baseline_lowering_weight_total"],
        sema["block_determinism_perf_baseline_lowering_parameter_entries"],
        sema["block_determinism_perf_baseline_lowering_capture_entries"],
        sema["block_determinism_perf_baseline_lowering_body_statement_entries"],
        sema["block_determinism_perf_baseline_lowering_deterministic_capture_sites"],
        sema["block_determinism_perf_baseline_lowering_heavy_tier_sites"],
        sema["block_determinism_perf_baseline_lowering_normalized_profile_sites"],
        sema["block_determinism_perf_baseline_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !23 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_block_determinism_perf_baseline_lowering_surface"
    ]
    lowering = manifest["lowering_block_determinism_perf_baseline"]

    block_literal_sites = sema["block_determinism_perf_baseline_lowering_sites"]
    baseline_weight_total = sema["block_determinism_perf_baseline_lowering_weight_total"]
    parameter_entries_total = sema["block_determinism_perf_baseline_lowering_parameter_entries"]
    capture_entries_total = sema["block_determinism_perf_baseline_lowering_capture_entries"]
    body_statement_entries_total = sema["block_determinism_perf_baseline_lowering_body_statement_entries"]
    deterministic_capture_sites = sema[
        "block_determinism_perf_baseline_lowering_deterministic_capture_sites"
    ]
    heavy_tier_sites = sema["block_determinism_perf_baseline_lowering_heavy_tier_sites"]
    normalized_profile_sites = sema[
    ...
    expected_replay_key = _expected_replay_key(
        ...
        baseline_weight_total=baseline_weight_total,
        ...
        deterministic_capture_sites=deterministic_capture_sites,
        heavy_tier_sites=heavy_tier_sites,
        normalized_profile_sites=normalized_profile_sites,
        contract_violation_sites=contract_violation_sites,
        deterministic=deterministic_handoff,
    )

    assert deterministic_capture_sites <= block_literal_sites
    assert heavy_tier_sites <= block_literal_sites
    assert normalized_profile_sites <= block_literal_sites
    assert contract_violation_sites <= block_literal_sites

    assert deterministic_handoff is True
    assert sema["lowering_block_determinism_perf_baseline_replay_key"] == expected_replay_key

    assert surface["block_literal_sites"] == block_literal_sites
    assert surface["baseline_weight_total"] == baseline_weight_total
    ...
    assert surface["deterministic_capture_sites"] == deterministic_capture_sites
    assert surface["heavy_tier_sites"] == heavy_tier_sites
    assert surface["normalized_profile_sites"] == normalized_profile_sites
...
def test_m170_validation_runbook_section_exists() -> None:
...
def test_m170_validation_manifest_packets_hold_deterministic_block_determinism_perf_baseline_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m170_validation_ir_markers_match_manifest_block_determinism_perf_baseline_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_block_determinism_perf_baseline_lowering = !{!23}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_block_determinism_perf_baseline"]["replay_key"]


def test_m170_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_block_determinism_perf_baseline"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_block_determinism_perf_baseline"]["replay_key"]
