...
SCRIPT_PATH = ROOT / "scripts" / "check_m226_e011_lane_e_performance_quality_guardrails_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m226_e011_lane_e_performance_quality_guardrails_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_m226_e011_lane_e_performance_quality_guardrails_contract.py")
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m226-lane-e-performance-quality-guardrails-contract-e011-v1"
    assert payload["ok"] is True


def test_contract_fails_closed_when_doc_contract_id_drifts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    drift_doc = tmp_path / "m226_lane_e_performance_quality_guardrails_e011_expectations.md"
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-lane-e-performance-quality-guardrails/m226-e011-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert exit_code == 1
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert any(failure["check_id"] == "M226-E011-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forced_ready_literal_present(
    ...
    assert exit_code == 1
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert any(failure["check_id"] == "M226-E011-FORB-01" for failure in payload["failures"])
