...
SCRIPT_PATH = ROOT / "scripts" / "check_m240_d023_runtime_registration_and_startup_integration_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m240_d023_runtime_registration_and_startup_integration_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m240_d023_runtime_registration_and_startup_integration_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m240-d023-runtime-registration-and-startup-integration-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m240_d023() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m240/M240-D023/")
...
def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            ...
            "Issue `#9999` defines canonical lane-D advanced diagnostics workpack (shard 2) scope.",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M240-D023-DOC-EXP-03" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_token_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m240_d023_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M240-D023-DOC-PKT-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M240-D023-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_metadata_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M240-D023-META-01" for failure in payload["failures"])
