...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_d012_toolchain_integration_and_optimization_controls_integration_closeout_and_gate_sign_off_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_d012_toolchain_integration_and_optimization_controls_integration_closeout_and_gate_sign_off_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == (
        "m246-d012-toolchain-integration-optimization-controls-integration-closeout-and-gate-sign-off-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_d012() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-D012/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_d012_expectations.md"
    ...
    assert any(failure["check_id"] == "M246-D012-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_metadata_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_d012_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-D012-DOC-PKT-03" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_runner_drops_d011_chain(tmp_path: Path) -> None:
    drift_readiness = tmp_path / "run_m246_d012_lane_d_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-D012-RDY-02" for failure in payload["failures"])


def test_contract_fails_closed_when_d011_dependency_token_drifts(tmp_path: Path) -> None:
    drift_d011_expectations = tmp_path / "m246_d011_expectations.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-D012-D011-DOC-02" for failure in payload["failures"])


def test_contract_fails_closed_when_d011_readiness_dependency_token_drifts(tmp_path: Path) -> None:
    drift_d011_readiness = tmp_path / "run_m246_d011_lane_d_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-D012-D011-RDY-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_d011_checker_dependency_path_missing(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-D012-DEP-D011-ARG-01" for failure in payload["failures"])
