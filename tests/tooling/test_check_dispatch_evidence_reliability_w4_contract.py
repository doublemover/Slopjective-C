...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "check_dispatch_evidence_reliability_w4_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_dispatch_evidence_reliability_w4_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_dispatch_evidence_reliability_w4_contract.py"
    )
...
def test_happy_path_is_deterministic() -> None:
    ...
    assert first_stdout == (
        "dispatch-evidence-reliability-w4-contract: OK\n"
...
def test_drift_path_returns_exit_1_with_stable_report(tmp_path: Path) -> None:
    ...
    assert "dispatch-evidence-reliability-w4-contract: contract drift detected" in first_stderr
    assert "- M35C-01\n" in first_stderr
    assert "zero_open/json output drift against" in first_stderr
    assert "remediation:\n" in first_stderr
    assert "python scripts/check_dispatch_evidence_reliability_w4_contract.py" in first_stderr
...
def test_hard_fail_returns_exit_2_for_missing_checker_script() -> None:
    ...
    assert stdout == ""
    assert (
        "dispatch-evidence-reliability-w4-contract: error: checker script does not exist:"
        in stderr
    )
