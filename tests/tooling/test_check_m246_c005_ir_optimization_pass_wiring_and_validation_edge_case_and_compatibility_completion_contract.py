...
def test_check_text_artifact_fails_closed_when_read_raises_os_error(
    ...
    checks_total, failures = contract.check_text_artifact(
        path=drift_doc,
        exists_check_id="M246-C005-TEST-EXISTS",
        snippets=(),
    )

    assert checks_total == 1
    assert len(failures) == 1
    assert failures[0].check_id == "M246-C005-TEST-EXISTS"
...
def test_contract_sorts_failures_by_check_id_artifact_and_detail(
    ...
    def fake_check_text_artifact(*, path: Path, exists_check_id: str, snippets: tuple[contract.SnippetCheck, ...]):
        if exists_check_id == "M246-C005-DOC-EXP-EXISTS":
            return (
                1,
                [
                    contract.Finding(artifact="zeta.md", check_id="M246-C005-Z", detail="z detail"),
                    contract.Finding(artifact="alpha.md", check_id="M246-C005-A", detail="z detail"),
                    contract.Finding(artifact="alpha.md", check_id="M246-C005-A", detail="a detail"),
                ],
            )
        return 1, []
    ...
    assert [failure["check_id"] for failure in failures] == ["M246-C005-A", "M246-C005-A", "M246-C005-Z"]
    ...
    assert stderr_lines[0].startswith("[M246-C005-A] alpha.md: a detail")
    assert stderr_lines[1].startswith("[M246-C005-A] alpha.md: z detail")
    assert stderr_lines[2].startswith("[M246-C005-Z] zeta.md: z detail")
