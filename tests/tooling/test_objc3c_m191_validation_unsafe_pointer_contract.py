...
LANE_CONTRACT = "m191-unsafe-pointer-extension-gating-lowering-v1"
LOWERING_PREFIX = "; unsafe_pointer_extension_lowering = "
PROFILE_PREFIX = "; frontend_objc_unsafe_pointer_extension_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_unsafe_pointer_extension_lowering_profile = "
    r"unsafe_pointer_extension_sites=(\d+), unsafe_keyword_sites=(\d+), "
    r"pointer_arithmetic_sites=(\d+), raw_pointer_type_sites=(\d+), "
    r"unsafe_operation_sites=(\d+), normalized_sites=(\d+), "
    r"gate_blocked_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_unsafe_pointer_extension_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!37 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    unsafe_pointer_extension_sites: int,
    unsafe_keyword_sites: int,
    pointer_arithmetic_sites: int,
    raw_pointer_type_sites: int,
    unsafe_operation_sites: int,
    ...
    return (
        f"unsafe_pointer_extension_sites={unsafe_pointer_extension_sites};"
        f"unsafe_keyword_sites={unsafe_keyword_sites};"
        f"pointer_arithmetic_sites={pointer_arithmetic_sites};"
        f"raw_pointer_type_sites={raw_pointer_type_sites};"
        f"unsafe_operation_sites={unsafe_operation_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["unsafe_pointer_extension_lowering_sites"],
        sema["unsafe_pointer_extension_lowering_unsafe_keyword_sites"],
        sema["unsafe_pointer_extension_lowering_pointer_arithmetic_sites"],
        sema["unsafe_pointer_extension_lowering_raw_pointer_type_sites"],
        sema["unsafe_pointer_extension_lowering_unsafe_operation_sites"],
        sema["unsafe_pointer_extension_lowering_normalized_sites"],
        sema["unsafe_pointer_extension_lowering_gate_blocked_sites"],
        sema["unsafe_pointer_extension_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !37 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_unsafe_pointer_extension_lowering_surface"
    ]
    lowering = manifest["lowering_unsafe_pointer_extension"]

    unsafe_pointer_extension_sites = sema["unsafe_pointer_extension_lowering_sites"]
    unsafe_keyword_sites = sema["unsafe_pointer_extension_lowering_unsafe_keyword_sites"]
    pointer_arithmetic_sites = sema["unsafe_pointer_extension_lowering_pointer_arithmetic_sites"]
    raw_pointer_type_sites = sema["unsafe_pointer_extension_lowering_raw_pointer_type_sites"]
    unsafe_operation_sites = sema["unsafe_pointer_extension_lowering_unsafe_operation_sites"]
    normalized_sites = sema["unsafe_pointer_extension_lowering_normalized_sites"]
    gate_blocked_sites = sema["unsafe_pointer_extension_lowering_gate_blocked_sites"]
    contract_violation_sites = sema["unsafe_pointer_extension_lowering_contract_violation_sites"]
    deterministic_handoff = sema["deterministic_unsafe_pointer_extension_lowering_handoff"]

    expected_replay_key = _expected_replay_key(
        unsafe_pointer_extension_sites=unsafe_pointer_extension_sites,
    ...
    assert unsafe_keyword_sites <= unsafe_pointer_extension_sites
    assert pointer_arithmetic_sites <= unsafe_pointer_extension_sites
    assert raw_pointer_type_sites <= unsafe_pointer_extension_sites
    assert unsafe_operation_sites <= unsafe_pointer_extension_sites
    assert normalized_sites <= unsafe_pointer_extension_sites
    assert gate_blocked_sites <= unsafe_pointer_extension_sites
    assert contract_violation_sites <= unsafe_pointer_extension_sites
    assert normalized_sites + gate_blocked_sites == unsafe_pointer_extension_sites

    assert deterministic_handoff is True
    assert sema["lowering_unsafe_pointer_extension_replay_key"] == expected_replay_key

    assert surface["unsafe_pointer_extension_sites"] == unsafe_pointer_extension_sites
    assert surface["unsafe_keyword_sites"] == unsafe_keyword_sites
    assert surface["pointer_arithmetic_sites"] == pointer_arithmetic_sites
    assert surface["raw_pointer_type_sites"] == raw_pointer_type_sites
    assert surface["unsafe_operation_sites"] == unsafe_operation_sites
...
def test_m191_validation_runbook_section_exists() -> None:
...
def test_m191_validation_manifest_packets_hold_deterministic_unsafe_pointer_extension_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m191_validation_ir_markers_match_manifest_unsafe_pointer_extension_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_unsafe_pointer_extension_lowering = !{!37}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_unsafe_pointer_extension"]["replay_key"]


def test_m191_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
