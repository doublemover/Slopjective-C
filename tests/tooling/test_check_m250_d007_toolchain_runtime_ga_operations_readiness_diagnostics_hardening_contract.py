...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m250_d007_toolchain_runtime_ga_operations_readiness_diagnostics_hardening_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_d007_toolchain_runtime_ga_operations_readiness_diagnostics_hardening_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m250-toolchain-runtime-ga-operations-readiness-diagnostics-hardening-contract-d007-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-toolchain-runtime-ga-operations-readiness-diagnostics-hardening/m250-d007-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-D007-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_diagnostics_hardening_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-D007-SUR-06", "M250-D007-FORB-01"}
