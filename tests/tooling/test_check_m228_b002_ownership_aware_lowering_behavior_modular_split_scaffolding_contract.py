...
SPEC = importlib.util.spec_from_file_location(
    "check_m228_b002_ownership_aware_lowering_behavior_modular_split_scaffolding_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_b002_ownership_aware_lowering_behavior_modular_split_scaffolding_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-b002-ownership-aware-lowering-behavior-modular-split-scaffolding-contract-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc = tmp_path / "m228_ownership_aware_lowering_behavior_modular_split_scaffolding_b002_expectations.md"
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-ownership-aware-lowering-behavior-modular-split-scaffolding/m228-b002-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M228-B002-DOC-01" for failure in payload["failures"])


def test_contract_fails_closed_when_scaffold_is_forced_true(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M228-B002-SCA-06", "M228-B002-FORB-01"}
