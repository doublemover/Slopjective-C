...
LANE_CONTRACT = "m188-actor-isolation-sendability-lowering-v1"
LOWERING_PREFIX = "; actor_isolation_sendability_lowering = "
PROFILE_PREFIX = "; frontend_objc_actor_isolation_sendability_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_actor_isolation_sendability_lowering_profile = "
    r"actor_isolation_sites=(\d+), sendability_check_sites=(\d+), "
    r"cross_actor_hop_sites=(\d+), non_sendable_capture_sites=(\d+), "
    r"sendable_transfer_sites=(\d+), isolation_boundary_sites=(\d+), "
    r"guard_blocked_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_actor_isolation_sendability_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!41 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    ...
    sendability_check_sites: int,
    cross_actor_hop_sites: int,
    non_sendable_capture_sites: int,
    sendable_transfer_sites: int,
    isolation_boundary_sites: int,
    ...
    return (
        ...
        f"sendability_check_sites={sendability_check_sites};"
        f"cross_actor_hop_sites={cross_actor_hop_sites};"
        f"non_sendable_capture_sites={non_sendable_capture_sites};"
        f"sendable_transfer_sites={sendable_transfer_sites};"
        f"isolation_boundary_sites={isolation_boundary_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["actor_isolation_sendability_lowering_sites"],
        sema["actor_isolation_sendability_lowering_sendability_check_sites"],
        sema["actor_isolation_sendability_lowering_cross_actor_hop_sites"],
        sema["actor_isolation_sendability_lowering_non_sendable_capture_sites"],
        sema["actor_isolation_sendability_lowering_sendable_transfer_sites"],
        sema["actor_isolation_sendability_lowering_isolation_boundary_sites"],
        sema["actor_isolation_sendability_lowering_guard_blocked_sites"],
        sema["actor_isolation_sendability_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !41 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_actor_isolation_sendability_lowering_surface"
    ]
    lowering = manifest["lowering_actor_isolation_sendability"]

    actor_isolation_sites = sema["actor_isolation_sendability_lowering_sites"]
    sendability_check_sites = sema["actor_isolation_sendability_lowering_sendability_check_sites"]
    cross_actor_hop_sites = sema["actor_isolation_sendability_lowering_cross_actor_hop_sites"]
    non_sendable_capture_sites = sema[
        "actor_isolation_sendability_lowering_non_sendable_capture_sites"
    ]
    sendable_transfer_sites = sema["actor_isolation_sendability_lowering_sendable_transfer_sites"]
    isolation_boundary_sites = sema["actor_isolation_sendability_lowering_isolation_boundary_sites"]
    guard_blocked_sites = sema["actor_isolation_sendability_lowering_guard_blocked_sites"]
    contract_violation_sites = sema["actor_isolation_sendability_lowering_contract_violation_sites"]
    deterministic_handoff = sema["deterministic_actor_isolation_sendability_lowering_handoff"]

    expected_replay_key = _expected_replay_key(
        actor_isolation_sites=actor_isolation_sites,
        sendability_check_sites=sendability_check_sites,
        cross_actor_hop_sites=cross_actor_hop_sites,
        non_sendable_capture_sites=non_sendable_capture_sites,
        sendable_transfer_sites=sendable_transfer_sites,
    ...
    assert sendability_check_sites <= actor_isolation_sites
    assert cross_actor_hop_sites <= actor_isolation_sites
    assert non_sendable_capture_sites <= sendability_check_sites
    assert sendable_transfer_sites <= sendability_check_sites
    assert isolation_boundary_sites <= actor_isolation_sites
    assert guard_blocked_sites <= actor_isolation_sites
    assert contract_violation_sites <= actor_isolation_sites
    assert isolation_boundary_sites + guard_blocked_sites == actor_isolation_sites

    assert deterministic_handoff is True
    assert sema["lowering_actor_isolation_sendability_replay_key"] == expected_replay_key

    assert surface["actor_isolation_sites"] == actor_isolation_sites
    assert surface["sendability_check_sites"] == sendability_check_sites
    assert surface["cross_actor_hop_sites"] == cross_actor_hop_sites
    assert surface["non_sendable_capture_sites"] == non_sendable_capture_sites
    assert surface["sendable_transfer_sites"] == sendable_transfer_sites
    assert surface["isolation_boundary_sites"] == isolation_boundary_sites
...
def test_m188_validation_runbook_section_exists() -> None:
...
def test_m188_validation_manifest_packets_hold_deterministic_actor_isolation_sendability_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m188_validation_ir_markers_match_manifest_actor_isolation_sendability_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_actor_isolation_sendability_lowering = !{!41}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_actor_isolation_sendability"][
            "replay_key"
        ]


def test_m188_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
