...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m227_a021_semantic_pass_integration_closeout_and_gate_signoff_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m227_a021_semantic_pass_integration_closeout_and_gate_signoff_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m227_a021_semantic_pass_integration_closeout_and_gate_signoff_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m227-semantic-pass-integration-closeout-and-gate-signoff-contract-a021-v1"
    )
...
def test_contract_fails_closed_when_surface_anchor_drifts(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["parse_surface"].read_text(encoding="utf-8").replace(
            ...
            "IsObjc3ToolchainRuntimeGaOperationsIntegrationCloseoutSignoffReadyDrift(",
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M227-A021-SUR-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_manifest_anchor_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-A021-ART-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_wiring_drifts(
    ...
    drift_package.write_text(
        contract.ARTIFACTS["package_json"].read_text(encoding="utf-8").replace(
            '"check:objc3c:m227-a021-lane-a-readiness"',
            '"check:objc3c:m227-a021-lane-a-readiness-drift"',
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M227-A021-CFG-03" for failure in payload["failures"])
