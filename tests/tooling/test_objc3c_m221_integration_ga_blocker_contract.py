...
def test_m221_integration_ga_blocker_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m221-ga-blocker-burndown"' in package_json
    assert (
        "npm run check:objc3c:m225-roadmap-seeding && python -m pytest "
        "tests/tooling/test_objc3c_m221_frontend_ga_blocker_contract.py "
        "tests/tooling/test_objc3c_m221_sema_ga_blocker_contract.py "
        "tests/tooling/test_objc3c_m221_lowering_ga_blocker_contract.py "
        "tests/tooling/test_objc3c_m221_validation_ga_blocker_contract.py "
        "tests/tooling/test_objc3c_m221_integration_ga_blocker_contract.py -q"
    ) in package_json

    assert "Run M221 GA blocker burn-down integration gate" in workflow
    assert "npm run check:objc3c:m221-ga-blocker-burndown" in workflow
