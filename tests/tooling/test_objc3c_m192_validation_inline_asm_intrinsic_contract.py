...
FIXTURE_ROOT = (
    ...
    / "fixtures"
    / "objc3c"
    / "m192_validation_inline_asm_intrinsic_contract"
)
...
LANE_CONTRACT = "m192-inline-asm-intrinsic-governance-lowering-v1"
LOWERING_PREFIX = "; inline_asm_intrinsic_governance_lowering = "
PROFILE_PREFIX = "; frontend_objc_inline_asm_intrinsic_governance_lowering_profile = "
PROFILE_RE = re.compile(
    r"^; frontend_objc_inline_asm_intrinsic_governance_lowering_profile = "
    r"inline_asm_intrinsic_sites=(\d+), inline_asm_sites=(\d+), "
    r"intrinsic_sites=(\d+), governed_intrinsic_sites=(\d+), "
    r"privileged_intrinsic_sites=(\d+), normalized_sites=(\d+), "
    r"gate_blocked_sites=(\d+), contract_violation_sites=(\d+), "
    r"deterministic_inline_asm_intrinsic_governance_lowering_handoff=(true|false)$"
)
METADATA_RE = re.compile(
    r"^!38 = !{i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i64 (\d+), i1 ([01])}$"
)
...
def _expected_replay_key(
    inline_asm_intrinsic_sites: int,
    inline_asm_sites: int,
    intrinsic_sites: int,
    governed_intrinsic_sites: int,
    privileged_intrinsic_sites: int,
    ...
    return (
        f"inline_asm_intrinsic_sites={inline_asm_intrinsic_sites};"
        f"inline_asm_sites={inline_asm_sites};"
        f"intrinsic_sites={intrinsic_sites};"
        f"governed_intrinsic_sites={governed_intrinsic_sites};"
        f"privileged_intrinsic_sites={privileged_intrinsic_sites};"
...
def _manifest_counts(manifest: dict) -> tuple[int, int, int, int, int, int, int, int]:
    ...
    return (
        sema["inline_asm_intrinsic_governance_lowering_sites"],
        sema["inline_asm_intrinsic_governance_lowering_inline_asm_sites"],
        sema["inline_asm_intrinsic_governance_lowering_intrinsic_sites"],
        sema["inline_asm_intrinsic_governance_lowering_governed_intrinsic_sites"],
        sema["inline_asm_intrinsic_governance_lowering_privileged_intrinsic_sites"],
        sema["inline_asm_intrinsic_governance_lowering_normalized_sites"],
        sema["inline_asm_intrinsic_governance_lowering_gate_blocked_sites"],
        sema["inline_asm_intrinsic_governance_lowering_contract_violation_sites"],
    )
...
def _extract_metadata_counts_and_deterministic(
    ...
    raise AssertionError("missing LLVM metadata tuple marker: !38 = !{i64 ..., i1 1}")


def _assert_manifest_packet_contract(manifest: dict) -> None:
    sema = manifest["frontend"]["pipeline"]["sema_pass_manager"]
    surface = manifest["frontend"]["pipeline"]["semantic_surface"][
        "objc_inline_asm_intrinsic_governance_lowering_surface"
    ]
    lowering = manifest["lowering_inline_asm_intrinsic_governance"]

    inline_asm_intrinsic_sites = sema["inline_asm_intrinsic_governance_lowering_sites"]
    inline_asm_sites = sema["inline_asm_intrinsic_governance_lowering_inline_asm_sites"]
    intrinsic_sites = sema["inline_asm_intrinsic_governance_lowering_intrinsic_sites"]
    governed_intrinsic_sites = sema[
        "inline_asm_intrinsic_governance_lowering_governed_intrinsic_sites"
    ]
    privileged_intrinsic_sites = sema[
        "inline_asm_intrinsic_governance_lowering_privileged_intrinsic_sites"
    ]
    normalized_sites = sema["inline_asm_intrinsic_governance_lowering_normalized_sites"]
    gate_blocked_sites = sema["inline_asm_intrinsic_governance_lowering_gate_blocked_sites"]
    contract_violation_sites = sema[
        "inline_asm_intrinsic_governance_lowering_contract_violation_sites"
    ]
    deterministic_handoff = sema["deterministic_inline_asm_intrinsic_governance_lowering_handoff"]

    expected_replay_key = _expected_replay_key(
        inline_asm_intrinsic_sites=inline_asm_intrinsic_sites,
    ...
    assert inline_asm_sites <= inline_asm_intrinsic_sites
    assert intrinsic_sites <= inline_asm_intrinsic_sites
    assert governed_intrinsic_sites <= intrinsic_sites
    assert privileged_intrinsic_sites <= governed_intrinsic_sites
    assert normalized_sites <= inline_asm_intrinsic_sites
    assert gate_blocked_sites <= inline_asm_intrinsic_sites
    assert contract_violation_sites <= inline_asm_intrinsic_sites
    assert normalized_sites + gate_blocked_sites == inline_asm_intrinsic_sites

    assert deterministic_handoff is True
    assert sema["lowering_inline_asm_intrinsic_governance_replay_key"] == expected_replay_key

    assert surface["inline_asm_intrinsic_sites"] == inline_asm_intrinsic_sites
    assert surface["inline_asm_sites"] == inline_asm_sites
    assert surface["intrinsic_sites"] == intrinsic_sites
    assert surface["governed_intrinsic_sites"] == governed_intrinsic_sites
    assert surface["privileged_intrinsic_sites"] == privileged_intrinsic_sites
...
def test_m192_validation_runbook_section_exists() -> None:
...
def test_m192_validation_manifest_packets_hold_deterministic_inline_asm_intrinsic_contract() -> None:
    for manifest_path in (RUN1_MANIFEST, RUN2_MANIFEST):
        _assert_manifest_packet_contract(_load_json(manifest_path))


def test_m192_validation_ir_markers_match_manifest_inline_asm_intrinsic_metadata() -> None:
    for manifest_path, ir_path in ((RUN1_MANIFEST, RUN1_IR), (RUN2_MANIFEST, RUN2_IR)):
        ...
        assert "!objc3.objc_inline_asm_intrinsic_governance_lowering = !{!38}" in ir_text
        ...
        assert _extract_lowering_replay_key(ir_text) == manifest["lowering_inline_asm_intrinsic_governance"][
            "replay_key"
        ]


def test_m192_validation_replay_key_and_packets_are_stable_across_replay_runs() -> None:
    ...
    run1_replay_key = run1_manifest["lowering_inline_asm_intrinsic_governance"]["replay_key"]
    run2_replay_key = run2_manifest["lowering_inline_asm_intrinsic_governance"]["replay_key"]
