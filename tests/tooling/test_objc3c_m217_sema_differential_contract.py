...
def test_m217_sema_differential_parity_profile_section_exists() -> None:
    fragment = _read(SEMA_DOC_FRAGMENT)
    for text in (
        "## M217 sema/type differential parity profile",
        "capture deterministic evidence packets in stable tuple order: `native`, `baseline-clang`, `baseline-llvm-direct`",
        ...
        "python -m pytest tests/tooling/test_objc3c_m217_sema_differential_contract.py -q",
    ):
        assert text in fragment


def test_m217_sema_differential_parity_profile_maps_to_source_anchors() -> None:
