...
def test_m224_validation_release_readiness_section_present() -> None:
    ...
    for text in (
        ...
        "any non-zero exit code in sequence is a hard stop; do not execute later commands.",
...
def test_m224_validation_release_readiness_run_order_commands_and_scripts_available() -> None:
    ...
    assert "scripts/check_objc3c_native_execution_smoke.ps1" in scripts[
        "test:objc3c:execution-smoke"
    ]
    assert "scripts/check_objc3c_execution_replay_proof.ps1" in scripts[
        "test:objc3c:execution-replay-proof"
    ]
