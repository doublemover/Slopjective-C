...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m236_c001_arc_style_lowering_insertion_and_cleanup_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m236_c001_arc_style_lowering_insertion_and_cleanup_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m236_c001_arc_style_lowering_insertion_and_cleanup_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m236-c001-arc-style-lowering-insertion-and-cleanup-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m236_c001() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m236/M236-C001/")


def test_contract_fails_closed_when_expectations_drop_issue_anchor(tmp_path: Path) -> None:
    drift_doc = (
        tmp_path
        / "m236_arc_style_lowering_insertion_and_cleanup_contract_and_architecture_freeze_c001_expectations.md"
    )
    drift_doc.write_text(
        contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            "Issue `#5892` defines canonical lane-C contract and architecture freeze scope.",
            "Issue `#5892` defines canonical lane-C scope.",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M236-C001-DOC-EXP-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_script_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M236-C001-PKG-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_lowering_anchor_drifts(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M236-C001-SPC-01" for failure in payload["failures"])
