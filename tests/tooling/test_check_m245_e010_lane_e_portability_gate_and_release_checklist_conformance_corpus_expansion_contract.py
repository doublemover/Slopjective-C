...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m245_e010_lane_e_portability_gate_and_release_checklist_conformance_corpus_expansion_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m245_e010_lane_e_portability_gate_and_release_checklist_conformance_corpus_expansion_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m245-e010-lane-e-portability-gate-release-checklist-conformance-corpus-expansion-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m245_e010() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m245/M245-E010/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m245_e010_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_b004_dependency_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m245_lane_e_portability_gate_and_release_checklist_conformance_corpus_expansion_e010_expectations.md"
    drift_doc.write_text(
        replace_once(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M245-E009`, `M245-A004`, `M245-B004`, `M245-C006`, `M245-D007`",
            "Dependencies: `M245-E009`, `M245-A004`, `M245-B099`, `M245-C006`, `M245-D007`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E010-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m245_e010_lane_e_portability_gate_and_release_checklist_conformance_corpus_expansion_packet.md"
    drift_doc.write_text(
        replace_once(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E010-DOC-PKT-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_missing_diagnostics_replay_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M245-E010-PKG-03" for failure in payload["failures"])


def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    drift_expectations = tmp_path / "m245_e010_expectations.md"
    drift_packet = tmp_path / "m245_e010_packet.md"
    ...
    drift_expectations.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-lane-e-portability-gate-release-checklist-conformance-corpus-expansion/m245-e010-drift`",
        ),
        encoding="utf-8",
    )
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M245-E009`, `M245-A004`, `M245-B004`, `M245-C006`, `M245-D007`",
            "Dependencies: `M245-E009`, `M245-A004`, `M245-B004`, `M245-C006`, `M245-D099`",
        ),
        encoding="utf-8",
    )
