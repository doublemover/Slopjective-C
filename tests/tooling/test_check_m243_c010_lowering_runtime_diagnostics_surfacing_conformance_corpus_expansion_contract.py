...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_c010_lowering_runtime_diagnostics_surfacing_conformance_corpus_expansion_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-c010-lowering-runtime-diagnostics-surfacing-conformance-corpus-expansion-contract-v1"
    )
...
def test_contract_default_summary_out_is_under_tmp_reports_m243_c010() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m243/M243-C010/")
...
def test_contract_fails_closed_when_expectations_dependency_token_drifts(
    ...
    drift_doc = tmp_path / "m243_c010_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-C010-DOC-02" for failure in payload["failures"])


def test_contract_fails_closed_when_package_readiness_chain_drops_c009(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C010-CFG-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_architecture_anchor_drifts(
    ...
    drift_arch = tmp_path / "ARCHITECTURE.md"
    drift_arch.write_text(
        contract.ARTIFACTS["architecture_doc"].read_text(encoding="utf-8").replace(
            ...
            "M243 lane-C C010 conformance corpus expansion anchors lowering/runtime diagnostics mismatch",
            1,
        ),
        encoding="utf-8",
    )
    ...
    artifacts = dict(contract.ARTIFACTS)
    artifacts["architecture_doc"] = drift_arch
    monkeypatch.setattr(contract, "ARTIFACTS", artifacts)
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M243-C010-ARC-01" for failure in payload["failures"])
