...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m250_e009_final_readiness_gate_documentation_signoff_conformance_matrix_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_e009_final_readiness_gate_documentation_signoff_conformance_matrix_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m250-final-readiness-gate-documentation-signoff-conformance-matrix-contract-e009-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-final-readiness-gate-documentation-signoff-conformance-matrix/m250-e009-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-E009-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_conformance_matrix_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-E009-SUR-06", "M250-E009-FORB-01"}
...
def test_contract_fails_closed_when_conformance_matrix_failure_reason_is_missing(
    ...
    drift_surface.write_text(
        contract.ARTIFACTS["core_surface_header"].read_text(encoding="utf-8").replace(
            ...
            "final readiness gate conformance drift marker",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M250-E009-SUR-15" for failure in payload["failures"])
