...
SCRIPT_PATH = ROOT / "scripts" / "check_m226_e003_lane_e_integration_gate_core_evidence_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m226_e003_lane_e_integration_gate_core_evidence_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Unable to load scripts/check_m226_e003_lane_e_integration_gate_core_evidence_contract.py")
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m226-e003-lane-e-integration-gate-core-evidence-contract-v1"
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m226_e003_contract.py"),
    )
...
def test_contract_fails_closed_when_freeze_doc_drops_e003_packet_row(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        contract.DEFAULT_FREEZE_DOC.read_text(encoding="utf-8").replace(
            ...
            "| `M226-E003` | `objc3c-lane-e-integration-gate-core-evidence-contract/m226-e003-drift` |",
            1,
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M226-E003-DOC-FRZ-03" for failure in payload["failures"])
