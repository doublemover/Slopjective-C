...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_e013_optimization_gate_and_perf_evidence_docs_and_operator_runbook_synchronization_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_e013_optimization_gate_and_perf_evidence_docs_and_operator_runbook_synchronization_contract.py"
    )
...
def test_parse_args_accepts_overrides(tmp_path: Path) -> None:
    ...
    custom_e012_packet = tmp_path / "e012_packet.md"
    custom_a010_expectations = tmp_path / "a010_expectations.md"
    custom_d010_readiness = tmp_path / "run_m246_d010_lane_d_readiness.py"
    ...
    args = contract.parse_args(
        [
            ...
            str(custom_packet),
            "--e012-packet-doc",
            str(custom_e012_packet),
            "--a010-expectations-doc",
            str(custom_a010_expectations),
            "--d010-readiness-script",
            str(custom_d010_readiness),
            "--summary-out",
            str(custom_summary),
        ]
    )
    ...
    assert args.e012_packet_doc == custom_e012_packet
    assert args.a010_expectations_doc == custom_a010_expectations
    assert args.d010_readiness_script == custom_d010_readiness
    assert args.summary_out == custom_summary


def test_contract_default_summary_out_is_under_tmp_reports_m246_e013() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-E013/")
...
def test_contract_passes_on_repository_sources_and_summary_contract(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-e013-optimization-gate-perf-evidence-docs-and-operator-runbook-synchronization-contract-v1"
...
def test_contract_fails_closed_when_prerequisite_dependency_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m246_e013_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_pending_b014_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_e013_expectations.md"
    drift_doc.write_text(
        replace_once(
            ...
            "| `M246-B099` | Dependency token `M246-B099` is mandatory as pending seeded lane-B docs and operator runbook synchronization assets. |",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E013-DOC-EXP-05" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_line_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_e013_packet.md"
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M246-E012`, `M246-A010`, `M246-B014`, `M246-C024`, `M246-D010`",
            "Dependencies: `M246-E012`, `M246-A010`, `M246-B014`, `M246-C024`, `M246-D099`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E013-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_chain_drops_real_d010_anchor(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m246_e013_lane_e_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E013-RUN-08" for failure in payload["failures"])
...
def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-optimization-gate-perf-evidence-docs-and-operator-runbook-synchronization/m246-e013-drift`",
        ),
        encoding="utf-8",
    )
    drift_runner.write_text(
