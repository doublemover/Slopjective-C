...
def test_m205_integration_macro_security_is_documented() -> None:
    library_api_doc = _read(LIBRARY_API_DOC)

    for text in (
        "## M205 integration macro security policy enforcement",
        "npm run check:objc3c:m205-macro-security",
        "check:objc3c:m206-canonical-optimization-stage1",
...
def test_m205_integration_macro_security_gate_is_wired() -> None:
    package_json = _read(PACKAGE_JSON)
    workflow = _read(TASK_HYGIENE_WORKFLOW)

    assert '"check:objc3c:m205-macro-security"' in package_json
    assert (
        "npm run check:objc3c:m206-canonical-optimization-stage1 && python -m pytest "
        "tests/tooling/test_objc3c_m205_frontend_macro_security_contract.py "
        "tests/tooling/test_objc3c_m205_sema_macro_security_contract.py "
        "tests/tooling/test_objc3c_m205_lowering_macro_security_contract.py "
        "tests/tooling/test_objc3c_m205_validation_macro_security_contract.py "
        "tests/tooling/test_objc3c_m205_integration_macro_security_contract.py -q"
    ) in package_json

    assert "Run M205 macro-security integration gate" in workflow
    assert "npm run check:objc3c:m205-macro-security" in workflow
