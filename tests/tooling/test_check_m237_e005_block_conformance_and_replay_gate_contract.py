...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m237_e005_block_conformance_and_replay_gate_contract.py"
    )
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = (
        tmp_path / "m237_block_conformance_and_replay_gate_edge_case_and_compatibility_completion_e005_expectations.md"
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M237-E005-DOC-EXP-03" for failure in payload["failures"])
