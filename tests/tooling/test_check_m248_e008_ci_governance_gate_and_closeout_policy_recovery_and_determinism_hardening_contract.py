...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m248_e008_ci_governance_gate_and_closeout_policy_recovery_and_determinism_hardening_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m248_e008_ci_governance_gate_and_closeout_policy_recovery_and_determinism_hardening_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m248_e008_ci_governance_gate_and_closeout_policy_recovery_and_determinism_hardening_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m248-e008-lane-e-ci-governance-gate-closeout-policy-recovery-and-determinism-hardening-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m248_e008() -> None:
    ...
    assert normalized.startswith("tmp/reports/m248/M248-E008/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m248_e008_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_dependency_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m248_lane_e_ci_governance_gate_and_closeout_policy_recovery_and_determinism_hardening_e008_expectations.md"
    original = contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8")
    anchor = "Dependencies: `M248-E007`, `M248-A003`, `M248-B004`, `M248-C004`, `M248-D006`"
    assert anchor in original
    drift_text = original.replace(anchor, "Dependencies: `M248-E007`, `M248-A003`, `M248-B004`, `M248-C099`, `M248-D006`")
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-E008-DOC-EXP-04" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_readiness_chain_drifts(tmp_path: Path) -> None:
    ...
    scripts["check:objc3c:m248-e008-lane-e-readiness"] = (
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M248-E008-PKG-03" for failure in payload["failures"])
...
def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-lane-e-ci-governance-gate-closeout-policy-recovery-and-determinism-hardening/m248-e008-drift`",
            1,
        ),
        encoding="utf-8",
    )
