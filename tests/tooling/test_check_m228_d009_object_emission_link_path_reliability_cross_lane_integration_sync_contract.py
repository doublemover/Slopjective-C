...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m228_d012_object_emission_link_path_reliability_cross_lane_integration_sync_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m228-d012-object-emission-link-path-reliability-cross-lane-integration-sync-contract-v1"
    )
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-object-emission-link-path-reliability-cross-lane-integration-sync/m228-d012-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M228-D012-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_forbidden_guardrail_ready_shortcut_appears(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D012-FORB-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_planning_packet_dependency_drifts(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M228-D012-PKT-04" for failure in payload["failures"])
