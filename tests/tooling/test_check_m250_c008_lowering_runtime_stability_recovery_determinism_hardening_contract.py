...
SCRIPT_PATH = (
    ROOT / "scripts" / "check_m250_c008_lowering_runtime_stability_recovery_determinism_hardening_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m250_c008_lowering_runtime_stability_recovery_determinism_hardening_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m250-lowering-runtime-stability-recovery-determinism-hardening-contract-c008-v1"
...
def test_contract_fails_closed_when_contract_id_drifts(
    ...
    drift_doc = tmp_path / "m250_lowering_runtime_stability_recovery_determinism_hardening_c008_expectations.md"
    drift_doc.write_text(
        contract.ARTIFACTS["contract_doc"].read_text(encoding="utf-8").replace(
            ...
            "Contract ID: `objc3c-lowering-runtime-stability-recovery-determinism-hardening/m250-c008-drift`",
            1,
        ),
        encoding="utf-8",
    )
    ...
    assert any(failure["check_id"] == "M250-C008-DOC-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_recovery_determinism_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M250-C008-SUR-02", "M250-C008-FORB-01"}
