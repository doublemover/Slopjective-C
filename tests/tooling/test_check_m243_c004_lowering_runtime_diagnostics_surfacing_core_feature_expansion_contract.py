...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m243_c004_lowering_runtime_diagnostics_surfacing_core_feature_expansion_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m243_c004_lowering_runtime_diagnostics_surfacing_core_feature_expansion_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m243_c004_lowering_runtime_diagnostics_surfacing_core_feature_expansion_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert (
        payload["mode"]
        == "m243-c004-lowering-runtime-diagnostics-surfacing-core-feature-expansion-contract-v1"
    )
...
def test_contract_fails_closed_when_dependency_drifts(
    ...
    drift_doc = tmp_path / "m243_c004_expectations.md"
    ...
    assert any(failure["check_id"] == "M243-C004-DOC-02" for failure in payload["failures"])
...
def test_contract_fails_closed_when_surface_forces_expansion_ready(
    ...
    assert payload["ok"] is False
    assert any(
        failure["check_id"] in {"M243-C004-SUR-06", "M243-C004-FORB-01"}
