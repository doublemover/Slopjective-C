...
SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "check_m234_e014_property_conformance_gate_and_docs_release_candidate_and_replay_dry_run_contract.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_m234_e014_property_conformance_gate_and_docs_release_candidate_and_replay_dry_run_contract",
    SCRIPT_PATH,
)
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m234-e014-property-conformance-gate-docs-release-candidate-and-replay-dry-run-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m234_e014() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m234/M234-E014/")
...
def test_contract_fails_closed_when_prerequisite_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m234_e014_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_b015_dependency_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m234_property_conformance_gate_and_docs_release_candidate_and_replay_dry_run_e014_expectations.md"
    drift_doc.write_text(
        replace_once(
            contract.DEFAULT_EXPECTATIONS_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M234-E013`, `M234-A014`, `M234-B015`, `M234-C015`, `M234-D011`",
            "Dependencies: `M234-E013`, `M234-A014`, `M234-B099`, `M234-C015`, `M234-D011`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-E014-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m234_e014_property_conformance_gate_and_docs_release_candidate_and_replay_dry_run_packet.md"
    drift_doc.write_text(
        replace_once(
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-E014-DOC-PKT-03" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_missing_perf_budget_token(tmp_path: Path) -> None:
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-E014-PKG-04" for failure in payload["failures"])


def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    drift_expectations = tmp_path / "m234_e014_expectations.md"
    drift_packet = tmp_path / "m234_e014_packet.md"
    ...
    drift_expectations.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-property-conformance-gate-docs-release-candidate-and-replay-dry-run/m234-e014-drift`",
        ),
        encoding="utf-8",
    )
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M234-E013`, `M234-A014`, `M234-B015`, `M234-C015`, `M234-D011`",
            "Dependencies: `M234-E013`, `M234-A014`, `M234-B015`, `M234-C015`, `M234-D099`",
        ),
        encoding="utf-8",
    )
