...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_e012_optimization_gate_and_perf_evidence_cross_lane_integration_sync_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_e012_optimization_gate_and_perf_evidence_cross_lane_integration_sync_contract.py"
    )
...
def test_parse_args_accepts_overrides(tmp_path: Path) -> None:
    ...
    custom_e011_packet = tmp_path / "e011_packet.md"
    ...
    assert args.e011_packet_doc == custom_e011_packet
...
def test_contract_default_summary_out_is_under_tmp_reports_m246_e012() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-E012/")
...
def test_contract_passes_on_repository_sources_and_summary_contract(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-e012-optimization-gate-perf-evidence-cross-lane-integration-sync-contract-v1"
...
def test_contract_fails_closed_when_prerequisite_dependency_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m246_e012_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_pending_b012_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_e012_expectations.md"
    drift_doc.write_text(
        replace_once(
            ...
            "| `M246-B099` | Dependency token `M246-B099` is mandatory as pending seeded lane-B cross-lane integration sync assets. |",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E012-DOC-EXP-05" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_line_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_e012_packet.md"
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M246-E011`, `M246-A008`, `M246-B012`, `M246-C020`, `M246-D008`",
            "Dependencies: `M246-E011`, `M246-A008`, `M246-B012`, `M246-C020`, `M246-D099`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E012-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_chain_drops_real_d008_anchor(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m246_e012_lane_e_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E012-RUN-08" for failure in payload["failures"])
...
def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-optimization-gate-perf-evidence-cross-lane-integration-sync/m246-e012-drift`",
        ),
        encoding="utf-8",
    )
    drift_runner.write_text(
