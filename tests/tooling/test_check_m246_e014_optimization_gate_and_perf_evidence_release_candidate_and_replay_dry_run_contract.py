...
SPEC = importlib.util.spec_from_file_location(
    "check_m246_e014_optimization_gate_and_perf_evidence_release_candidate_and_replay_dry_run_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m246_e014_optimization_gate_and_perf_evidence_release_candidate_and_replay_dry_run_contract.py"
    )
...
def test_parse_args_accepts_overrides(tmp_path: Path) -> None:
    ...
    custom_e013_packet = tmp_path / "e013_packet.md"
    custom_a011_expectations = tmp_path / "a011_expectations.md"
    ...
    args = contract.parse_args(
        [
            ...
            str(custom_packet),
            "--e013-packet-doc",
            str(custom_e013_packet),
            "--a011-expectations-doc",
            str(custom_a011_expectations),
    ...
    assert args.e013_packet_doc == custom_e013_packet
    assert args.a011_expectations_doc == custom_a011_expectations
    assert args.d011_readiness_script == custom_d011_readiness
    assert args.summary_out == custom_summary


def test_contract_default_summary_out_is_under_tmp_reports_m246_e014() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m246/M246-E014/")
...
def test_contract_passes_on_repository_sources_and_summary_contract(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m246-e014-optimization-gate-perf-evidence-release-candidate-and-replay-dry-run-contract-v1"
...
def test_contract_fails_closed_when_prerequisite_dependency_asset_is_missing(
    ...
    drift_assets[0] = contract.AssetCheck(
        ...
        relative_path=Path("scripts/does_not_exist_m246_e014_contract.py"),
    )
...
def test_contract_fails_closed_when_expectations_drop_pending_b015_token(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m246_e014_expectations.md"
    drift_doc.write_text(
        replace_once(
            ...
            "| `M246-B099` | Dependency token `M246-B099` is mandatory as pending seeded lane-B release-candidate and replay dry-run assets. |",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E014-DOC-EXP-05" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_line_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m246_e014_packet.md"
    drift_packet.write_text(
        replace_once(
            contract.DEFAULT_PACKET_DOC.read_text(encoding="utf-8"),
            "Dependencies: `M246-E013`, `M246-A011`, `M246-B015`, `M246-C025`, `M246-D011`",
            "Dependencies: `M246-E013`, `M246-A011`, `M246-B015`, `M246-C025`, `M246-D099`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E014-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_readiness_chain_drops_real_d011_anchor(tmp_path: Path) -> None:
    drift_runner = tmp_path / "run_m246_e014_lane_e_readiness.py"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M246-E014-RUN-08" for failure in payload["failures"])
...
def test_contract_drift_findings_are_deterministic_across_runs(tmp_path: Path) -> None:
    ...
    drift_doc.write_text(
        replace_once(
            ...
            "Contract ID: `objc3c-optimization-gate-perf-evidence-release-candidate-and-replay-dry-run/m246-e014-drift`",
        ),
        encoding="utf-8",
    )
    drift_runner.write_text(
        replace_once(
            contract.DEFAULT_READINESS_SCRIPT.read_text(encoding="utf-8"),
            "check:objc3c:m246-b015-lane-b-readiness",
