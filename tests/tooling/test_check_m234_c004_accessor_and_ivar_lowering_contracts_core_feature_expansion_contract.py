...
SCRIPT_PATH = ROOT / "scripts" / "check_m234_c004_accessor_and_ivar_lowering_contracts_core_feature_expansion_contract.py"
SPEC = importlib.util.spec_from_file_location(
    "check_m234_c004_accessor_and_ivar_lowering_contracts_core_feature_expansion_contract",
    SCRIPT_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(
        "Unable to load scripts/check_m234_c004_accessor_and_ivar_lowering_contracts_core_feature_expansion_contract.py"
    )
...
def test_contract_passes_on_repository_sources(tmp_path: Path) -> None:
    ...
    assert payload["mode"] == "m234-c004-accessor-and-ivar-lowering-contracts-core-feature-expansion-contract-v1"
...
def test_contract_default_summary_out_is_under_tmp_reports_m234_c004() -> None:
    args = contract.parse_args([])
    normalized = str(args.summary_out).replace("\\", "/")
    assert normalized.startswith("tmp/reports/m234/M234-C004/")
...
def test_contract_fails_closed_when_expectations_issue_anchor_drifts(tmp_path: Path) -> None:
    drift_doc = tmp_path / "m234_accessor_and_ivar_lowering_contracts_core_feature_expansion_c004_expectations.md"
    ...
    assert any(failure["check_id"] == "M234-C004-DOC-EXP-04" for failure in payload["failures"])


def test_contract_fails_closed_when_packet_dependency_token_drifts(tmp_path: Path) -> None:
    drift_packet = tmp_path / "m234_c004_accessor_and_ivar_lowering_contracts_core_feature_expansion_packet.md"
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-C004-DOC-PKT-04" for failure in payload["failures"])


def test_contract_fails_closed_when_c003_contract_id_drifts(tmp_path: Path) -> None:
    drift_c003_doc = tmp_path / "m234_accessor_and_ivar_lowering_contracts_core_feature_implementation_c003_expectations.md"
    drift_c003_doc.write_text(
        replace_all_occurrences(
            ...
            "Contract ID: `objc3c-accessor-and-ivar-lowering-contracts-core-feature-implementation/m234-c003-drift`",
        ),
        encoding="utf-8",
    )
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-C004-C003-01" for failure in payload["failures"])
...
def test_contract_fails_closed_when_package_chain_drops_c003_readiness(tmp_path: Path) -> None:
    ...
    scripts["check:objc3c:m234-c004-lane-c-readiness"] = (
    ...
    payload = json.loads(summary_out.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert any(failure["check_id"] == "M234-C004-PKG-03" for failure in payload["failures"])
