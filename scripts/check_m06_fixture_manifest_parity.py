#!/usr/bin/env python3
"""Fail-closed M06 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M06-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M06-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(
        ...
        first_issue=1414,
        last_issue=1422,
        required_group_name="m06_lane_a_issue_1414_1422_ast_source_mapping_scope_freeze",
    ),
    LaneSpec(lane="B", bucket="semantic", first_issue=1423, last_issue=1431),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1432, last_issue=1440),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1441, last_issue=1448),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M06 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
