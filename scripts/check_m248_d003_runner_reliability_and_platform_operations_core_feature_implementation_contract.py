#!/usr/bin/env python3
"""Fail-closed checker for M248-D003 runner core feature implementation."""
...
MODE = "m248-d003-runner-reliability-platform-operations-core-feature-implementation-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m248_runner_reliability_and_platform_operations_core_feature_implementation_d003_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    ...
    / "compiler"
    / "m248"
    / "m248_d003_runner_reliability_and_platform_operations_core_feature_implementation_packet.md"
)
...
DEFAULT_D002_CHECKER = (
    ROOT / "scripts" / "check_m248_d002_runner_reliability_and_platform_operations_modular_split_scaffolding_contract.py"
)
DEFAULT_D002_TEST = (
    ROOT / "tests" / "tooling" / "test_check_m248_d002_runner_reliability_and_platform_operations_modular_split_scaffolding_contract.py"
)
...
D002_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M248-D003-D002-DOC-01",
        "Contract ID: `objc3c-runner-reliability-platform-operations-modular-split-scaffolding/m248-d002-v1`",
    ),
    SnippetCheck("M248-D003-D002-DOC-02", "Dependencies: `M248-D001`"),
)

D002_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M248-D003-D002-PKT-01", "Packet: `M248-D002`"),
    SnippetCheck("M248-D003-D002-PKT-02", "Dependencies: `M248-D001`"),
)
...
LOWERING_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck("M248-D003-SPC-03", "nullable-tool-path safety drift"),
)
...
METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M248-D003-META-02",
        "explicit `M248-D002` dependency continuity so nullable-tool-path core feature drift fails closed",
    ),
)
...
PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        ...
        '&& npm run check:objc3c:m248-d003-runner-reliability-platform-operations-core-feature-implementation-contract '
        '&& npm run test:tooling:m248-d003-runner-reliability-platform-operations-core-feature-implementation-contract"',
    ),
    SnippetCheck("M248-D003-PKG-04", '"compile:objc3c": '),
    SnippetCheck("M248-D003-PKG-05", '"proof:objc3c": '),
    SnippetCheck("M248-D003-PKG-06", '"test:objc3c:execution-replay-proof": '),
    SnippetCheck("M248-D003-PKG-07", '"test:objc3c:perf-budget": '),
)

FRONTEND_ANCHOR_REQUIRED_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M248-D003-CORE-01",
...
FRONTEND_ANCHOR_FORBIDDEN_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M248-D003-CORE-06", "std::filesystem::path(options->clang_path)"),
    SnippetCheck("M248-D003-CORE-07", "std::filesystem::path(options->llc_path)"),
)
...
def check_source_contract(
    *,
    ...
    findings: list[Finding] = []
    if not path.exists():
        findings.append(Finding(display_path(path), exists_check_id, f"required source is missing: {display_path(path)}"))
        return checks_total, findings
    if not path.is_file():
        findings.append(Finding(display_path(path), exists_check_id, f"required source is not a file: {display_path(path)}"))
        return checks_total, findings
    ...
    for snippet in required_snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(Finding(display_path(path), snippet.check_id, f"missing required source snippet: {snippet.snippet}"))
    for snippet in forbidden_snippets:
        checks_total += 1
        if snippet.snippet in text:
            findings.append(Finding(display_path(path), snippet.check_id, f"forbidden source snippet present: {snippet.snippet}"))
    return checks_total, findings
...
def run(argv: Sequence[str]) -> int:
    ...
    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M248-D003-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M248-D003-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.d002_expectations_doc, "M248-D003-D002-DOC-EXISTS", D002_EXPECTATIONS_SNIPPETS),
        (args.d002_packet_doc, "M248-D003-D002-PKT-EXISTS", D002_PACKET_SNIPPETS),
        (args.d002_checker, "M248-D003-D002-CHK-EXISTS", (SnippetCheck("M248-D003-D002-CHK-01", "MODE = \"m248-d002-runner-reliability-platform-operations-modular-split-scaffolding-contract-v1\""),)),
        (
            args.d002_test,
            "M248-D003-D002-TST-EXISTS",
            (
                SnippetCheck(
                    "M248-D003-D002-TST-01",
                    "check_m248_d002_runner_reliability_and_platform_operations_modular_split_scaffolding_contract",
                ),
            ),
        ),
        (args.architecture_doc, "M248-D003-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M248-D003-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M248-D003-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M248-D003-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
        count, findings = check_doc_contract(path=path, exists_check_id=exists_check_id, snippets=snippets)
        checks_total += count
        failures.extend(findings)

    source_count, source_findings = check_source_contract(
        path=args.frontend_anchor,
        exists_check_id="M248-D003-CORE-EXISTS",
        required_snippets=FRONTEND_ANCHOR_REQUIRED_SNIPPETS,
        forbidden_snippets=FRONTEND_ANCHOR_FORBIDDEN_SNIPPETS,
    )
    checks_total += source_count
