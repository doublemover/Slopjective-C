#!/usr/bin/env python3
"""Fail-closed M21 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M21-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M21-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2468, last_issue=2482),
    LaneSpec(lane="B", bucket="semantic", first_issue=2483, last_issue=2496),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2497, last_issue=2510),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2511, last_issue=2524),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M21 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
