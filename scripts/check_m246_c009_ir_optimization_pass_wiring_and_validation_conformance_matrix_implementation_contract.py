#!/usr/bin/env python3
"""Fail-closed checker for M246-C009 IR optimization pass wiring conformance matrix implementation."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m246-c009-ir-optimization-pass-wiring-validation-conformance-matrix-implementation-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_c009_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m246"
    / "m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_packet.md"
)
DEFAULT_C001_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_ir_optimization_pass_wiring_and_validation_contract_and_architecture_freeze_c001_expectations.md"
)
DEFAULT_C002_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_c002_expectations.md"
)
DEFAULT_C008_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_c008_expectations.md"
)
DEFAULT_C008_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m246"
    / "m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_packet.md"
)
DEFAULT_C008_READINESS_SCRIPT = ROOT / "scripts" / "run_m246_c008_lane_c_readiness.py"
DEFAULT_READINESS_SCRIPT = ROOT / "scripts" / "run_m246_c009_lane_c_readiness.py"
DEFAULT_PACKAGE_JSON = ROOT / "package.json"
DEFAULT_SUMMARY_OUT = Path(
    "tmp/reports/m246/M246-C009/ir_optimization_pass_wiring_validation_conformance_matrix_implementation_summary.json"
)


@dataclass(frozen=True)
class AssetCheck:
    check_id: str
    relative_path: Path


@dataclass(frozen=True)
class SnippetCheck:
    check_id: str
    snippet: str


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


PREREQUISITE_ASSETS: tuple[AssetCheck, ...] = (
    AssetCheck(
        "M246-C009-DEP-C001-01",
        Path("docs/contracts/m246_ir_optimization_pass_wiring_and_validation_contract_and_architecture_freeze_c001_expectations.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C001-02",
        Path("spec/planning/compiler/m246/m246_c001_ir_optimization_pass_wiring_and_validation_contract_and_architecture_freeze_packet.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C001-03",
        Path("scripts/check_m246_c001_ir_optimization_pass_wiring_and_validation_contract_and_architecture_freeze_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C001-04",
        Path("tests/tooling/test_check_m246_c001_ir_optimization_pass_wiring_and_validation_contract_and_architecture_freeze_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C002-01",
        Path("docs/contracts/m246_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_c002_expectations.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C002-02",
        Path("spec/planning/compiler/m246/m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_packet.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C002-03",
        Path("scripts/check_m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C002-04",
        Path("tests/tooling/test_check_m246_c002_ir_optimization_pass_wiring_and_validation_modular_split_scaffolding_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C008-01",
        Path("docs/contracts/m246_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_c008_expectations.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C008-02",
        Path("spec/planning/compiler/m246/m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_packet.md"),
    ),
    AssetCheck(
        "M246-C009-DEP-C008-03",
        Path("scripts/check_m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C008-04",
        Path("tests/tooling/test_check_m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_contract.py"),
    ),
    AssetCheck(
        "M246-C009-DEP-C008-05",
        Path("scripts/run_m246_c008_lane_c_readiness.py"),
    ),
    AssetCheck(
        "M246-C009-OWN-01",
        Path("docs/contracts/m246_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_c009_expectations.md"),
    ),
    AssetCheck(
        "M246-C009-OWN-02",
        Path("spec/planning/compiler/m246/m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_packet.md"),
    ),
    AssetCheck(
        "M246-C009-OWN-03",
        Path("scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py"),
    ),
    AssetCheck(
        "M246-C009-OWN-04",
        Path("tests/tooling/test_check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py"),
    ),
    AssetCheck(
        "M246-C009-OWN-05",
        Path("scripts/run_m246_c009_lane_c_readiness.py"),
    ),
)

EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-DOC-EXP-01",
        "# M246 IR Optimization Pass Wiring and Validation Conformance Matrix Implementation Expectations (C009)",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-02",
        "Contract ID: `objc3c-ir-optimization-pass-wiring-validation-conformance-matrix-implementation/m246-c009-v1`",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-03",
        "Issue `#5085` defines canonical lane-C conformance matrix implementation scope.",
    ),
    SnippetCheck("M246-C009-DOC-EXP-12", "## Issue Anchor"),
    SnippetCheck("M246-C009-DOC-EXP-13", "Primary issue anchor: `#5085`"),
    SnippetCheck(
        "M246-C009-DOC-EXP-04",
        "Dependencies: `M246-C008`",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-05",
        "Predecessor anchors inherited via `M246-C008`: `M246-C001`, `M246-C002`, `M246-C003`, `M246-C004`, `M246-C005`, `M246-C006`, `M246-C007`.",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-06",
        "docs/contracts/m246_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_c008_expectations.md",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-07",
        "scripts/run_m246_c008_lane_c_readiness.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-08",
        "scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-09",
        "tests/tooling/test_check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-10",
        "improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-11",
        "tmp/reports/m246/M246-C009/ir_optimization_pass_wiring_validation_conformance_matrix_implementation_summary.json",
    ),
    SnippetCheck(
        "M246-C009-DOC-EXP-14",
        "`python scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py --emit-json --summary-out tmp/reports/m246/M246-C009/ir_optimization_pass_wiring_validation_conformance_matrix_implementation_summary.json`",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-DOC-PKT-01",
        "# M246-C009 IR Optimization Pass Wiring and Validation Conformance Matrix Implementation Packet",
    ),
    SnippetCheck("M246-C009-DOC-PKT-02", "Packet: `M246-C009`"),
    SnippetCheck("M246-C009-DOC-PKT-03", "Issue: `#5085`"),
    SnippetCheck("M246-C009-DOC-PKT-04", "Freeze date: `2026-03-04`"),
    SnippetCheck("M246-C009-DOC-PKT-05", "Dependencies: `M246-C008`"),
    SnippetCheck("M246-C009-DOC-PKT-13", "## Issue Anchor"),
    SnippetCheck("M246-C009-DOC-PKT-14", "Primary issue anchor: `#5085`"),
    SnippetCheck(
        "M246-C009-DOC-PKT-06",
        "Predecessor anchors inherited via `M246-C008`: `M246-C001`, `M246-C002`, `M246-C003`, `M246-C004`, `M246-C005`, `M246-C006`, `M246-C007`.",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-07",
        "scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-08",
        "tests/tooling/test_check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-09",
        "scripts/run_m246_c009_lane_c_readiness.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-10",
        "scripts/run_m246_c008_lane_c_readiness.py",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-11",
        "treated as mandatory scope",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-12",
        "tmp/reports/m246/M246-C009/ir_optimization_pass_wiring_validation_conformance_matrix_implementation_summary.json",
    ),
    SnippetCheck(
        "M246-C009-DOC-PKT-15",
        "python scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py --emit-json --summary-out tmp/reports/m246/M246-C009/ir_optimization_pass_wiring_validation_conformance_matrix_implementation_summary.json",
    ),
)

C001_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-C001-01",
        "Contract ID: `objc3c-ir-optimization-pass-wiring-validation/m246-c001-v1`",
    ),
    SnippetCheck("M246-C009-C001-02", "Dependencies: none"),
)

C002_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-C002-01",
        "Contract ID: `objc3c-ir-optimization-pass-wiring-validation-modular-split-scaffolding/m246-c002-v1`",
    ),
    SnippetCheck(
        "M246-C009-C002-02",
        "Dependencies: `M246-C001`",
    ),
)

C008_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-C008-EXP-01",
        "Contract ID: `objc3c-ir-optimization-pass-wiring-validation-recovery-and-determinism-hardening/m246-c008-v1`",
    ),
    SnippetCheck(
        "M246-C009-C008-EXP-02",
        "Dependencies: `M246-C007`",
    ),
    SnippetCheck(
        "M246-C009-C008-EXP-03",
        "scripts/run_m246_c008_lane_c_readiness.py",
    ),
)

C008_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M246-C009-C008-PKT-01", "Packet: `M246-C008`"),
    SnippetCheck("M246-C009-C008-PKT-02", "Dependencies: `M246-C007`"),
    SnippetCheck("M246-C009-C008-PKT-03", "scripts/run_m246_c008_lane_c_readiness.py"),
)

C008_READINESS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-C008-RUN-01",
        "scripts/run_m246_c007_lane_c_readiness.py",
    ),
    SnippetCheck(
        "M246-C009-C008-RUN-02",
        "scripts/check_m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_contract.py",
    ),
    SnippetCheck(
        "M246-C009-C008-RUN-03",
        "tests/tooling/test_check_m246_c008_ir_optimization_pass_wiring_and_validation_recovery_and_determinism_hardening_contract.py",
    ),
    SnippetCheck(
        "M246-C009-C008-RUN-04",
        "[ok] M246-C008 lane-C readiness chain completed",
    ),
)

READINESS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-RUN-01",
        "scripts/run_m246_c008_lane_c_readiness.py",
    ),
    SnippetCheck(
        "M246-C009-RUN-02",
        "scripts/check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-RUN-03",
        "tests/tooling/test_check_m246_c009_ir_optimization_pass_wiring_and_validation_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M246-C009-RUN-04",
        "[ok] M246-C009 lane-C readiness chain completed",
    ),
)

PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-C009-PKG-01",
        '"check:objc3c:m246-c002-lane-c-readiness": ',
    ),
    SnippetCheck("M246-C009-PKG-02", '"compile:objc3c": '),
    SnippetCheck("M246-C009-PKG-03", '"proof:objc3c": '),
    SnippetCheck("M246-C009-PKG-04", '"test:objc3c:execution-replay-proof": '),
    SnippetCheck("M246-C009-PKG-05", '"test:objc3c:perf-budget": '),
)


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def display_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        return resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return resolved.as_posix()


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expectations-doc", type=Path, default=DEFAULT_EXPECTATIONS_DOC)
    parser.add_argument("--packet-doc", type=Path, default=DEFAULT_PACKET_DOC)
    parser.add_argument("--c001-expectations-doc", type=Path, default=DEFAULT_C001_EXPECTATIONS_DOC)
    parser.add_argument("--c002-expectations-doc", type=Path, default=DEFAULT_C002_EXPECTATIONS_DOC)
    parser.add_argument("--c008-expectations-doc", type=Path, default=DEFAULT_C008_EXPECTATIONS_DOC)
    parser.add_argument("--c008-packet-doc", type=Path, default=DEFAULT_C008_PACKET_DOC)
    parser.add_argument("--c008-readiness-script", type=Path, default=DEFAULT_C008_READINESS_SCRIPT)
    parser.add_argument("--readiness-script", type=Path, default=DEFAULT_READINESS_SCRIPT)
    parser.add_argument("--package-json", type=Path, default=DEFAULT_PACKAGE_JSON)
    parser.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY_OUT)
    parser.add_argument(
        "--emit-json",
        action="store_true",
        help="Emit the summary payload to stdout as canonical JSON.",
    )
    return parser.parse_args(argv)


def check_prerequisite_assets() -> tuple[int, list[Finding]]:
    checks_total = 0
    findings: list[Finding] = []
    for asset in PREREQUISITE_ASSETS:
        checks_total += 1
        absolute = ROOT / asset.relative_path
        if not absolute.exists():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"missing prerequisite asset: {asset.relative_path.as_posix()}",
                )
            )
            continue
        if not absolute.is_file():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"prerequisite path is not a file: {asset.relative_path.as_posix()}",
                )
            )
    return checks_total, findings


def check_text_artifact(
    *,
    path: Path,
    exists_check_id: str,
    snippets: tuple[SnippetCheck, ...],
) -> tuple[int, list[Finding]]:
    checks_total = 1
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required document is missing: {display_path(path)}",
            )
        )
        return checks_total, findings
    if not path.is_file():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required path is not a file: {display_path(path)}",
            )
        )
        return checks_total, findings

    try:
        text = path.read_text(encoding="utf-8")
    except OSError as exc:
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"unable to read required document: {exc}",
            )
        )
        return checks_total, findings

    for snippet in snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(
                Finding(
                    artifact=display_path(path),
                    check_id=snippet.check_id,
                    detail=f"missing required snippet: {snippet.snippet}",
                )
            )
    return checks_total, findings


def finding_sort_key(finding: Finding) -> tuple[str, str, str]:
    return (finding.check_id, finding.artifact, finding.detail)


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    checks_total = 0
    failures: list[Finding] = []

    dep_checks, dep_failures = check_prerequisite_assets()
    checks_total += dep_checks
    failures.extend(dep_failures)

    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M246-C009-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M246-C009-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.c001_expectations_doc, "M246-C009-C001-EXISTS", C001_EXPECTATIONS_SNIPPETS),
        (args.c002_expectations_doc, "M246-C009-C002-EXISTS", C002_EXPECTATIONS_SNIPPETS),
        (args.c008_expectations_doc, "M246-C009-C008-EXP-EXISTS", C008_EXPECTATIONS_SNIPPETS),
        (args.c008_packet_doc, "M246-C009-C008-PKT-EXISTS", C008_PACKET_SNIPPETS),
        (args.c008_readiness_script, "M246-C009-C008-RUN-EXISTS", C008_READINESS_SNIPPETS),
        (args.readiness_script, "M246-C009-RUN-EXISTS", READINESS_SNIPPETS),
        (args.package_json, "M246-C009-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
        count, findings = check_text_artifact(path=path, exists_check_id=exists_check_id, snippets=snippets)
        checks_total += count
        failures.extend(findings)

    sorted_failures = sorted(failures, key=finding_sort_key)
    checks_passed = checks_total - len(sorted_failures)
    summary_payload = {
        "mode": MODE,
        "ok": not sorted_failures,
        "checks_total": checks_total,
        "checks_passed": checks_passed,
        "failures": [
            {"artifact": f.artifact, "check_id": f.check_id, "detail": f.detail}
            for f in sorted_failures
        ],
    }

    summary_path = args.summary_out if args.summary_out.is_absolute() else ROOT / args.summary_out
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary_path.write_text(canonical_json(summary_payload), encoding="utf-8")

    if args.emit_json:
        print(canonical_json(summary_payload), end="")

    if sorted_failures:
        for finding in sorted_failures:
            print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
        return 1
    if not args.emit_json:
        print(f"[ok] {MODE}: {checks_passed}/{checks_total} checks passed")
    return 0


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))
