#!/usr/bin/env python3
"""Fail-closed contract validator for v0.16 M38 lane-B release and scale foundation tooling."""
...
MODE = "release-scale-foundation-w2-contract-v1"
...
def validate_zero_open_json_schema(
    ...
    if not isinstance(inputs, dict):
        findings.append(DriftFinding(check_id="M38C-06", detail="payload field 'inputs' must be an object"))
    ...
    if not isinstance(freshness, dict):
        findings.append(
            DriftFinding(check_id="M38C-07", detail="payload field 'freshness' must be an object")
        )
    else:
    ...
    if not isinstance(triggers, list):
        findings.append(
            DriftFinding(check_id="M38C-08", detail="payload field 'triggers' must be an array")
        )
    else:
    ...
    if not isinstance(open_blockers, dict):
        findings.append(
            DriftFinding(check_id="M38C-09", detail="payload field 'open_blockers' must be an object")
        )
...
def render_drift_report(*, findings: list[DriftFinding], checker_script: Path, fixture_root: Path) -> str:
    rerun_command = render_command(
        [
            "python",
            "scripts/check_release_and_scale_foundation_w2_contract.py",
            "--checker-script",
    ...
    lines = [
        "release-scale-foundation-w2-contract: contract drift detected "
...
def check_contract(*, checker_script: Path, fixture_root: Path) -> int:
    ...
    print("release-scale-foundation-w2-contract: OK")
    print(f"- mode={MODE}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="check_release_and_scale_foundation_w2_contract.py",
        description=(
            "Fail-closed validator for v0.16 M38 lane-B activation-trigger schema/order "
            "and fixture parity contracts for release and scale foundation W2."
        ),
    )
    parser.add_argument(
        "--checker-script",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"release-scale-foundation-w2-contract: error: {exc}", file=sys.stderr)
        return 2
