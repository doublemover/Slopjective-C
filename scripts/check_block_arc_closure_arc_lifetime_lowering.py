...
OUT_DIR = ROOT / "tmp/reports/block-arc-closure/arc-lifetime-lowering"
JSON_OUT = OUT_DIR / "arc_lifetime_insertion_cleanup_lowering_summary.json"
SUMMARY_CONTRACT_ID = "objc3c.block_arc.closure.arc.lifetime.insertion.cleanup.lowering.summary.v1"
REQUIRED_SURFACES = {
...
def main() -> int:
    ...
    expect(
        unified_surface.get("arc_runtime_boundary_model")
        ...
        "block/ARC unified source surface drifted from the ARC runtime boundary model",
    )
    expect(
        ownership_surface.get("cleanup_ownership_transfer_field") == "cleanup_ownership_transfer_enforced",
        "ownership-transfer capture surface drifted from the cleanup ownership transfer field",
    )
    expect(
        lowering_surface.get("arc_inference_lifetime_contract_id") == "objc3c.arc.inference.lifetime.v1",
        "block/ARC lowering/helper surface drifted from the ARC inference lifetime contract",
    )
    expect(
        lowering_surface.get("llvm_ir_summary_paths") == [
        ...
        "block/ARC lowering/helper surface drifted from the expected ARC lowering summary paths",
    )
    expect(
        abi_surface.get("arc_debug_state_snapshot_symbol")
        == "objc3_runtime_copy_arc_debug_state_for_testing",
        "block/ARC runtime ABI surface drifted from the ARC debug state snapshot symbol",
    )
    ...
    payload = {
        ...
        "status": "PASS",
        "runner_path": "scripts/check_block_arc_closure_arc_lifetime_lowering.py",
