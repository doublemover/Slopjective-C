#!/usr/bin/env python3
"""Fail-closed M24 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M24-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M24-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2684, last_issue=2698),
    LaneSpec(lane="B", bucket="semantic", first_issue=2699, last_issue=2712),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2713, last_issue=2726),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2727, last_issue=2740),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M24 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
