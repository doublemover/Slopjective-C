...
DISTRIBUTION_CREDIBILITY_ACTION_SPECS: dict[str, ActionSpec] = {
    ...
    "validate-distribution-credibility": ActionSpec("validate-distribution-credibility", "run the integrated distribution-credibility workflow", "runner-internal distribution-credibility child actions", validation_tier="nightly", guarantee_owner="distribution trust signals, release-drill reporting, dashboard publication, and trust-report publication stay executable on the live shipped release surfaces"),
    "validate-distribution-credibility-end-to-end": ActionSpec("validate-distribution-credibility-end-to-end", "validate distribution trust-report publication and evidence linkage end to end", "python:scripts/check_objc3c_distribution_credibility_end_to_end.py", validation_tier="full", guarantee_owner="distribution credibility artifacts stay coherent with the live package-channel, release-operations, trust-signal, release-drill, and operator-action evidence paths"),
}
