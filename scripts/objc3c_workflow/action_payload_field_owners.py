...
ACTION_PAYLOAD_FIELD_OWNERS: tuple[ActionPayloadFieldOwner, ...] = (
    ...
    ActionPayloadFieldOwner(
        ...
        fields=(
            ...
            "registry_schema_owner_surface",
            "registry_schema_index_schema_id",
            "registry_schema_index_owner_surface",
        ),
    ...
    ActionPayloadFieldOwner(
        field_group="owner-contracts",
        fields=("payload_contracts",),
        owner_surface=ACTION_PAYLOAD_FIELD_OWNER_SURFACE,
        payload_surface="npm run objc3c -- --list-json actions[]",
        capability_truth_scope="workflow-action-payload-owner-contracts",
        public_contract=True,
    ),
)
...
def action_payload_owner_contracts() -> dict[str, object]:
    ...
    from .action_catalog_sections import action_catalog_section_merge_contract
    from .action_payload_category import action_category_contract_payload

    return {
        "catalog_section_merge": action_catalog_section_merge_contract(),
...
def action_payload_owner_fields(action: str) -> dict[str, object]:
    return {
        ...
        "payload_contracts": action_payload_owner_contracts(),
    }
