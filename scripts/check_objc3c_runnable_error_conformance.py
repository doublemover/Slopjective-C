#!/usr/bin/env python3
"""Validate runnable error conformance against the integrated live workflow."""
...
SUMMARY_CONTRACT_ID = "objc3c.runtime.runnable.error.conformance.summary.v1"
...
def main() -> int:
    ...
    for case_id in sorted(REQUIRED_CASES):
        ...
        expect(case.get("passed") is True, f"required error case {case_id} did not pass")
    ...
    abi_surface = acceptance_report["runtime_error_runtime_abi_cleanup_surface"]
    implementation_surface = acceptance_report[
        "runtime_error_propagation_catch_cleanup_runtime_implementation_surface"
    ]
    expect(
        "error-runtime-abi-cleanup" in abi_surface.get("authoritative_case_ids", []),
        "error runtime ABI surface did not carry the error-runtime-abi-cleanup case",
    )
    expect(
        "live-error-runtime-integration"
        in implementation_surface.get("authoritative_case_ids", []),
        "error runtime implementation surface did not carry the live-error-runtime-integration case",
    )
    expect(
        implementation_surface.get("private_error_runtime_abi_boundary")
        == abi_surface.get("private_error_runtime_abi_boundary"),
        "error runtime implementation surface drifted from the private error runtime ABI boundary",
    )
    ...
    payload = {
        ...
        "status": "PASS",
        "runner_path": "scripts/check_objc3c_runnable_error_conformance.py",
        ...
        "error_runtime_abi_cleanup_surface": abi_surface,
        "error_runtime_implementation_surface": implementation_surface,
    }
