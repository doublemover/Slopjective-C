...
RUNNER = ROOT / "scripts/check_objc3c_runnable_storage_reflection_conformance.py"
RUNNER_REPORT = ROOT / "tmp/reports/runtime/runnable-storage-reflection-conformance/summary.json"
ACCEPTANCE_REPORT = ROOT / "tmp/reports/runtime/acceptance/summary.json"
OUT_DIR = ROOT / "tmp/reports/object-model-closure/property-reflection-artifact"
JSON_OUT = OUT_DIR / "property_ivar_aggregate_reflection_artifact_summary.json"
SUMMARY_CONTRACT_ID = "objc3c.object_model.closure.property.ivar.aggregate.reflection.artifact.summary.v1"
REQUIRED_SURFACES = {
    "storage_accessor_runtime_abi_surface": "objc3c.runtime.storage.accessor.abi.surface.v1",
    "runtime_property_ivar_accessor_reflection_implementation_surface": "objc3c.runtime.property.ivar.accessor.reflection.implementation.surface.v1",
    "runtime_property_atomicity_synthesis_reflection_source_surface": "objc3c.runtime.property.atomicity.synthesis.reflection.source.surface.v1",
...
def main() -> int:
    if os.environ.get("OBJC3C_SKIP_STORAGE_REFLECTION_CONFORMANCE_RERUN") != "1":
        ...
        if result.returncode != 0:
            raise RuntimeError("runnable storage/reflection conformance runner failed")

    runner_report = load_json(RUNNER_REPORT)
    acceptance_report = load_json(ACCEPTANCE_REPORT)
    expect(runner_report.get("status") == "PASS", "storage/reflection conformance report did not publish PASS")
    ...
    implementation_surface = acceptance_report["runtime_property_ivar_accessor_reflection_implementation_surface"]
    expect(
        implementation_surface.get("implementation_snapshot_symbol") == "objc3_runtime_copy_storage_accessor_implementation_snapshot_for_testing",
        "storage/reflection implementation surface drifted from the implementation snapshot symbol",
    )
    expect(
        abi_surface.get("property_registry_state_snapshot_symbol") == "objc3_runtime_copy_property_registry_state_for_testing",
    ...
    payload = {
        ...
        "status": "PASS",
        "runner_path": "scripts/check_object_model_closure_property_reflection_artifact.py",
        "used_existing_storage_reflection_conformance_report": os.environ.get(
            "OBJC3C_SKIP_STORAGE_REFLECTION_CONFORMANCE_RERUN"
        ) == "1",
