...
def ensure_integration_report() -> dict[str, Any]:
    if INTEGRATION_REPORT.is_file():
        ...
        if report.get("status") == "PASS":
            return report
    completed = run_capture([sys.executable, str(RUNNER), "validate-public-conformance-reporting-integration"])
...
def main() -> int:
    ...
    expect(
        '"validate-public-conformance-reporting"' in runner_source,
        "test-nightly wiring no longer references validate-public-conformance-reporting",
    )
