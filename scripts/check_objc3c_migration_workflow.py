...
def main() -> int:
    ...
    expect(positive.payload["observed_surfaces"] == {surface: True for surface in contract["required_surfaces"]}, "positive observed surfaces drifted", failures)
    expect(positive.payload["rewrite_plan"]["automatic_edit_count"] >= 6, "rewrite plan did not include safe automatic edits", failures)
    expect(positive.payload["rewrite_plan"]["manual_step_count"] >= 3, "rewrite plan did not surface manual migration steps", failures)
    ...
    expect(any(item["code"] == "O3M210" for item in negative.payload["diagnostics"]), "negative unsafe mixed-image diagnostic missing", failures)
