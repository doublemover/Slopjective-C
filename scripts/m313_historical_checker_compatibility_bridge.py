...
def build_bridge_summary(bridge: dict[str, object], summary_out: Path | None) -> dict[str, object]:
    ...
    if summary_out is None:
        summary_out = ROOT / "tmp" / "reports" / "m313" / "compatibility" / str(bridge["bridge_id"]) / "summary.json"
    ...
    return {
        ...
        "producer": {
            "tool": "scripts/m313_historical_checker_compatibility_bridge.py",
            "issue": "M313-C003",
        ...
        "replay": {
            "commands": [
                f"python scripts/m313_historical_checker_compatibility_bridge.py --run-bridge {bridge['bridge_id']} --summary-out {summary_rel}"
            ],
            "cwd": ".",
        },
        "measurements": {
