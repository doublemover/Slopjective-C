#!/usr/bin/env python3
"""Fail-closed M15 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M15-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M15-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    ...
    LaneSpec(lane="B", bucket="semantic", first_issue=1907, last_issue=1916),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1917, last_issue=1926),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1927, last_issue=1936),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M15 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
