...
def assert_editor_tooling_reports(
    ...
    source_graph = editor_surface.get("source_graph", {})
    expect(source_graph.get("contract_id") == "objc3c.developer.tooling.source.graph.v1", "expected editor tooling source graph contract id", failures)
    expect(source_graph.get("evidence", {}).get("lexical_candidates_authoritative") is False, "expected source graph lexical candidates to remain non-authoritative", failures)
