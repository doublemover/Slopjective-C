#!/usr/bin/env python3
"""Validate the integrated performance-governance workflow report."""
...
def ensure_validate_report() -> dict[str, Any]:
    ...
    expect(completed.returncode == 0, "validate-performance-governance command failed during integration validation")
    return load_json(VALIDATE_REPORT)


def main() -> int:
    workflow_report = ensure_validate_report()
    expect(workflow_report.get("status") == "PASS", "validate-performance-governance workflow report did not pass")
    steps = workflow_report.get("steps", [])
    expect(isinstance(steps, list), "validate-performance-governance workflow report steps drifted")
    step_actions = [str(step.get("action")) for step in steps if isinstance(step, dict)]
    expect(step_actions == REQUIRED_STEPS, "validate-performance-governance workflow report step inventory drifted")
    ...
    for relative_path, contract_id in REQUIRED_CHILD_REPORTS.items():
        expect(relative_path in seen_report_paths, f"validate-performance-governance did not publish {relative_path}")
        payload = load_json(ROOT / relative_path)
        expect(payload.get("contract_id") == contract_id, f"performance-governance child report contract drifted for {relative_path}")
        expect(payload.get("status") == "PASS", f"performance-governance child report did not pass: {relative_path}")
        child_reports[relative_path] = payload

    dashboard = child_reports["tmp/reports/performance-governance/dashboard-summary.json"]
    public_summary = child_reports["tmp/reports/performance-governance/public-summary.json"]
    expect(public_summary.get("release_status") == dashboard.get("release_status"), "public performance summary release_status drifted from dashboard")
    expect(public_summary.get("claim_ready") == dashboard.get("claim_ready"), "public performance summary claim_ready drifted from dashboard")
    ...
    expect((ROOT / report_markdown_path).is_file(), "performance-governance markdown report is missing")
    ...
    payload = {
        ...
        "release_status": dashboard.get("release_status"),
        "claim_ready": dashboard.get("claim_ready"),
    }
    REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    REPORT_PATH.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"summary_path: {repo_rel(REPORT_PATH)}")
    print("objc3c-performance-governance-integration: PASS")
    return 0
