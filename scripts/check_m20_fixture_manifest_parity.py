#!/usr/bin/env python3
"""Fail-closed M20 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M20-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M20-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2396, last_issue=2410),
    LaneSpec(lane="B", bucket="semantic", first_issue=2411, last_issue=2424),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2425, last_issue=2438),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2439, last_issue=2452),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M20 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
