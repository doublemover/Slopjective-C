#!/usr/bin/env python3
"""Fail-closed checker for M235-D008 interop behavior recovery and determinism hardening."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m235_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_d008_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m235"
    / "m235_d008_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_packet.md"
)
DEFAULT_D007_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m235_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_d007_expectations.md"
)
DEFAULT_D007_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m235"
    / "m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_packet.md"
)
DEFAULT_D007_CHECKER = (
    ROOT / "scripts" / "check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py"
)
DEFAULT_D007_TEST = (
    ROOT
    / "tests"
    / "tooling"
    / "test_check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py"
)
DEFAULT_ARCHITECTURE_DOC = ROOT / "native" / "objc3c" / "src" / "ARCHITECTURE.md"
DEFAULT_LOWERING_SPEC = ROOT / "spec" / "LOWERING_AND_RUNTIME_CONTRACTS.md"
DEFAULT_METADATA_SPEC = ROOT / "spec" / "MODULE_METADATA_AND_ABI_TABLES.md"
DEFAULT_PACKAGE_JSON = ROOT / "package.json"
DEFAULT_SUMMARY_OUT = Path(
    "tmp/reports/m235/M235-D008/interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract_summary.json"
)


@dataclass(frozen=True)
class AssetCheck:
    check_id: str
    relative_path: Path


@dataclass(frozen=True)
class SnippetCheck:
    check_id: str
    snippet: str


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


PREREQUISITE_ASSETS: tuple[AssetCheck, ...] = (
    AssetCheck(
        "M235-D008-DEP-D007-01",
        Path("docs/contracts/m235_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_d007_expectations.md"),
    ),
    AssetCheck(
        "M235-D008-DEP-D007-02",
        Path("spec/planning/compiler/m235/m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_packet.md"),
    ),
    AssetCheck(
        "M235-D008-DEP-D007-03",
        Path("scripts/check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py"),
    ),
    AssetCheck(
        "M235-D008-DEP-D007-04",
        Path("tests/tooling/test_check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py"),
    ),
)

EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-DOC-EXP-01",
        "# M235 Interop Behavior for Qualified Generic APIs Recovery and Determinism Hardening Expectations (D008)",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-02",
        "Contract ID: `objc3c-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening/m235-d008-v1`",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-03",
        "Issue `#5838` defines canonical lane-D recovery and determinism hardening scope.",
    ),
    SnippetCheck("M235-D008-DOC-EXP-04", "Dependencies: `M235-D007`"),
    SnippetCheck(
        "M235-D008-DOC-EXP-05",
        "milestone optimization improvements are mandatory scope inputs.",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-06",
        "docs/contracts/m235_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_d007_expectations.md",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-07",
        "scripts/check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-08",
        "tests/tooling/test_check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py",
    ),
    SnippetCheck("M235-D008-DOC-EXP-09", "`check:objc3c:m235-d007-lane-d-readiness`"),
    SnippetCheck(
        "M235-D008-DOC-EXP-10",
        "`check:objc3c:m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract`",
    ),
    SnippetCheck(
        "M235-D008-DOC-EXP-11",
        "`check:objc3c:m235-d008-lane-d-readiness`",
    ),
    SnippetCheck("M235-D008-DOC-EXP-12", "`compile:objc3c`"),
    SnippetCheck(
        "M235-D008-DOC-EXP-13",
        "`tmp/reports/m235/M235-D008/interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract_summary.json`",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-DOC-PKT-01",
        "# M235-D008 Interop Behavior for Qualified Generic APIs Recovery and Determinism Hardening Packet",
    ),
    SnippetCheck("M235-D008-DOC-PKT-02", "Packet: `M235-D008`"),
    SnippetCheck("M235-D008-DOC-PKT-03", "Issue: `#5838`"),
    SnippetCheck("M235-D008-DOC-PKT-04", "Dependencies: `M235-D007`"),
    SnippetCheck(
        "M235-D008-DOC-PKT-05",
        "including code/spec anchors and milestone optimization",
    ),
    SnippetCheck(
        "M235-D008-DOC-PKT-06",
        "docs/contracts/m235_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_d008_expectations.md",
    ),
    SnippetCheck(
        "M235-D008-DOC-PKT-07",
        "scripts/check_m235_d008_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract.py",
    ),
    SnippetCheck(
        "M235-D008-DOC-PKT-08",
        "tests/tooling/test_check_m235_d008_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract.py",
    ),
    SnippetCheck("M235-D008-DOC-PKT-09", "`npm run check:objc3c:m235-d007-lane-d-readiness`"),
    SnippetCheck(
        "M235-D008-DOC-PKT-10",
        "`tmp/reports/m235/M235-D008/interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract_summary.json`",
    ),
    SnippetCheck(
        "M235-D008-DOC-PKT-11",
        "spec/planning/compiler/m235/m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_packet.md",
    ),
    SnippetCheck("M235-D008-DOC-PKT-12", "`npm run check:objc3c:m235-d008-lane-d-readiness`"),
)

D007_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-D007-DOC-01",
        "# M235 Interop Behavior for Qualified Generic APIs Diagnostics Hardening Expectations (D007)",
    ),
    SnippetCheck(
        "M235-D008-D007-DOC-02",
        "Contract ID: `objc3c-interop-behavior-for-qualified-generic-apis-diagnostics-hardening/m235-d007-v1`",
    ),
    SnippetCheck(
        "M235-D008-D007-DOC-03",
        "Issue `#5837` defines canonical lane-D diagnostics hardening scope.",
    ),
)

D007_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M235-D008-D007-PKT-01", "Packet: `M235-D007`"),
    SnippetCheck("M235-D008-D007-PKT-02", "Issue: `#5837`"),
    SnippetCheck("M235-D008-D007-PKT-03", "Dependencies: `M235-D006`"),
    SnippetCheck(
        "M235-D008-D007-PKT-04",
        "`tmp/reports/m235/M235-D007/interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract_summary.json`",
    ),
)

ARCHITECTURE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-ARCH-01",
        "M235 lane-D D008 interop behavior for qualified generic APIs recovery and determinism hardening anchors explicit",
    ),
    SnippetCheck(
        "M235-D008-ARCH-02",
        "docs/contracts/m235_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_d008_expectations.md",
    ),
)

LOWERING_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-SPC-01",
        "interop behavior for qualified generic APIs recovery and determinism hardening governance shall preserve explicit",
    ),
    SnippetCheck(
        "M235-D008-SPC-02",
        "lane-D dependency anchors (`M235-D007`) and fail closed on recovery and determinism hardening interop evidence drift",
    ),
)

METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-META-01",
        "deterministic lane-D interop behavior for qualified generic APIs recovery and determinism hardening metadata anchors for `M235-D008`",
    ),
    SnippetCheck(
        "M235-D008-META-02",
        "with explicit `M235-D007` dependency continuity so recovery and determinism hardening interop drift fails closed.",
    ),
)

PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M235-D008-PKG-01",
        '"check:objc3c:m235-d007-interop-behavior-for-qualified-generic-apis-diagnostics-hardening-contract": '
        '"python scripts/check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py"',
    ),
    SnippetCheck(
        "M235-D008-PKG-02",
        '"test:tooling:m235-d007-interop-behavior-for-qualified-generic-apis-diagnostics-hardening-contract": '
        '"python -m pytest tests/tooling/test_check_m235_d007_interop_behavior_for_qualified_generic_apis_diagnostics_hardening_contract.py -q"',
    ),
    SnippetCheck(
        "M235-D008-PKG-03",
        '"check:objc3c:m235-d007-lane-d-readiness": '
        '"npm run check:objc3c:m235-d006-lane-d-readiness '
        '&& npm run check:objc3c:m235-d007-interop-behavior-for-qualified-generic-apis-diagnostics-hardening-contract '
        '&& npm run test:tooling:m235-d007-interop-behavior-for-qualified-generic-apis-diagnostics-hardening-contract"',
    ),
    SnippetCheck(
        "M235-D008-PKG-04",
        '"check:objc3c:m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract": '
        '"python scripts/check_m235_d008_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract.py"',
    ),
    SnippetCheck(
        "M235-D008-PKG-05",
        '"test:tooling:m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract": '
        '"python -m pytest tests/tooling/test_check_m235_d008_interop_behavior_for_qualified_generic_apis_recovery_and_determinism_hardening_contract.py -q"',
    ),
    SnippetCheck(
        "M235-D008-PKG-06",
        '"check:objc3c:m235-d008-lane-d-readiness": '
        '"npm run check:objc3c:m235-d007-lane-d-readiness '
        '&& npm run check:objc3c:m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract '
        '&& npm run test:tooling:m235-d008-interop-behavior-for-qualified-generic-apis-recovery-and-determinism-hardening-contract"',
    ),
    SnippetCheck("M235-D008-PKG-07", '"compile:objc3c": '),
    SnippetCheck("M235-D008-PKG-08", '"proof:objc3c": '),
    SnippetCheck("M235-D008-PKG-09", '"test:objc3c:execution-replay-proof": '),
    SnippetCheck("M235-D008-PKG-10", '"test:objc3c:perf-budget": '),
)


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def display_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        return resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return resolved.as_posix()


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expectations-doc", type=Path, default=DEFAULT_EXPECTATIONS_DOC)
    parser.add_argument("--packet-doc", type=Path, default=DEFAULT_PACKET_DOC)
    parser.add_argument("--d007-expectations-doc", type=Path, default=DEFAULT_D007_EXPECTATIONS_DOC)
    parser.add_argument("--d007-packet-doc", type=Path, default=DEFAULT_D007_PACKET_DOC)
    parser.add_argument("--d007-checker", type=Path, default=DEFAULT_D007_CHECKER)
    parser.add_argument("--d007-test", type=Path, default=DEFAULT_D007_TEST)
    parser.add_argument("--architecture-doc", type=Path, default=DEFAULT_ARCHITECTURE_DOC)
    parser.add_argument("--lowering-spec", type=Path, default=DEFAULT_LOWERING_SPEC)
    parser.add_argument("--metadata-spec", type=Path, default=DEFAULT_METADATA_SPEC)
    parser.add_argument("--package-json", type=Path, default=DEFAULT_PACKAGE_JSON)
    parser.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY_OUT)
    return parser.parse_args(argv)


def check_prerequisite_assets() -> tuple[int, list[Finding]]:
    checks_total = 0
    findings: list[Finding] = []
    for asset in PREREQUISITE_ASSETS:
        checks_total += 1
        absolute = ROOT / asset.relative_path
        if not absolute.exists():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"missing prerequisite asset: {asset.relative_path.as_posix()}",
                )
            )
            continue
        if not absolute.is_file():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"prerequisite path is not a file: {asset.relative_path.as_posix()}",
                )
            )
    return checks_total, findings


def check_doc_contract(
    *,
    path: Path,
    exists_check_id: str,
    snippets: tuple[SnippetCheck, ...],
) -> tuple[int, list[Finding]]:
    checks_total = 1
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                display_path(path),
                exists_check_id,
                f"required document is missing: {display_path(path)}",
            )
        )
        return checks_total, findings
    if not path.is_file():
        findings.append(
            Finding(
                display_path(path),
                exists_check_id,
                f"required path is not a file: {display_path(path)}",
            )
        )
        return checks_total, findings

    text = path.read_text(encoding="utf-8")
    for snippet in snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(
                Finding(
                    display_path(path),
                    snippet.check_id,
                    f"missing required snippet: {snippet.snippet}",
                )
            )
    return checks_total, findings


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    checks_total = 0
    failures: list[Finding] = []

    dep_checks, dep_failures = check_prerequisite_assets()
    checks_total += dep_checks
    failures.extend(dep_failures)

    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M235-D008-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M235-D008-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.d007_expectations_doc, "M235-D008-D007-DOC-EXISTS", D007_EXPECTATIONS_SNIPPETS),
        (args.d007_packet_doc, "M235-D008-D007-PKT-EXISTS", D007_PACKET_SNIPPETS),
        (args.architecture_doc, "M235-D008-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M235-D008-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M235-D008-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M235-D008-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
        count, finding_batch = check_doc_contract(
            path=path,
            exists_check_id=exists_check_id,
            snippets=snippets,
        )
        checks_total += count
        failures.extend(finding_batch)

    for path, check_id in (
        (args.d007_checker, "M235-D008-DEP-D007-ARG-01"),
        (args.d007_test, "M235-D008-DEP-D007-ARG-02"),
    ):
        checks_total += 1
        if not path.exists():
            failures.append(
                Finding(
                    display_path(path),
                    check_id,
                    f"required dependency path is missing: {display_path(path)}",
                )
            )
            continue
        if not path.is_file():
            failures.append(
                Finding(
                    display_path(path),
                    check_id,
                    f"required dependency path is not a file: {display_path(path)}",
                )
            )

    checks_passed = checks_total - len(failures)
    summary_payload = {
        "mode": MODE,
        "ok": not failures,
        "checks_total": checks_total,
        "checks_passed": checks_passed,
        "failures": [
            {"artifact": finding.artifact, "check_id": finding.check_id, "detail": finding.detail}
            for finding in failures
        ],
    }

    summary_path = args.summary_out if args.summary_out.is_absolute() else ROOT / args.summary_out
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary_path.write_text(canonical_json(summary_payload), encoding="utf-8")

    if failures:
        for finding in failures:
            print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
        return 1
    print(f"[ok] {MODE}: {checks_passed}/{checks_total} checks passed")
    return 0


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))






