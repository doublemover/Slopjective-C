#!/usr/bin/env python3
"""Fail-closed M18 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M18-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M18-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2252, last_issue=2266),
    LaneSpec(lane="B", bucket="semantic", first_issue=2267, last_issue=2280),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2281, last_issue=2294),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2295, last_issue=2308),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M18 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
