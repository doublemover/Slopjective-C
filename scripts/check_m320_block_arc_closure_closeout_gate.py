...
def main() -> int:
    run_capture([sys.executable, str(ROOT / "scripts/build_block_arc_closure_executable_proof_summary.py")])
    run_capture(
        [sys.executable, str(ROOT / "scripts/check_block_arc_closure_escaping_block_lowering.py")],
        env={"OBJC3C_SKIP_BLOCK_ARC_CONFORMANCE_RERUN": "1"},
    )
    run_capture(
        [sys.executable, str(ROOT / "scripts/check_block_arc_closure_arc_lifetime_lowering.py")],
        env={"OBJC3C_SKIP_BLOCK_ARC_CONFORMANCE_RERUN": "1"},
    )
    run_capture(
        [sys.executable, str(ROOT / "scripts/check_block_arc_closure_live_block_runtime.py")],
        env={"OBJC3C_SKIP_BLOCK_ARC_E2E_RERUN": "1"},
    )
    run_capture(
        [sys.executable, str(ROOT / "scripts/check_block_arc_closure_live_arc_runtime.py")],
        env={"OBJC3C_SKIP_BLOCK_ARC_E2E_RERUN": "1"},
    )
    ...
    payload = {
        "contract_id": "objc3c.block_arc.closure.closeout.gate.summary.v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "runner_path": "scripts/check_m320_block_arc_closure_closeout_gate.py",
