#!/usr/bin/env python3
"""Fail-closed validator for M226-B008 parser-sema recovery/determinism hardening."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m226-parser-sema-recovery-determinism-hardening-contract-b008-v1"

ARTIFACTS: dict[str, Path] = {
    "sema_pass_manager": ROOT / "native" / "objc3c" / "src" / "sema" / "objc3_sema_pass_manager.cpp",
    "sema_pass_manager_contract": ROOT
    / "native"
    / "objc3c"
    / "src"
    / "sema"
    / "objc3_sema_pass_manager_contract.h",
    "contract_doc": ROOT / "docs" / "contracts" / "m226_parser_sema_recovery_determinism_hardening_b008_expectations.md",
}

REQUIRED_SNIPPETS: dict[str, tuple[tuple[str, str], ...]] = {
    "sema_pass_manager": (
        (
            "M226-B008-SEM-01",
            "bool IsEquivalentBlockDeterminismPerfBaselineSiteMetadata(",
        ),
        ("M226-B008-SEM-02", "bool AreEquivalentBlockDeterminismPerfBaselineSites("),
        (
            "M226-B008-SEM-03",
            ".fail_closed_diagnostic_sites <=\n"
            "          result.type_metadata_handoff.error_diagnostics_recovery_summary\n"
            "              .diagnostic_emit_sites &&",
        ),
        (
            "M226-B008-SEM-04",
            "AreEquivalentBlockDeterminismPerfBaselineSites(\n"
            "          result.integration_surface.block_determinism_perf_baseline_sites_lexicographic,\n"
            "          result.type_metadata_handoff.block_determinism_perf_baseline_sites_lexicographic) &&",
        ),
        (
            "M226-B008-SEM-05",
            "const bool recovery_and_block_determinism_hardening_consistent =",
        ),
        (
            "M226-B008-SEM-06",
            "result.deterministic_type_metadata_handoff =\n"
            "      result.deterministic_type_metadata_handoff &&\n"
            "      recovery_and_block_determinism_hardening_consistent;",
        ),
        (
            "M226-B008-SEM-07",
            "if (!recovery_and_block_determinism_hardening_consistent) {\n"
            "    return result;\n"
            "  }",
        ),
        (
            "M226-B008-SEM-08",
            ".fail_closed_diagnostic_sites <=\n"
            "          result.parity_surface.error_diagnostics_recovery_summary\n"
            "              .diagnostic_emit_sites &&",
        ),
    ),
    "sema_pass_manager_contract": (
        (
            "M226-B008-CON-01",
            ".fail_closed_diagnostic_sites <=\n"
            "             surface.error_diagnostics_recovery_summary\n"
            "                 .diagnostic_emit_sites &&",
        ),
    ),
    "contract_doc": (
        (
            "M226-B008-DOC-01",
            "Contract ID: `objc3c-parser-sema-recovery-determinism-hardening-contract/m226-b008-v1`",
        ),
        (
            "M226-B008-DOC-02",
            "`fail_closed_diagnostic_sites <= diagnostic_emit_sites`",
        ),
        (
            "M226-B008-DOC-03",
            "`AreEquivalentBlockDeterminismPerfBaselineSites(...)`",
        ),
        (
            "M226-B008-DOC-04",
            "`if (!recovery_and_block_determinism_hardening_consistent) { return result; }`",
        ),
        (
            "M226-B008-DOC-05",
            "`python scripts/check_m226_b008_parser_sema_recovery_determinism_hardening_contract.py`",
        ),
        (
            "M226-B008-DOC-06",
            "`python -m pytest tests/tooling/test_check_m226_b008_parser_sema_recovery_determinism_hardening_contract.py -q`",
        ),
    ),
}


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--summary-out",
        type=Path,
        default=Path("tmp/reports/m226/M226-B008/parser_sema_recovery_determinism_hardening_summary.json"),
    )
    return parser.parse_args(argv)


def load_text(path: Path, *, artifact: str) -> str:
    if not path.exists() or not path.is_file():
        raise ValueError(f"{artifact} missing file: {path.as_posix()}")
    return path.read_text(encoding="utf-8")


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)

    findings: list[Finding] = []
    total_checks = 0
    passed_checks = 0

    for artifact, path in ARTIFACTS.items():
        text = load_text(path, artifact=artifact)
        for check_id, snippet in REQUIRED_SNIPPETS.get(artifact, ()):
            total_checks += 1
            if snippet in text:
                passed_checks += 1
            else:
                findings.append(Finding(artifact, check_id, f"expected snippet missing: {snippet}"))

    ok = not findings
    summary = {
        "mode": MODE,
        "ok": ok,
        "checks_total": total_checks,
        "checks_passed": passed_checks,
        "failures": [
            {
                "artifact": finding.artifact,
                "check_id": finding.check_id,
                "detail": finding.detail,
            }
            for finding in findings
        ],
    }
    args.summary_out.parent.mkdir(parents=True, exist_ok=True)
    args.summary_out.write_text(canonical_json(summary), encoding="utf-8")

    if ok:
        return 0
    for finding in findings:
        print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
    return 1


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))
