#!/usr/bin/env python3
"""Fail-closed contract validator for v0.15 M27 lane-B activation tooling."""
...
def validate_zero_open_json_schema(
    ...
    if not isinstance(inputs, dict):
        findings.append(DriftFinding(check_id="W1C-06", detail="payload field 'inputs' must be an object"))
    ...
    if not isinstance(freshness, dict):
        findings.append(
            DriftFinding(check_id="W1C-07", detail="payload field 'freshness' must be an object")
        )
    else:
    ...
    if not isinstance(triggers, list):
        findings.append(
            DriftFinding(check_id="W1C-08", detail="payload field 'triggers' must be an array")
        )
    else:
    ...
    if not isinstance(open_blockers, dict):
        findings.append(
            DriftFinding(check_id="W1C-09", detail="payload field 'open_blockers' must be an object")
        )
...
def render_drift_report(*, findings: list[DriftFinding], checker_script: Path, fixture_root: Path) -> str:
    rerun_command = render_command(
        [
            "python",
            "scripts/check_activation_execution_hardening_w1_contract.py",
            "--checker-script",
    ...
    lines = [
        "activation-execution-hardening-w1-contract: contract drift detected "
...
def check_contract(*, checker_script: Path, fixture_root: Path) -> int:
    ...
    print("activation-execution-hardening-w1-contract: OK")
    print(f"- mode={MODE}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="check_activation_execution_hardening_w1_contract.py",
        description=(
            "Fail-closed validator for v0.15 M27 lane-B activation-trigger schema/order "
            "and fixture parity contracts for activation execution hardening W1."
        ),
    )
    parser.add_argument(
        "--checker-script",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"activation-execution-hardening-w1-contract: error: {exc}", file=sys.stderr)
        return 2
