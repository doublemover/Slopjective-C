#!/usr/bin/env python3
"""Fail-closed checker for M229-C012 interop boundary ABI handling contract freeze."""
...
MODE = "m229-c012-interop-boundary-abi-handling-cross-lane-integration-sync-contract-v1"
...
DEFAULT_PACKET_DOC = (
    ...
    / "compiler"
    / "m229"
    / "m229_c012_interop_boundary_abi_handling_cross_lane_integration_sync_packet.md"
)
...
EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M229-C012-DOC-EXP-01",
        "# M229 Interop boundary ABI handling Cross-lane Integration Sync Expectations (B001)",
    ),
    SnippetCheck(
        "M229-C012-DOC-EXP-02",
        "Contract ID: `objc3c-interop-boundary-abi-handling/m229-c012-v1`",
    ),
    SnippetCheck("M229-C012-DOC-EXP-03", "Issue: `#5340`"),
    SnippetCheck("M229-C012-DOC-EXP-04", "Dependencies: `M229-C011`"),
    SnippetCheck(
        "M229-C012-DOC-EXP-05",
        "code/spec anchors and milestone optimization improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M229-C012-DOC-EXP-06",
        "`check:objc3c:m229-c012-lane-c-readiness`",
    ),
    SnippetCheck(
        "M229-C012-DOC-EXP-07",
        "`tmp/reports/m229/M229-C012/interop_boundary_abi_handling_cross_lane_integration_sync_summary.json`",
    ),
)
...
PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M229-C012-DOC-PKT-01",
        "# M229-C012 Interop boundary ABI handling Cross-lane Integration Sync Packet",
    ),
    SnippetCheck("M229-C012-DOC-PKT-02", "Packet: `M229-C012`"),
    SnippetCheck("M229-C012-DOC-PKT-03", "Issue: `#5340`"),
    SnippetCheck("M229-C012-DOC-PKT-04", "Dependencies: `M229-C011`"),
    SnippetCheck(
        "M229-C012-DOC-PKT-05",
        "check:objc3c:m229-c012-interop-boundary-abi-handling-cross-lane-integration-sync-contract",
    ),
    SnippetCheck(
        "M229-C012-DOC-PKT-06",
        "tmp/reports/m229/M229-C012/interop_boundary_abi_handling_cross_lane_integration_sync_summary.json",
    ),
)
...
PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M229-C012-PKG-03",
        '"check:objc3c:m229-c012-lane-c-readiness": '
        '"npm run check:objc3c:m229-c011-lane-c-readiness && npm run check:objc3c:m229-c012-interop-boundary-abi-handling-cross-lane-integration-sync-contract '
        '&& npm run test:tooling:m229-c012-interop-boundary-abi-handling-cross-lane-integration-sync-contract"',
    ),
)
...
def run(argv: Sequence[str]) -> int:
    ...
    artifact_inputs = (
        ("expectations_doc", "M229-C012-EXPECTATIONS", args.expectations_doc, EXPECTATIONS_SNIPPETS),
        ("packet_doc", "M229-C012-PACKET", args.packet_doc, PACKET_SNIPPETS),
        ("architecture_doc", "M229-C012-ARCHITECTURE", args.architecture_doc, ARCHITECTURE_SNIPPETS),
        ("lowering_spec", "M229-C012-LOWERING-SPEC", args.lowering_spec, LOWERING_SPEC_SNIPPETS),
        ("metadata_spec", "M229-C012-METADATA-SPEC", args.metadata_spec, METADATA_SPEC_SNIPPETS),
        ("package_json", "M229-C012-PACKAGE", args.package_json, PACKAGE_SNIPPETS),
    )
