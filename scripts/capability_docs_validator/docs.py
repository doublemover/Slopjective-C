...
def _support_markdown_docs() -> list[Path]:
    return sorted(SUPPORT_DOCS_DIR.glob("*.md"))


def _validate_public_doc_claim_tokens(
    docs_text: str,
    matrix_support_claims: set[str],
) -> None:
...
def _validate_docs_reference_rows(rows: list[dict[str, Any]]) -> None:
    ...
    docs_text = "\n".join(path.read_text(encoding="utf-8") for path in _support_markdown_docs())
    ...
    _validate_public_doc_claim_tokens(docs_text, matrix_support_claims)
