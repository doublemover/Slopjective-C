#!/usr/bin/env python3
"""Fail-closed contract validator for v0.15 M33 lane-B dispatch evidence reliability tooling."""
...
MODE = "dispatch-evidence-reliability-w2-contract-v1"
...
def validate_zero_open_json_schema(
    ...
    if not isinstance(inputs, dict):
        findings.append(DriftFinding(check_id="M33C-06", detail="payload field 'inputs' must be an object"))
    ...
    if not isinstance(freshness, dict):
        findings.append(
            DriftFinding(check_id="M33C-07", detail="payload field 'freshness' must be an object")
        )
    else:
    ...
    if not isinstance(triggers, list):
        findings.append(
            DriftFinding(check_id="M33C-08", detail="payload field 'triggers' must be an array")
        )
    else:
    ...
    if not isinstance(open_blockers, dict):
        findings.append(
            DriftFinding(check_id="M33C-09", detail="payload field 'open_blockers' must be an object")
        )
...
def render_drift_report(*, findings: list[DriftFinding], checker_script: Path, fixture_root: Path) -> str:
    rerun_command = render_command(
        [
            "python",
            "scripts/check_dispatch_evidence_reliability_w2_contract.py",
            "--checker-script",
    ...
    lines = [
        "dispatch-evidence-reliability-w2-contract: contract drift detected "
...
def check_contract(*, checker_script: Path, fixture_root: Path) -> int:
    ...
    print("dispatch-evidence-reliability-w2-contract: OK")
    print(f"- mode={MODE}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="check_dispatch_evidence_reliability_w2_contract.py",
        description=(
            "Fail-closed validator for v0.15 M33 lane-B activation-trigger schema/order "
            "and fixture parity contracts for dispatch evidence reliability W2."
        ),
    )
    parser.add_argument(
        "--checker-script",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"dispatch-evidence-reliability-w2-contract: error: {exc}", file=sys.stderr)
        return 2
