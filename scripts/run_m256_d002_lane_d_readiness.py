#!/usr/bin/env python3
"""Run the M256-D002 lane-D readiness chain without recursive npm nesting."""
...
DEPENDENCY_TOKEN = "M256-A001..A003 + M256-B001..B004 + M256-C001..C003 + M256-D001..D002"
...
COMMAND_CHAIN: tuple[Sequence[str], ...] = (
    ...
    (
        sys.executable,
        "scripts/check_m256_d001_class_realization_runtime_contract_and_architecture_freeze.py",
    ),
    (
        sys.executable,
        "-m",
        "pytest",
        "tests/tooling/test_check_m256_d001_class_realization_runtime_contract_and_architecture_freeze.py",
        "-q",
    ),
    (
        sys.executable,
        "scripts/check_m256_d002_metaclass_graph_and_root_class_baseline_core_feature_implementation.py",
    ),
    (
        sys.executable,
        "-m",
        "pytest",
        "tests/tooling/test_check_m256_d002_metaclass_graph_and_root_class_baseline_core_feature_implementation.py",
        "-q",
    ),
)
...
def run_chain() -> int:
    print(
        ...
        f"{DEPENDENCY_TOKEN} (source closure, object-model sema, executable lowering, runtime freeze, and realized graph baseline must stay aligned without recursive issue runners)"
    )
    ...
    print("[ok] M256-D002 lane-D readiness chain completed")
    return 0
