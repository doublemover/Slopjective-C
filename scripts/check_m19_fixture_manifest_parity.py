#!/usr/bin/env python3
"""Fail-closed M19 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M19-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M19-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2324, last_issue=2338),
    LaneSpec(lane="B", bucket="semantic", first_issue=2339, last_issue=2352),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2353, last_issue=2366),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2367, last_issue=2380),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M19 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
