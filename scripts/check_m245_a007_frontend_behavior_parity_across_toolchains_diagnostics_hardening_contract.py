#!/usr/bin/env python3
"""Fail-closed checker for M245-A007 frontend behavior parity diagnostics hardening."""
...
MODE = "m245-a007-frontend-behavior-parity-toolchains-diagnostics-hardening-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m245_frontend_behavior_parity_across_toolchains_diagnostics_hardening_a007_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    ...
    / "compiler"
    / "m245"
    / "m245_a007_frontend_behavior_parity_across_toolchains_diagnostics_hardening_packet.md"
)
DEFAULT_A006_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m245_frontend_behavior_parity_across_toolchains_edge_case_expansion_and_robustness_a006_expectations.md"
)
DEFAULT_A006_PACKET_DOC = (
    ...
    / "compiler"
    / "m245"
    / "m245_a006_frontend_behavior_parity_across_toolchains_edge_case_expansion_and_robustness_packet.md"
)
DEFAULT_A006_CHECKER = (
    ROOT / "scripts" / "check_m245_a006_frontend_behavior_parity_across_toolchains_edge_case_expansion_and_robustness_contract.py"
)
DEFAULT_A006_TEST = (
    ROOT
    / "tests"
    / "tooling"
    / "test_check_m245_a006_frontend_behavior_parity_across_toolchains_edge_case_expansion_and_robustness_contract.py"
)
...
PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M245-A007-DOC-PKT-01",
        "# M245-A007 Frontend Behavior Parity Across Toolchains Diagnostics Hardening Packet",
    ),
    SnippetCheck("M245-A007-DOC-PKT-02", "Packet: `M245-A007`"),
    SnippetCheck("M245-A007-DOC-PKT-03", "Dependencies: `M245-A006`"),
    SnippetCheck(
        "M245-A007-DOC-PKT-04",
        "code/spec anchors and milestone optimization improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M245-A007-DOC-PKT-05",
        "scripts/check_m245_a007_frontend_behavior_parity_across_toolchains_diagnostics_hardening_contract.py",
    ),
    SnippetCheck(
        "M245-A007-DOC-PKT-06",
        "tests/tooling/test_check_m245_a007_frontend_behavior_parity_across_toolchains_diagnostics_hardening_contract.py",
    ),
    SnippetCheck("M245-A007-DOC-PKT-07", "`npm run check:objc3c:m245-a007-lane-a-readiness`"),
)

A006_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M245-A007-A006-DOC-01",
        "# M245 Frontend Behavior Parity Across Toolchains Edge-Case Expansion and Robustness Expectations (A006)",
    ),
    SnippetCheck(
        "M245-A007-A006-DOC-02",
...
A006_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M245-A007-A006-PKT-01", "Packet: `M245-A006`"),
    SnippetCheck("M245-A007-A006-PKT-02", "Dependencies: `M245-A005`"),
)
...
METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M245-A007-META-02",
        "explicit `M245-A006` dependency continuity so diagnostics hardening drift fails closed",
    ),
)
...
PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        ...
        '&& npm run check:objc3c:m245-a007-frontend-behavior-parity-toolchains-diagnostics-hardening-contract '
        '&& npm run test:tooling:m245-a007-frontend-behavior-parity-toolchains-diagnostics-hardening-contract"',
    ),
    SnippetCheck("M245-A007-PKG-04", '"test:objc3c:parser-replay-proof": '),
    SnippetCheck("M245-A007-PKG-05", '"test:objc3c:parser-ast-extraction": '),
)
...
def run(argv: Sequence[str]) -> int:
    ...
    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M245-A007-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M245-A007-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.a006_expectations_doc, "M245-A007-A006-DOC-EXISTS", A006_EXPECTATIONS_SNIPPETS),
        (args.a006_packet_doc, "M245-A007-A006-PKT-EXISTS", A006_PACKET_SNIPPETS),
        (args.architecture_doc, "M245-A007-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M245-A007-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M245-A007-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M245-A007-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
    ...
    for path, check_id in (
        (args.a006_checker, "M245-A007-DEP-A006-ARG-01"),
        (args.a006_test, "M245-A007-DEP-A006-ARG-02"),
    ):
        checks_total += 1
        if not path.exists():
            failures.append(
