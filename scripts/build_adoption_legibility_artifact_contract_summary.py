#!/usr/bin/env python3
"""Build the adoption and legibility artifact contract summary."""
...
CONTRACT_PATH = ROOT / "tests" / "tooling" / "fixtures" / "adoption_legibility" / "artifact_contract.json"
...
def main() -> int:
    ...
    for artifact in generated_artifacts if isinstance(generated_artifacts, list) else []:
        ...
        expect(str(artifact.get("path", "")).startswith(str(contract["artifact_root"])), f"{artifact_id} path outside artifact root", failures)
        expect(artifact.get("schema") == contract["schema"], f"{artifact_id} schema drifted", failures)
    expect(
        {"adoption_legibility_evidence", "evaluator_publication"}.issubset(artifact_ids),
        "artifact contract must include evidence and evaluator publication outputs",
        failures,
    )

    payload = {
        "contract_id": "objc3c.adoption_legibility.artifact_contract.summary.v1",
    ...
    print("adoption-legibility-artifact-contract: PASS" if not failures else "adoption-legibility-artifact-contract: FAIL")
