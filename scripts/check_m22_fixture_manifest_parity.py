#!/usr/bin/env python3
"""Fail-closed M22 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M22-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M22-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2540, last_issue=2554),
    LaneSpec(lane="B", bucket="semantic", first_issue=2555, last_issue=2568),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2569, last_issue=2582),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2583, last_issue=2596),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M22 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
