#!/usr/bin/env python3
"""Fail-closed checker for M248-D010 runner/platform operations conformance corpus expansion contract."""
...
MODE = "m248-d010-runner-reliability-platform-operations-conformance-corpus-expansion-contract-v1"
...
PREREQUISITE_ASSETS: tuple[AssetCheck, ...] = (
    ...
    AssetCheck(
        ...
        Path(
            ...
            "m248_d009_runner_reliability_and_platform_operations_conformance_matrix_implementation_packet.md"
        ),
    ),
    AssetCheck(
        "M248-D010-D009-03",
    ...
    AssetCheck(
        ...
        Path(
            "tests/tooling/"
            "test_check_m248_d009_runner_reliability_and_platform_operations_conformance_matrix_implementation_contract.py"
        ),
    ),
)
...
EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M248-D010-DOC-EXP-01",
        "Contract ID: `objc3c-runner-reliability-platform-operations-conformance-corpus-expansion/m248-d010-v1`",
    ),
    SnippetCheck("M248-D010-DOC-EXP-02", "Dependencies: `M248-D009`"),
    SnippetCheck(
        "M248-D010-DOC-EXP-03",
        "Issue `#6845` defines canonical lane-D conformance corpus expansion scope.",
    ),
    SnippetCheck("M248-D010-DOC-EXP-04", "`check:objc3c:m248-d010-lane-d-readiness`"),
    SnippetCheck("M248-D010-DOC-EXP-05", "`check:objc3c:m248-d009-lane-d-readiness`"),
    SnippetCheck("M248-D010-DOC-EXP-06", "`test:objc3c:execution-replay-proof`"),
    SnippetCheck(
        "M248-D010-DOC-EXP-07",
        "tmp/reports/m248/M248-D010/runner_reliability_and_platform_operations_conformance_corpus_expansion_contract_summary.json",
    ),
    SnippetCheck(
        "M248-D010-DOC-EXP-08",
        "Code/spec anchors and milestone optimization improvements are mandatory scope inputs.",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M248-D010-DOC-PKT-01", "Packet: `M248-D010`"),
    SnippetCheck("M248-D010-DOC-PKT-02", "Issue: `#6845`"),
    SnippetCheck("M248-D010-DOC-PKT-03", "Dependencies"),
    SnippetCheck("M248-D010-DOC-PKT-04", "`M248-D009`"),
    SnippetCheck(
        "M248-D010-DOC-PKT-05",
        "`scripts/check_m248_d010_runner_reliability_and_platform_operations_conformance_corpus_expansion_contract.py`",
    ),
    SnippetCheck(
        "M248-D010-DOC-PKT-06",
        "`tests/tooling/test_check_m248_d010_runner_reliability_and_platform_operations_conformance_corpus_expansion_contract.py`",
    ),
    SnippetCheck("M248-D010-DOC-PKT-07", "`check:objc3c:m248-d010-lane-d-readiness`"),
    SnippetCheck(
        "M248-D010-DOC-PKT-08",
        "Code/spec anchors and milestone optimization improvements are mandatory scope inputs.",
    ),
)
...
PACKAGE_SCRIPT_KEY_CHECKS: tuple[PackageScriptKeyCheck, ...] = (
    ...
    PackageScriptKeyCheck("M248-D010-CFG-03", "check:objc3c:m248-d010-lane-d-readiness"),
    PackageScriptKeyCheck("M248-D010-CFG-07", "compile:objc3c"),
    PackageScriptKeyCheck("M248-D010-CFG-08", "proof:objc3c"),
    PackageScriptKeyCheck("M248-D010-CFG-09", "test:objc3c:execution-replay-proof"),
    PackageScriptKeyCheck("M248-D010-CFG-10", "test:objc3c:perf-budget"),
)
...
PACKAGE_SCRIPT_CHECKS: tuple[PackageScriptCheck, ...] = (
    ...
    PackageScriptCheck(
        ...
        "npm run check:objc3c:m248-d009-lane-d-readiness && npm run check:objc3c:m248-d010-runner-reliability-platform-operations-conformance-corpus-expansion-contract && npm run test:tooling:m248-d010-runner-reliability-platform-operations-conformance-corpus-expansion-contract",
    ),
)
...
LOWERING_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck("M248-D010-SPC-04", "platform replay and lane-e performance guardrail advances."),
)
...
METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M248-D010-META-02",
        "explicit `M248-D009` dependency continuity and fail-closed conformance corpus evidence continuity",
    ),
)
...
def check_package_contract(path: Path) -> tuple[int, list[Finding]]:
    ...
    findings: list[Finding] = []
    if not path.exists() or not path.is_file():
        findings.append(Finding(artifact=display_path(path), check_id="M248-D010-CFG-00", detail=f"required document is missing: {display_path(path)}"))
        return checks_total, findings
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        findings.append(Finding(artifact=display_path(path), check_id="M248-D010-CFG-00", detail=f"unable to parse package.json: {exc}"))
        return checks_total, findings
    scripts = payload.get("scripts")
    checks_total += 1
    if not isinstance(scripts, dict):
        findings.append(Finding(artifact="package_json", check_id="M248-D010-CFG-00", detail='expected top-level "scripts" object in package.json'))
        return checks_total, findings
...
def run(argv: Sequence[str]) -> int:
    ...
    for artifact_name, path, exists_check_id, snippets in (
        ("expectations_doc", args.expectations_doc, "M248-D010-DOC-EXP-00", EXPECTATIONS_SNIPPETS),
        ("packet_doc", args.packet_doc, "M248-D010-DOC-PKT-00", PACKET_SNIPPETS),
        ("architecture_doc", args.architecture_doc, "M248-D010-ARCH-00", ARCHITECTURE_SNIPPETS),
        ("lowering_spec", args.lowering_spec, "M248-D010-SPC-00", LOWERING_SPEC_SNIPPETS),
        ("metadata_spec", args.metadata_spec, "M248-D010-META-00", METADATA_SPEC_SNIPPETS),
    ):
