#!/usr/bin/env python3
"""Fail-closed M04 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M04-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M04-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(
        ...
        first_issue=1306,
        last_issue=1315,
        required_group_name="m04_lane_a_issue_1306_1315_frontend_scope_freeze",
    ),
    LaneSpec(lane="B", bucket="semantic", first_issue=1316, last_issue=1325),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1326, last_issue=1334),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1335, last_issue=1343),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M04 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
