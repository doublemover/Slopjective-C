...
def validate_blocked_section(section: dict[str, Any], field: str, expectation_state: str) -> None:
    status = require_nonempty_string(section.get("status"), f"{field}.status")
    if expectation_state in BLOCKED_STATES and status not in BLOCKED_STATES:
        raise RuntimeError(f"{field}.status must stay blocked or reserved for {expectation_state}")
...
def validate_expectation(
    ...
    validate_blocked_section(diagnostics, f"{family_id}.diagnostics", expectation_state)
    for index, raw_case in enumerate(require_list(expectation["negative_cases"], f"{family_id}.negative_cases")):
        ...
        validate_blocked_section(case, f"{family_id}.negative_cases[{index}]", expectation_state)
    ...
    for section_name in (
        ...
        validate_blocked_section(section, f"{family_id}.{section_name}", expectation_state)
