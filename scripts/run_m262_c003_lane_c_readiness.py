#!/usr/bin/env python3
"""Run the focused M262-C003 lane-C readiness chain."""
...
DEPENDENCY_TOKEN = "M262-A001..A002 + M262-B001..B003 + M262-C001..C002"
...
COMMAND_CHAIN: tuple[Sequence[str], ...] = (
    ...
    (
        ...
        "fast",
        "--reason",
        "m262-c003-lane-c-readiness",
        "--summary-out",
        "tmp/reports/m262/M262-C003/ensure_objc3c_native_build_summary.json",
    ),
    ...
    (sys.executable, "scripts/check_m262_c003_cleanup_emission_weak_load_store_lowering_and_lifetime_extension_hooks_core_feature_implementation.py"),
    (sys.executable, "-m", "pytest", "tests/tooling/test_check_m262_c003_cleanup_emission_weak_load_store_lowering_and_lifetime_extension_hooks_core_feature_implementation.py", "-q"),
)


def run_chain() -> int:
    print(f"[info] dependency continuity token: {DEPENDENCY_TOKEN} (lane C now proves supported ARC scope cleanup, weak current-property hooks, and block-capture lifetime cleanup above the retained C002 helper-insertion baseline)")
    ...
    print("[ok] M262-C003 lane-C readiness chain completed")
    return 0
