#!/usr/bin/env python3
"""Fail-closed checker for M249-E010 release gate/docs/runbooks conformance corpus expansion."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m249-e010-lane-e-release-gate-docs-runbooks-conformance-corpus-expansion-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m249_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_e010_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m249"
    / "m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_packet.md"
)
DEFAULT_RUNNER_SCRIPT = ROOT / "scripts" / "run_m249_e010_lane_e_readiness.py"
DEFAULT_ARCHITECTURE_DOC = ROOT / "native" / "objc3c" / "src" / "ARCHITECTURE.md"
DEFAULT_LOWERING_SPEC = ROOT / "spec" / "LOWERING_AND_RUNTIME_CONTRACTS.md"
DEFAULT_METADATA_SPEC = ROOT / "spec" / "MODULE_METADATA_AND_ABI_TABLES.md"
DEFAULT_PACKAGE_JSON = ROOT / "package.json"
DEFAULT_SUMMARY_OUT = Path(
    "tmp/reports/m249/M249-E010/lane_e_release_gate_docs_runbooks_conformance_corpus_expansion_summary.json"
)


@dataclass(frozen=True)
class AssetCheck:
    check_id: str
    relative_path: Path


@dataclass(frozen=True)
class SnippetCheck:
    check_id: str
    snippet: str


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


PREREQUISITE_ASSETS: tuple[AssetCheck, ...] = (
    AssetCheck(
        "M249-E010-DEP-E009-01",
        Path(
            "docs/contracts/m249_lane_e_release_gate_docs_and_runbooks_conformance_matrix_implementation_e009_expectations.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-E009-02",
        Path(
            "spec/planning/compiler/m249/m249_e009_lane_e_release_gate_docs_and_runbooks_conformance_matrix_implementation_packet.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-E009-03",
        Path(
            "scripts/check_m249_e009_lane_e_release_gate_docs_and_runbooks_conformance_matrix_implementation_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-E009-04",
        Path(
            "tests/tooling/test_check_m249_e009_lane_e_release_gate_docs_and_runbooks_conformance_matrix_implementation_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-A009-01",
        Path(
            "docs/contracts/m249_feature_packaging_surface_and_compatibility_contracts_integration_closeout_and_gate_signoff_a009_expectations.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-A009-02",
        Path(
            "spec/planning/compiler/m249/m249_a009_feature_packaging_surface_and_compatibility_contracts_integration_closeout_and_gate_signoff_packet.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-A009-03",
        Path(
            "scripts/check_m249_a009_feature_packaging_surface_and_compatibility_contracts_integration_closeout_and_gate_signoff_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-A009-04",
        Path(
            "tests/tooling/test_check_m249_a009_feature_packaging_surface_and_compatibility_contracts_integration_closeout_and_gate_signoff_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-B010-01",
        Path(
            "docs/contracts/m249_semantic_compatibility_and_migration_checks_conformance_corpus_expansion_b010_expectations.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-B010-02",
        Path(
            "spec/planning/compiler/m249/m249_b010_semantic_compatibility_and_migration_checks_conformance_corpus_expansion_packet.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-B010-03",
        Path(
            "scripts/check_m249_b010_semantic_compatibility_and_migration_checks_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-B010-04",
        Path(
            "tests/tooling/test_check_m249_b010_semantic_compatibility_and_migration_checks_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-C010-01",
        Path(
            "docs/contracts/m249_ir_object_packaging_and_symbol_policy_conformance_corpus_expansion_c010_expectations.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-C010-02",
        Path(
            "spec/planning/compiler/m249/m249_c010_ir_object_packaging_and_symbol_policy_conformance_corpus_expansion_packet.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-C010-03",
        Path(
            "scripts/check_m249_c010_ir_object_packaging_and_symbol_policy_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-C010-04",
        Path(
            "tests/tooling/test_check_m249_c010_ir_object_packaging_and_symbol_policy_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-D010-01",
        Path(
            "docs/contracts/m249_installer_runtime_operations_and_support_tooling_conformance_corpus_expansion_d010_expectations.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-D010-02",
        Path(
            "spec/planning/compiler/m249/m249_d010_installer_runtime_operations_and_support_tooling_conformance_corpus_expansion_packet.md"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-D010-03",
        Path(
            "scripts/check_m249_d010_installer_runtime_operations_and_support_tooling_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-D010-04",
        Path(
            "tests/tooling/test_check_m249_d010_installer_runtime_operations_and_support_tooling_conformance_corpus_expansion_contract.py"
        ),
    ),
    AssetCheck(
        "M249-E010-DEP-D010-05",
        Path("scripts/run_m249_d010_lane_d_readiness.py"),
    ),
)

EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-DOC-EXP-01",
        "# M249 Lane E Release Gate, Docs, and Runbooks Conformance Corpus Expansion Expectations (E010)",
    ),
    SnippetCheck(
        "M249-E010-DOC-EXP-02",
        "Contract ID: `objc3c-lane-e-release-gate-docs-runbooks-conformance-corpus-expansion/m249-e010-v1`",
    ),
    SnippetCheck("M249-E010-DOC-EXP-03", "- Issue: `#6957`"),
    SnippetCheck("M249-E010-DOC-EXP-04", "`M249-E009`"),
    SnippetCheck("M249-E010-DOC-EXP-05", "`M249-A009`"),
    SnippetCheck("M249-E010-DOC-EXP-06", "`M249-B010`"),
    SnippetCheck(
        "M249-E010-DOC-EXP-07",
        "conformance corpus expansion dependency",
    ),
    SnippetCheck(
        "M249-E010-DOC-EXP-08",
        "Dependency token `M249-C010` is mandatory for lane-C conformance corpus expansion readiness chaining.",
    ),
    SnippetCheck("M249-E010-DOC-EXP-09", "`M249-D010`"),
    SnippetCheck(
        "M249-E010-DOC-EXP-10",
        "`scripts/run_m249_e010_lane_e_readiness.py`",
    ),
    SnippetCheck(
        "M249-E010-DOC-EXP-11",
        "`scripts/check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py`",
    ),
    SnippetCheck(
        "M249-E010-DOC-EXP-12",
        "`tests/tooling/test_check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py`",
    ),
    SnippetCheck(
        "M249-E010-DOC-EXP-13",
        "`tmp/reports/m249/M249-E010/lane_e_release_gate_docs_runbooks_conformance_corpus_expansion_summary.json`",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-DOC-PKT-01",
        "# M249-E010 Lane-E Release Gate, Docs, and Runbooks Conformance Corpus Expansion Packet",
    ),
    SnippetCheck("M249-E010-DOC-PKT-02", "Packet: `M249-E010`"),
    SnippetCheck("M249-E010-DOC-PKT-03", "Issue: `#6957`"),
    SnippetCheck(
        "M249-E010-DOC-PKT-04",
        "Dependencies: `M249-E009`, `M249-A009`, `M249-B010`, `M249-C010`, `M249-D010`",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-05",
        "docs/contracts/m249_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_e010_expectations.md",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-06",
        "scripts/check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-07",
        "tests/tooling/test_check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-08",
        "scripts/run_m249_e010_lane_e_readiness.py",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-09",
        "scripts/run_m249_e009_lane_e_readiness.py",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-10",
        "check:objc3c:m249-b010-lane-b-readiness",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-11",
        "scripts/run_m249_d010_lane_d_readiness.py",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-12",
        "milestone optimization improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M249-E010-DOC-PKT-13",
        "tmp/reports/m249/M249-E010/lane_e_release_gate_docs_runbooks_conformance_corpus_expansion_summary.json",
    ),
)

RUNNER_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M249-E010-RUN-01", '"""Run M249-E010 lane-E readiness checks without deep npm nesting."""'),
    SnippetCheck("M249-E010-RUN-02", "scripts/run_m249_e009_lane_e_readiness.py"),
    SnippetCheck("M249-E010-RUN-03", "check:objc3c:m249-a009-lane-a-readiness"),
    SnippetCheck("M249-E010-RUN-04", "check:objc3c:m249-b010-lane-b-readiness"),
    SnippetCheck("M249-E010-RUN-05", "check:objc3c:m249-c010-lane-c-readiness"),
    SnippetCheck("M249-E010-RUN-06", "scripts/run_m249_d010_lane_d_readiness.py"),
    SnippetCheck(
        "M249-E010-RUN-07",
        "scripts/check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py",
    ),
    SnippetCheck(
        "M249-E010-RUN-08",
        "tests/tooling/test_check_m249_e010_lane_e_release_gate_docs_and_runbooks_conformance_corpus_expansion_contract.py",
    ),
    SnippetCheck("M249-E010-RUN-09", "[ok] M249-E010 lane-E readiness chain completed"),
)

ARCHITECTURE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-ARCH-01",
        "M249 lane-E E003 release gate/docs/runbooks core feature implementation",
    ),
    SnippetCheck(
        "M249-E010-ARCH-02",
        "`M249-E002`, `M249-A003`, `M249-B003`,",
    ),
    SnippetCheck(
        "M249-E010-ARCH-03",
        "`M249-C003`, and `M249-D003`",
    ),
)

LOWERING_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-SPC-01",
        "release gate/docs/runbooks core feature implementation wiring shall preserve",
    ),
    SnippetCheck(
        "M249-E010-SPC-02",
        "`M249-E002`, `M249-A003`, `M249-B003`,",
    ),
    SnippetCheck(
        "M249-E010-SPC-03",
        "`M249-C003`, and `M249-D003`",
    ),
)

METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-META-01",
        "deterministic lane-E release gate/docs/runbooks core feature implementation dependency anchors for",
    ),
    SnippetCheck(
        "M249-E010-META-02",
        "`M249-E002`, `M249-A003`, `M249-B003`, `M249-C003`, and `M249-D003`",
    ),
)

PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M249-E010-PKG-01",
        '"check:objc3c:m249-e009-lane-e-readiness": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-02",
        '"check:objc3c:m249-a009-lane-a-readiness": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-03",
        '"check:objc3c:m249-b010-lane-b-readiness": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-04",
        '"check:objc3c:m249-c010-lane-c-readiness": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-05",
        '"check:objc3c:m249-d010-lane-d-readiness": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-06",
        '"check:objc3c:m249-e010-lane-e-release-gate-docs-runbooks-conformance-corpus-expansion-contract": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-07",
        '"test:tooling:m249-e010-lane-e-release-gate-docs-runbooks-conformance-corpus-expansion-contract": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-08",
        '"check:objc3c:m249-e010-lane-e-readiness": "python scripts/run_m249_e010_lane_e_readiness.py"',
    ),
    SnippetCheck(
        "M249-E010-PKG-09",
        '"test:objc3c:parser-replay-proof": ',
    ),
    SnippetCheck(
        "M249-E010-PKG-10",
        '"test:objc3c:parser-ast-extraction": ',
    ),
)


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def display_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        return resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return resolved.as_posix()


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expectations-doc", type=Path, default=DEFAULT_EXPECTATIONS_DOC)
    parser.add_argument("--packet-doc", type=Path, default=DEFAULT_PACKET_DOC)
    parser.add_argument("--runner-script", type=Path, default=DEFAULT_RUNNER_SCRIPT)
    parser.add_argument("--architecture-doc", type=Path, default=DEFAULT_ARCHITECTURE_DOC)
    parser.add_argument("--lowering-spec", type=Path, default=DEFAULT_LOWERING_SPEC)
    parser.add_argument("--metadata-spec", type=Path, default=DEFAULT_METADATA_SPEC)
    parser.add_argument("--package-json", type=Path, default=DEFAULT_PACKAGE_JSON)
    parser.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY_OUT)
    return parser.parse_args(argv)


def check_prerequisite_assets() -> tuple[int, list[Finding]]:
    checks_total = 0
    findings: list[Finding] = []
    for asset in PREREQUISITE_ASSETS:
        checks_total += 1
        absolute = ROOT / asset.relative_path
        if not absolute.exists():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"missing prerequisite asset: {asset.relative_path.as_posix()}",
                )
            )
            continue
        if not absolute.is_file():
            findings.append(
                Finding(
                    artifact=asset.relative_path.as_posix(),
                    check_id=asset.check_id,
                    detail=f"prerequisite path is not a file: {asset.relative_path.as_posix()}",
                )
            )
    return checks_total, findings


def check_doc_contract(
    *,
    path: Path,
    exists_check_id: str,
    snippets: tuple[SnippetCheck, ...],
) -> tuple[int, list[Finding]]:
    checks_total = 1
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                display_path(path),
                exists_check_id,
                f"required document is missing: {display_path(path)}",
            )
        )
        return checks_total, findings
    if not path.is_file():
        findings.append(
            Finding(
                display_path(path),
                exists_check_id,
                f"required path is not a file: {display_path(path)}",
            )
        )
        return checks_total, findings

    text = path.read_text(encoding="utf-8")
    for snippet in snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(
                Finding(
                    display_path(path),
                    snippet.check_id,
                    f"missing required snippet: {snippet.snippet}",
                )
            )
    return checks_total, findings


def finding_sort_key(finding: Finding) -> tuple[str, str, str]:
    return (finding.artifact, finding.check_id, finding.detail)


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    checks_total = 0
    failures: list[Finding] = []

    dep_checks, dep_failures = check_prerequisite_assets()
    checks_total += dep_checks
    failures.extend(dep_failures)

    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M249-E010-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M249-E010-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.runner_script, "M249-E010-RUN-EXISTS", RUNNER_SNIPPETS),
        (args.architecture_doc, "M249-E010-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M249-E010-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M249-E010-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M249-E010-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
        count, findings = check_doc_contract(
            path=path,
            exists_check_id=exists_check_id,
            snippets=snippets,
        )
        checks_total += count
        failures.extend(findings)

    failures = sorted(failures, key=finding_sort_key)
    checks_passed = checks_total - len(failures)
    summary_payload = {
        "mode": MODE,
        "ok": not failures,
        "checks_total": checks_total,
        "checks_passed": checks_passed,
        "failures": [
            {"artifact": f.artifact, "check_id": f.check_id, "detail": f.detail}
            for f in failures
        ],
    }

    summary_path = args.summary_out if args.summary_out.is_absolute() else ROOT / args.summary_out
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary_path.write_text(canonical_json(summary_payload), encoding="utf-8")

    if failures:
        for finding in failures:
            print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
        return 1
    print(f"[ok] {MODE}: {checks_passed}/{checks_total} checks passed")
    return 0


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))
