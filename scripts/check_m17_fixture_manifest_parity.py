#!/usr/bin/env python3
"""Fail-closed M17 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M17-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M17-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    ...
    LaneSpec(lane="B", bucket="semantic", first_issue=2044, last_issue=2095),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2096, last_issue=2147),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2148, last_issue=2199),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M17 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
