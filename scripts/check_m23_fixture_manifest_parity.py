#!/usr/bin/env python3
"""Fail-closed M23 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M23-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M23-{self.lane}\d{{3}}\.json$")


LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(lane="A", bucket="parser", first_issue=2612, last_issue=2626),
    LaneSpec(lane="B", bucket="semantic", first_issue=2627, last_issue=2640),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=2641, last_issue=2654),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=2655, last_issue=2668),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M23 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
