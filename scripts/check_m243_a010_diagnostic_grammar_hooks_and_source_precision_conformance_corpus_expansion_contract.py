#!/usr/bin/env python3
"""Fail-closed checker for M243-A010 diagnostic grammar-hooks conformance corpus expansion."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = (
    "m243-a010-diagnostic-grammar-hooks-and-source-precision-"
    "conformance-corpus-expansion-contract-v1"
)
FIXTURE_ROOT = (
    ROOT / "tests" / "tooling" / "fixtures" / "m243_a010_diagnostic_grammar_hooks_conformance_corpus"
)

ARTIFACTS: dict[str, Path] = {
    "a009_contract_doc": ROOT
    / "docs"
    / "contracts"
    / "m243_diagnostic_grammar_hooks_and_source_precision_a009_expectations.md",
    "contract_doc": ROOT
    / "docs"
    / "contracts"
    / "m243_diagnostic_grammar_hooks_and_source_precision_a010_expectations.md",
    "packet_doc": ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m243"
    / "m243_a010_diagnostic_grammar_hooks_and_source_precision_conformance_corpus_expansion_packet.md",
    "frontend_types": ROOT
    / "native"
    / "objc3c"
    / "src"
    / "pipeline"
    / "objc3_frontend_types.h",
    "readiness_surface": ROOT
    / "native"
    / "objc3c"
    / "src"
    / "pipeline"
    / "objc3_parse_lowering_readiness_surface.h",
    "frontend_artifacts": ROOT
    / "native"
    / "objc3c"
    / "src"
    / "pipeline"
    / "objc3_frontend_artifacts.cpp",
    "architecture_doc": ROOT / "native" / "objc3c" / "src" / "ARCHITECTURE.md",
    "package_json": ROOT / "package.json",
    "fixture_manifest": FIXTURE_ROOT / "manifest.json",
}

REQUIRED_SNIPPETS: dict[str, tuple[tuple[str, str], ...]] = {
    "a009_contract_doc": (
        (
            "M243-A010-DEP-01",
            "Contract ID: `objc3c-diagnostic-grammar-hooks-and-source-precision-conformance-matrix-implementation/m243-a009-v1`",
        ),
    ),
    "contract_doc": (
        (
            "M243-A010-DOC-01",
            "Contract ID: `objc3c-diagnostic-grammar-hooks-and-source-precision-conformance-corpus-expansion/m243-a010-v1`",
        ),
        (
            "M243-A010-DOC-02",
            "parser_diagnostic_grammar_hooks_conformance_corpus_consistent",
        ),
        (
            "M243-A010-DOC-03",
            "parser_diagnostic_grammar_hooks_conformance_corpus_ready",
        ),
        (
            "M243-A010-DOC-04",
            "Code/spec anchors and milestone optimization improvements are mandatory scope inputs.",
        ),
        ("M243-A010-DOC-05", "Dependencies: `M243-A009`"),
        (
            "M243-A010-DOC-06",
            "tests/tooling/fixtures/m243_a010_diagnostic_grammar_hooks_conformance_corpus/manifest.json",
        ),
        (
            "M243-A010-DOC-07",
            "npm run check:objc3c:m243-a010-lane-a-readiness",
        ),
    ),
    "packet_doc": (
        ("M243-A010-PKT-01", "Packet: `M243-A010`"),
        ("M243-A010-PKT-02", "Dependencies: `M243-A009`"),
        (
            "M243-A010-PKT-03",
            "scripts/check_m243_a010_diagnostic_grammar_hooks_and_source_precision_conformance_corpus_expansion_contract.py",
        ),
        (
            "M243-A010-PKT-04",
            "tests/tooling/fixtures/m243_a010_diagnostic_grammar_hooks_conformance_corpus/manifest.json",
        ),
    ),
    "frontend_types": (
        (
            "M243-A010-TYP-01",
            "bool parser_diagnostic_grammar_hooks_conformance_corpus_consistent = false;",
        ),
        (
            "M243-A010-TYP-02",
            "bool parser_diagnostic_grammar_hooks_conformance_corpus_ready = false;",
        ),
        (
            "M243-A010-TYP-03",
            "std::string parser_diagnostic_grammar_hooks_conformance_corpus_key;",
        ),
    ),
    "readiness_surface": (
        ("M243-A010-RDY-01", "BuildObjc3DiagnosticGrammarHooksConformanceCorpusKey("),
        (
            "M243-A010-RDY-02",
            "surface.parser_diagnostic_grammar_hooks_conformance_corpus_consistent =",
        ),
        (
            "M243-A010-RDY-03",
            "surface.parser_diagnostic_grammar_hooks_conformance_corpus_key =",
        ),
        (
            "M243-A010-RDY-04",
            "surface.parser_diagnostic_grammar_hooks_conformance_corpus_ready =",
        ),
        (
            "M243-A010-RDY-05",
            "surface.parser_diagnostic_grammar_hooks_conformance_corpus_ready &&",
        ),
        (
            "M243-A010-RDY-06",
            "parser diagnostic grammar hooks conformance corpus is inconsistent",
        ),
        (
            "M243-A010-RDY-07",
            "parser diagnostic grammar hooks conformance corpus is not ready",
        ),
    ),
    "frontend_artifacts": (
        (
            "M243-A010-ART-01",
            "parser_diagnostic_grammar_hooks_conformance_corpus_consistent",
        ),
        (
            "M243-A010-ART-02",
            "parser_diagnostic_grammar_hooks_conformance_corpus_ready",
        ),
        (
            "M243-A010-ART-03",
            "parser_diagnostic_grammar_hooks_conformance_corpus_key",
        ),
    ),
    "architecture_doc": (
        (
            "M243-A010-ARC-01",
            "M243 lane-A A010 conformance corpus expansion anchors parser diagnostic",
        ),
    ),
    "package_json": (
        (
            "M243-A010-CFG-01",
            '"check:objc3c:m243-a010-diagnostic-grammar-hooks-and-source-precision-conformance-corpus-expansion-contract"',
        ),
        (
            "M243-A010-CFG-02",
            '"test:tooling:m243-a010-diagnostic-grammar-hooks-and-source-precision-conformance-corpus-expansion-contract"',
        ),
        (
            "M243-A010-CFG-03",
            '"check:objc3c:m243-a010-lane-a-readiness"',
        ),
        (
            "M243-A010-CFG-04",
            '"check:objc3c:m243-a010-lane-a-readiness": "npm run check:objc3c:m243-a009-lane-a-readiness',
        ),
    ),
}

FORBIDDEN_SNIPPETS: dict[str, tuple[tuple[str, str], ...]] = {
    "readiness_surface": (
        (
            "M243-A010-FORB-01",
            "surface.parser_diagnostic_grammar_hooks_conformance_corpus_ready = true;",
        ),
    ),
}

REQUIRED_CASES: tuple[tuple[str, str, str | None], ...] = (
    ("A010-C001", "accept_deterministic_grammar_hook_surface.objc3", None),
    ("A010-C002", "reject_actor_nonreentrant_modifier.objc3", "OBJC3-E-ACTOR-NONREENTRANT-DEFERRED"),
    ("A010-C003", "reject_trailing_comma_parameter.objc3", "O3P104"),
    ("A010-C004", "reject_nondeclaration_after_comma.objc3", "O3P100"),
    ("A010-C005", "reject_non_objc_pointer_parameter.objc3", "O3P114"),
)


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--summary-out",
        type=Path,
        default=Path(
            "tmp/reports/m243/M243-A010/"
            "diagnostic_grammar_hooks_and_source_precision_conformance_corpus_expansion_contract_summary.json"
        ),
    )
    parser.add_argument(
        "--emit-json",
        action="store_true",
        help="Emit the summary JSON to stdout in addition to writing --summary-out.",
    )
    return parser.parse_args(argv)


def load_text(path: Path, *, artifact: str) -> str:
    if not path.exists() or not path.is_file():
        raise ValueError(f"{artifact} missing file: {path.as_posix()}")
    return path.read_text(encoding="utf-8")


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    findings: list[Finding] = []
    total_checks = 0
    passed_checks = 0

    for artifact, path in ARTIFACTS.items():
        text = load_text(path, artifact=artifact)
        for check_id, snippet in REQUIRED_SNIPPETS.get(artifact, ()):
            total_checks += 1
            if snippet in text:
                passed_checks += 1
            else:
                findings.append(Finding(artifact, check_id, f"expected snippet missing: {snippet}"))
        for check_id, snippet in FORBIDDEN_SNIPPETS.get(artifact, ()):
            total_checks += 1
            if snippet in text:
                findings.append(Finding(artifact, check_id, f"forbidden snippet present: {snippet}"))
            else:
                passed_checks += 1

    manifest_text = load_text(ARTIFACTS["fixture_manifest"], artifact="fixture_manifest")
    try:
        manifest = json.loads(manifest_text)
    except json.JSONDecodeError as exc:
        findings.append(Finding("fixture_manifest", "M243-A010-FIX-01", f"invalid JSON: {exc}"))
        manifest = {}

    total_checks += 1
    if (
        manifest.get("contract_id")
        == "objc3c-diagnostic-grammar-hooks-and-source-precision-conformance-corpus-expansion/m243-a010-v1"
    ):
        passed_checks += 1
    else:
        findings.append(Finding("fixture_manifest", "M243-A010-FIX-02", "contract_id mismatch"))

    cases = {entry.get("id"): entry for entry in manifest.get("cases", []) if isinstance(entry, dict)}
    for case_id, path_name, diagnostic_code in REQUIRED_CASES:
        total_checks += 1
        entry = cases.get(case_id)
        if entry is None:
            findings.append(Finding("fixture_manifest", f"M243-A010-FIX-{case_id}", f"missing case {case_id}"))
            continue
        if entry.get("path") != path_name or entry.get("diagnostic_code") != diagnostic_code:
            findings.append(
                Finding(
                    "fixture_manifest",
                    f"M243-A010-FIX-{case_id}",
                    f"unexpected mapping for {case_id}: {entry}",
                )
            )
            continue
        case_path = FIXTURE_ROOT / path_name
        total_checks += 1
        if case_path.exists() and case_path.is_file() and case_path.read_text(encoding="utf-8").strip():
            passed_checks += 2
        else:
            findings.append(
                Finding("fixture", f"M243-A010-FIX-CONTENT-{case_id}", f"missing or empty {path_name}")
            )

    ok = not findings
    summary = {
        "mode": MODE,
        "ok": ok,
        "checks_total": total_checks,
        "checks_passed": passed_checks,
        "failures": [
            {"artifact": finding.artifact, "check_id": finding.check_id, "detail": finding.detail}
            for finding in findings
        ],
    }
    args.summary_out.parent.mkdir(parents=True, exist_ok=True)
    args.summary_out.write_text(canonical_json(summary), encoding="utf-8")

    if args.emit_json:
        print(canonical_json(summary), end="")

    if ok:
        return 0
    for finding in findings:
        print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
    return 1


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))
