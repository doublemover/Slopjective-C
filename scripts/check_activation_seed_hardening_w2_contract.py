#!/usr/bin/env python3
"""Fail-closed contract validator for v0.15 M25 lane-B activation trigger tooling."""
...
MODE = "activation-seed-hardening-w2-contract-v1"
...
BASE_FAIL_CLOSED_SCENARIOS: tuple[tuple[str, str], ...] = (
    ("W2C-10", "drift_open_blockers_unsorted"),
    ("W2C-11", "hard_fail_invalid_issues_json"),
    ("W2C-12", "drift_open_blockers_line_alias_mismatch"),
    ("W2C-13", "drift_t4_overlay_conflicting_alias"),
    ("W2C-14", "drift_open_blockers_row_key_order"),
    ("W2C-15", "drift_open_blockers_root_key_order"),
    ("W2C-16", "drift_t4_overlay_unknown_field"),
    ("W2C-21", "drift_issues_root_key_order"),
    ("W2C-22", "drift_issues_unknown_field"),
    ("W2C-23", "drift_milestones_metadata_pair_mismatch"),
    ("W2C-24", "drift_catalog_root_key_order"),
    ("W2C-25", "drift_catalog_unknown_field"),
    ("W2C-26", "drift_catalog_task_count_mismatch"),
    ("W2C-27", "drift_catalog_unsorted_task_ids"),
    ("W2C-28", "drift_catalog_status_alias_mismatch"),
    ("W2C-29", "hard_fail_catalog_missing_status"),
    ("W2C-30", "drift_milestones_root_key_order"),
    ("W2C-31", "drift_milestones_unknown_field"),
    ("W2C-32", "drift_milestones_row_key_order"),
    ("W2C-33", "drift_milestones_row_unknown_field"),
    ("W2C-34", "drift_milestones_unsorted_numbers"),
    ("W2C-35", "drift_milestones_duplicate_number"),
    ("W2C-36", "hard_fail_milestones_title_whitespace"),
)
...
def discover_fail_closed_scenarios(*, fixture_root: Path) -> list[tuple[str, str]]:
    for _, scenario_name in BASE_FAIL_CLOSED_SCENARIOS:
    ...
    return list(BASE_FAIL_CLOSED_SCENARIOS)
...
def collect_findings(
    *,
    ...
    fail_closed_scenarios: Sequence[tuple[str, str]],
) -> list[DriftFinding]:
    findings: list[DriftFinding] = []
    ...
    for check_id, scenario_name in fail_closed_scenarios:
        scenario_root = fixture_root / scenario_name
...
def render_drift_report(*, findings: list[DriftFinding], checker_script: Path, fixture_root: Path) -> str:
    rerun_command = render_command(
        [
            "python",
            "scripts/check_activation_seed_hardening_w2_contract.py",
            "--checker-script",
    ...
    lines = [
        "activation-seed-hardening-w2-contract: contract drift detected "
...
def check_contract(*, checker_script: Path, fixture_root: Path) -> int:
    ...
    print("activation-seed-hardening-w2-contract: OK")
    print(f"- mode={MODE}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="check_activation_seed_hardening_w2_contract.py",
        description=(
            "Fail-closed validator for v0.15 M25 lane-B activation-trigger schema/order "
            "and fixture parity contracts."
        ),
    )
    parser.add_argument(
        "--checker-script",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"activation-seed-hardening-w2-contract: error: {exc}", file=sys.stderr)
        return 2
