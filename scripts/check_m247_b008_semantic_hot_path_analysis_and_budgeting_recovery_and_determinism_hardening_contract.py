#!/usr/bin/env python3
"""Fail-closed checker for M247-B008 semantic hot-path recovery/determinism hardening."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m247-b008-semantic-hot-path-analysis-budgeting-recovery-and-determinism-hardening-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m247_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_b008_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m247"
    / "m247_b008_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_packet.md"
)
DEFAULT_ARCHITECTURE_DOC = ROOT / "native" / "objc3c" / "src" / "ARCHITECTURE.md"
DEFAULT_LOWERING_SPEC = ROOT / "spec" / "LOWERING_AND_RUNTIME_CONTRACTS.md"
DEFAULT_METADATA_SPEC = ROOT / "spec" / "MODULE_METADATA_AND_ABI_TABLES.md"
DEFAULT_PACKAGE_JSON = ROOT / "package.json"
DEFAULT_SUMMARY_OUT = Path(
    "tmp/reports/m247/M247-B008/semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_contract_summary.json"
)


@dataclass(frozen=True)
class SnippetCheck:
    check_id: str
    snippet: str


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-DOC-EXP-01",
        "# M247 Semantic Hot-Path Analysis and Budgeting Recovery and Determinism Hardening Expectations (B008)",
    ),
    SnippetCheck(
        "M247-B008-DOC-EXP-02",
        "Contract ID: `objc3c-semantic-hot-path-analysis-and-budgeting-recovery-and-determinism-hardening/m247-b008-v1`",
    ),
    SnippetCheck("M247-B008-DOC-EXP-03", "- Dependencies: `M247-B007`"),
    SnippetCheck("M247-B008-DOC-EXP-04", "Issue `#6731` defines canonical lane-B recovery and determinism hardening scope."),
    SnippetCheck("M247-B008-DOC-EXP-05", "`M247-B007` remains a mandatory dependency token"),
    SnippetCheck("M247-B008-DOC-EXP-06", "code/spec anchors and milestone optimization improvements as mandatory scope inputs."),
    SnippetCheck(
        "M247-B008-DOC-EXP-07",
        "`check:objc3c:m247-b008-lane-b-readiness`",
    ),
    SnippetCheck(
        "M247-B008-DOC-EXP-08",
        "`tmp/reports/m247/M247-B008/semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_contract_summary.json`",
    ),
    SnippetCheck(
        "M247-B008-DOC-EXP-09",
        "`npm run --if-present check:objc3c:m247-b007-lane-b-readiness`",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-DOC-PKT-01",
        "# M247-B008 Semantic Hot-Path Analysis and Budgeting Recovery and Determinism Hardening Packet",
    ),
    SnippetCheck("M247-B008-DOC-PKT-02", "Packet: `M247-B008`"),
    SnippetCheck("M247-B008-DOC-PKT-03", "Issue: `#6731`"),
    SnippetCheck("M247-B008-DOC-PKT-04", "Dependencies: `M247-B007`"),
    SnippetCheck(
        "M247-B008-DOC-PKT-05",
        "`scripts/check_m247_b008_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_contract.py`",
    ),
    SnippetCheck(
        "M247-B008-DOC-PKT-06",
        "`check:objc3c:m247-b008-lane-b-readiness`",
    ),
    SnippetCheck(
        "M247-B008-DOC-PKT-07",
        "optimization improvements as mandatory scope inputs.",
    ),
)

ARCHITECTURE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-ARCH-01",
        "M247 lane-B B008 semantic hot-path analysis/budgeting recovery and determinism hardening anchors",
    ),
    SnippetCheck(
        "M247-B008-ARCH-02",
        "docs/contracts/m247_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_b008_expectations.md",
    ),
)

LOWERING_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-SPC-01",
        "semantic hot-path analysis/budgeting recovery and determinism hardening wiring",
    ),
    SnippetCheck(
        "M247-B008-SPC-02",
        "dependency anchor (`M247-B007`) and fail closed",
    ),
)

METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-META-01",
        "deterministic lane-B semantic hot-path analysis/budgeting metadata anchors for `M247-B008`",
    ),
    SnippetCheck(
        "M247-B008-META-02",
        "pending token `M247-B007`",
    ),
)

PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M247-B008-PKG-01",
        '"check:objc3c:m247-b008-semantic-hot-path-analysis-and-budgeting-recovery-and-determinism-hardening-contract": '
        '"python scripts/check_m247_b008_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_contract.py"',
    ),
    SnippetCheck(
        "M247-B008-PKG-02",
        '"test:tooling:m247-b008-semantic-hot-path-analysis-and-budgeting-recovery-and-determinism-hardening-contract": '
        '"python -m pytest tests/tooling/test_check_m247_b008_semantic_hot_path_analysis_and_budgeting_recovery_and_determinism_hardening_contract.py -q"',
    ),
    SnippetCheck(
        "M247-B008-PKG-03",
        '"check:objc3c:m247-b008-lane-b-readiness": '
        '"npm run --if-present check:objc3c:m247-b007-lane-b-readiness '
        "&& npm run check:objc3c:m247-b008-semantic-hot-path-analysis-and-budgeting-recovery-and-determinism-hardening-contract "
        '&& npm run test:tooling:m247-b008-semantic-hot-path-analysis-and-budgeting-recovery-and-determinism-hardening-contract"',
    ),
    SnippetCheck("M247-B008-PKG-04", '"test:objc3c:sema-pass-manager-diagnostics-bus": '),
    SnippetCheck("M247-B008-PKG-05", '"test:objc3c:perf-budget": '),
)


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2) + "\n"


def display_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        return resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return resolved.as_posix()


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expectations-doc", type=Path, default=DEFAULT_EXPECTATIONS_DOC)
    parser.add_argument("--packet-doc", type=Path, default=DEFAULT_PACKET_DOC)
    parser.add_argument("--architecture-doc", type=Path, default=DEFAULT_ARCHITECTURE_DOC)
    parser.add_argument("--lowering-spec", type=Path, default=DEFAULT_LOWERING_SPEC)
    parser.add_argument("--metadata-spec", type=Path, default=DEFAULT_METADATA_SPEC)
    parser.add_argument("--package-json", type=Path, default=DEFAULT_PACKAGE_JSON)
    parser.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY_OUT)
    return parser.parse_args(argv)


def check_doc_contract(
    *,
    path: Path,
    exists_check_id: str,
    snippets: tuple[SnippetCheck, ...],
) -> tuple[int, list[Finding]]:
    checks_total = 1
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required document is missing: {display_path(path)}",
            )
        )
        return checks_total, findings
    if not path.is_file():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required path is not a file: {display_path(path)}",
            )
        )
        return checks_total, findings

    text = path.read_text(encoding="utf-8")
    for snippet in snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(
                Finding(
                    artifact=display_path(path),
                    check_id=snippet.check_id,
                    detail=f"missing required snippet: {snippet.snippet}",
                )
            )
    return checks_total, findings


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    checks_total = 0
    failures: list[Finding] = []

    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M247-B008-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M247-B008-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.architecture_doc, "M247-B008-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M247-B008-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M247-B008-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M247-B008-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
        check_count, findings = check_doc_contract(
            path=path,
            exists_check_id=exists_check_id,
            snippets=snippets,
        )
        checks_total += check_count
        failures.extend(findings)

    checks_passed = checks_total - len(failures)
    summary_payload = {
        "mode": MODE,
        "ok": not failures,
        "checks_total": checks_total,
        "checks_passed": checks_passed,
        "failures": [
            {
                "artifact": finding.artifact,
                "check_id": finding.check_id,
                "detail": finding.detail,
            }
            for finding in failures
        ],
    }

    summary_path = args.summary_out
    if not summary_path.is_absolute():
        summary_path = ROOT / summary_path
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary_path.write_text(canonical_json(summary_payload), encoding="utf-8")

    if failures:
        for finding in failures:
            print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
        return 1
    print(f"[ok] {MODE}: {checks_passed}/{checks_total} checks passed")
    return 0


def main() -> int:
    return run(sys.argv[1:])


if __name__ == "__main__":
    raise SystemExit(main())
