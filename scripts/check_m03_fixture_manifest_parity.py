#!/usr/bin/env python3
"""Fail-closed M03 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M03-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M03-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(
        ...
        first_issue=1258,
        last_issue=1267,
        required_group_name="m03_lane_a_issue_1258_1267_frontend_scope_freeze",
    ),
    LaneSpec(lane="B", bucket="semantic", first_issue=1268, last_issue=1277),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1278, last_issue=1286),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1287, last_issue=1295),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M03 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
