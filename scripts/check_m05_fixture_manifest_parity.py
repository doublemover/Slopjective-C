#!/usr/bin/env python3
"""Fail-closed M05 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M05-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M05-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(
        ...
        first_issue=1354,
        last_issue=1365,
        required_group_name="m05_lane_a_issue_1354_1365_frontend_scope_freeze",
    ),
    LaneSpec(lane="B", bucket="semantic", first_issue=1366, last_issue=1377),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1378, last_issue=1389),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1390, last_issue=1401),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M05 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
