#!/usr/bin/env python3
"""Fail-closed checker for M245-A009 frontend behavior parity conformance matrix implementation."""
...
MODE = "m245-a009-frontend-behavior-parity-toolchains-conformance-matrix-implementation-contract-v1"
...
DEFAULT_A008_EXPECTATIONS_DOC = (
    ...
    / "docs"
    / "contracts"
    / "m245_frontend_behavior_parity_across_toolchains_recovery_and_determinism_hardening_a008_expectations.md"
)
DEFAULT_A008_PACKET_DOC = (
    ...
    / "compiler"
    / "m245"
    / "m245_a008_frontend_behavior_parity_across_toolchains_recovery_and_determinism_hardening_packet.md"
)
DEFAULT_A008_CHECKER = (
    ROOT / "scripts" / "check_m245_a008_frontend_behavior_parity_across_toolchains_recovery_and_determinism_hardening_contract.py"
)
DEFAULT_A008_TEST = (
    ROOT
    / "tests"
    / "tooling"
    / "test_check_m245_a008_frontend_behavior_parity_across_toolchains_recovery_and_determinism_hardening_contract.py"
)
...
PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M245-A009-DOC-PKT-01",
        "# M245-A009 Frontend Behavior Parity Across Toolchains Conformance Matrix Implementation Packet",
    ),
    SnippetCheck("M245-A009-DOC-PKT-02", "Packet: `M245-A009`"),
    SnippetCheck("M245-A009-DOC-PKT-03", "Dependencies: `M245-A008`"),
    SnippetCheck(
        "M245-A009-DOC-PKT-04",
        "code/spec anchors and milestone optimization improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M245-A009-DOC-PKT-05",
        "scripts/check_m245_a009_frontend_behavior_parity_across_toolchains_conformance_matrix_implementation_contract.py",
    ),
    SnippetCheck(
        "M245-A009-DOC-PKT-06",
...
A008_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M245-A009-A008-PKT-01", "Packet: `M245-A008`"),
    SnippetCheck("M245-A009-A008-PKT-02", "Dependencies: `M245-A007`"),
)
...
METADATA_SPEC_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M245-A009-META-02",
        "explicit `M245-A008` dependency continuity so conformance matrix implementation drift fails closed",
    ),
)
...
PACKAGE_SNIPPETS: tuple[SnippetCheck, ...] = (
    ...
    SnippetCheck(
        "M245-A009-PKG-03",
        '"check:objc3c:m245-a009-lane-a-readiness": '
        '"npm run check:objc3c:m245-a008-lane-a-readiness '
        '&& npm run check:objc3c:m245-a009-frontend-behavior-parity-toolchains-conformance-matrix-implementation-contract '
        '&& npm run test:tooling:m245-a009-frontend-behavior-parity-toolchains-conformance-matrix-implementation-contract"',
    ),
    SnippetCheck("M245-A009-PKG-04", '"test:objc3c:parser-replay-proof": '),
    SnippetCheck("M245-A009-PKG-05", '"test:objc3c:parser-ast-extraction": '),
)
...
def run(argv: Sequence[str]) -> int:
    ...
    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M245-A009-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M245-A009-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.a008_expectations_doc, "M245-A009-A008-DOC-EXISTS", A008_EXPECTATIONS_SNIPPETS),
        (args.a008_packet_doc, "M245-A009-A008-PKT-EXISTS", A008_PACKET_SNIPPETS),
        (args.architecture_doc, "M245-A009-ARCH-EXISTS", ARCHITECTURE_SNIPPETS),
        (args.lowering_spec, "M245-A009-SPC-EXISTS", LOWERING_SPEC_SNIPPETS),
        (args.metadata_spec, "M245-A009-META-EXISTS", METADATA_SPEC_SNIPPETS),
        (args.package_json, "M245-A009-PKG-EXISTS", PACKAGE_SNIPPETS),
    ):
    ...
    for path, check_id in (
        (args.a008_checker, "M245-A009-DEP-A008-ARG-01"),
        (args.a008_test, "M245-A009-DEP-A008-ARG-02"),
    ):
        checks_total += 1
        if not path.exists():
            failures.append(
