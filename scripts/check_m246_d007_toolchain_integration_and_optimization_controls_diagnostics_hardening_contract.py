#!/usr/bin/env python3
"""Fail-closed checker for M246-D007 toolchain integration/optimization controls diagnostics hardening."""

from __future__ import annotations

import argparse
import json
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Sequence

ROOT = Path(__file__).resolve().parents[1]
MODE = "m246-d007-toolchain-integration-optimization-controls-diagnostics-hardening-contract-v1"

DEFAULT_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_toolchain_integration_and_optimization_controls_diagnostics_hardening_d007_expectations.md"
)
DEFAULT_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m246"
    / "m246_d007_toolchain_integration_and_optimization_controls_diagnostics_hardening_packet.md"
)
DEFAULT_READINESS_SCRIPT = ROOT / "scripts" / "run_m246_d007_lane_d_readiness.py"
DEFAULT_D006_EXPECTATIONS_DOC = (
    ROOT
    / "docs"
    / "contracts"
    / "m246_toolchain_integration_and_optimization_controls_edge_case_expansion_and_robustness_d006_expectations.md"
)
DEFAULT_D006_PACKET_DOC = (
    ROOT
    / "spec"
    / "planning"
    / "compiler"
    / "m246"
    / "m246_d006_toolchain_integration_and_optimization_controls_edge_case_expansion_and_robustness_packet.md"
)
DEFAULT_D006_CHECKER = (
    ROOT
    / "scripts"
    / "check_m246_d006_toolchain_integration_and_optimization_controls_edge_case_expansion_and_robustness_contract.py"
)
DEFAULT_D006_TEST = (
    ROOT
    / "tests"
    / "tooling"
    / "test_check_m246_d006_toolchain_integration_and_optimization_controls_edge_case_expansion_and_robustness_contract.py"
)
DEFAULT_D006_READINESS_SCRIPT = ROOT / "scripts" / "run_m246_d006_lane_d_readiness.py"
DEFAULT_SUMMARY_OUT = Path(
    "tmp/reports/m246/M246-D007/toolchain_integration_optimization_controls_diagnostics_hardening_contract_summary.json"
)


@dataclass(frozen=True)
class SnippetCheck:
    check_id: str
    snippet: str


@dataclass(frozen=True)
class Finding:
    artifact: str
    check_id: str
    detail: str


EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-D007-DOC-EXP-01",
        "# M246 Toolchain Integration and Optimization Controls Diagnostics Hardening Expectations (D007)",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-02",
        "Contract ID: `objc3c-toolchain-integration-optimization-controls-diagnostics-hardening/m246-d007-v1`",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-03",
        "Issue `#6686` defines canonical lane-D diagnostics hardening scope.",
    ),
    SnippetCheck("M246-D007-DOC-EXP-04", "Dependencies: `M246-D006`"),
    SnippetCheck(
        "M246-D007-DOC-EXP-05",
        "scripts/check_m246_d006_toolchain_integration_and_optimization_controls_edge_case_expansion_and_robustness_contract.py",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-06",
        "scripts/run_m246_d007_lane_d_readiness.py",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-07",
        "code/spec anchors and milestone optimization improvements as mandatory scope inputs.",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-08",
        "Readiness chain order: `D006 readiness -> D007 checker -> D007 pytest`.",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-09",
        "tests/tooling/test_check_m246_d007_toolchain_integration_and_optimization_controls_diagnostics_hardening_contract.py",
    ),
    SnippetCheck(
        "M246-D007-DOC-EXP-10",
        "tmp/reports/m246/M246-D007/toolchain_integration_optimization_controls_diagnostics_hardening_contract_summary.json",
    ),
)

PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-D007-DOC-PKT-01",
        "# M246-D007 Toolchain Integration and Optimization Controls Diagnostics Hardening Packet",
    ),
    SnippetCheck("M246-D007-DOC-PKT-02", "Packet: `M246-D007`"),
    SnippetCheck("M246-D007-DOC-PKT-03", "Issue: `#6686`"),
    SnippetCheck("M246-D007-DOC-PKT-04", "Dependencies: `M246-D006`"),
    SnippetCheck("M246-D007-DOC-PKT-05", "Freeze date: `2026-03-04`"),
    SnippetCheck("M246-D007-DOC-PKT-06", "Dependency anchors from `M246-D006`:"),
    SnippetCheck(
        "M246-D007-DOC-PKT-07",
        "scripts/run_m246_d007_lane_d_readiness.py",
    ),
    SnippetCheck(
        "M246-D007-DOC-PKT-08",
        "python -m pytest tests/tooling/test_check_m246_d007_toolchain_integration_and_optimization_controls_diagnostics_hardening_contract.py -q",
    ),
    SnippetCheck(
        "M246-D007-DOC-PKT-09",
        "`D006 readiness -> D007 checker -> D007 pytest`",
    ),
    SnippetCheck(
        "M246-D007-DOC-PKT-10",
        "tmp/reports/m246/M246-D007/toolchain_integration_optimization_controls_diagnostics_hardening_contract_summary.json",
    ),
)

D006_EXPECTATIONS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck(
        "M246-D007-D006-DOC-01",
        "Contract ID: `objc3c-toolchain-integration-optimization-controls-edge-case-expansion-and-robustness/m246-d006-v1`",
    ),
    SnippetCheck(
        "M246-D007-D006-DOC-02",
        "Dependencies: `M246-D005`",
    ),
    SnippetCheck(
        "M246-D007-D006-DOC-03",
        "Issue `#6685` defines canonical lane-D edge-case expansion and robustness scope.",
    ),
)

D006_PACKET_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M246-D007-D006-PKT-01", "Packet: `M246-D006`"),
    SnippetCheck("M246-D007-D006-PKT-02", "Issue: `#6685`"),
    SnippetCheck("M246-D007-D006-PKT-03", "Dependencies: `M246-D005`"),
    SnippetCheck("M246-D007-D006-PKT-04", "Freeze date: `2026-03-04`"),
)

READINESS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M246-D007-RDY-01", 'DEPENDENCY_TOKEN = "M246-D006"'),
    SnippetCheck(
        "M246-D007-RDY-02",
        "scripts/run_m246_d006_lane_d_readiness.py",
    ),
    SnippetCheck(
        "M246-D007-RDY-03",
        "scripts/check_m246_d007_toolchain_integration_and_optimization_controls_diagnostics_hardening_contract.py",
    ),
    SnippetCheck(
        "M246-D007-RDY-04",
        "tests/tooling/test_check_m246_d007_toolchain_integration_and_optimization_controls_diagnostics_hardening_contract.py",
    ),
    SnippetCheck("M246-D007-RDY-05", "[ok] M246-D007 lane-D readiness chain completed"),
)

D006_READINESS_SNIPPETS: tuple[SnippetCheck, ...] = (
    SnippetCheck("M246-D007-D006-RDY-01", 'DEPENDENCY_TOKEN = "M246-D005"'),
    SnippetCheck(
        "M246-D007-D006-RDY-02",
        "scripts/run_m246_d005_lane_d_readiness.py",
    ),
    SnippetCheck("M246-D007-D006-RDY-03", "[ok] M246-D006 lane-D readiness chain completed"),
)


def canonical_json(payload: object) -> str:
    return json.dumps(payload, indent=2, sort_keys=True) + "\n"


def display_path(path: Path) -> str:
    resolved = path.resolve()
    try:
        return resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return resolved.as_posix()


def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expectations-doc", type=Path, default=DEFAULT_EXPECTATIONS_DOC)
    parser.add_argument("--packet-doc", type=Path, default=DEFAULT_PACKET_DOC)
    parser.add_argument("--readiness-script", type=Path, default=DEFAULT_READINESS_SCRIPT)
    parser.add_argument("--d006-expectations-doc", type=Path, default=DEFAULT_D006_EXPECTATIONS_DOC)
    parser.add_argument("--d006-packet-doc", type=Path, default=DEFAULT_D006_PACKET_DOC)
    parser.add_argument("--d006-checker", type=Path, default=DEFAULT_D006_CHECKER)
    parser.add_argument("--d006-test", type=Path, default=DEFAULT_D006_TEST)
    parser.add_argument("--d006-readiness-script", type=Path, default=DEFAULT_D006_READINESS_SCRIPT)
    parser.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY_OUT)
    return parser.parse_args(argv)


def check_text_artifact(
    *,
    path: Path,
    exists_check_id: str,
    snippets: tuple[SnippetCheck, ...],
) -> tuple[int, list[Finding]]:
    checks_total = 1
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required document is missing: {display_path(path)}",
            )
        )
        return checks_total, findings
    if not path.is_file():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"required path is not a file: {display_path(path)}",
            )
        )
        return checks_total, findings

    try:
        text = path.read_text(encoding="utf-8")
    except OSError as exc:
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=exists_check_id,
                detail=f"unable to read required document: {exc}",
            )
        )
        return checks_total, findings

    for snippet in snippets:
        checks_total += 1
        if snippet.snippet not in text:
            findings.append(
                Finding(
                    artifact=display_path(path),
                    check_id=snippet.check_id,
                    detail=f"missing required snippet: {snippet.snippet}",
                )
            )
    return checks_total, findings


def check_dependency_path(path: Path, check_id: str) -> tuple[int, list[Finding]]:
    findings: list[Finding] = []
    if not path.exists():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=check_id,
                detail=f"required dependency path is missing: {display_path(path)}",
            )
        )
    elif not path.is_file():
        findings.append(
            Finding(
                artifact=display_path(path),
                check_id=check_id,
                detail=f"required dependency path is not a file: {display_path(path)}",
            )
        )
    return 1, findings


def write_summary(summary_path: Path, payload: object) -> tuple[bool, str]:
    try:
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        summary_path.write_text(canonical_json(payload), encoding="utf-8")
    except OSError as exc:
        return False, f"unable to write summary file: {exc}"
    return True, ""


def run(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    checks_total = 0
    failures: list[Finding] = []

    for path, exists_check_id, snippets in (
        (args.expectations_doc, "M246-D007-DOC-EXP-EXISTS", EXPECTATIONS_SNIPPETS),
        (args.packet_doc, "M246-D007-DOC-PKT-EXISTS", PACKET_SNIPPETS),
        (args.d006_expectations_doc, "M246-D007-D006-DOC-EXISTS", D006_EXPECTATIONS_SNIPPETS),
        (args.d006_packet_doc, "M246-D007-D006-PKT-EXISTS", D006_PACKET_SNIPPETS),
        (args.readiness_script, "M246-D007-RDY-EXISTS", READINESS_SNIPPETS),
        (args.d006_readiness_script, "M246-D007-D006-RDY-EXISTS", D006_READINESS_SNIPPETS),
    ):
        count, findings = check_text_artifact(
            path=path,
            exists_check_id=exists_check_id,
            snippets=snippets,
        )
        checks_total += count
        failures.extend(findings)

    for path, check_id in (
        (args.d006_checker, "M246-D007-DEP-D006-ARG-01"),
        (args.d006_test, "M246-D007-DEP-D006-ARG-02"),
    ):
        count, findings = check_dependency_path(path, check_id)
        checks_total += count
        failures.extend(findings)

    checks_passed = checks_total - len(failures)
    summary_payload = {
        "mode": MODE,
        "ok": not failures,
        "checks_total": checks_total,
        "checks_passed": checks_passed,
        "failures": [
            {"artifact": failure.artifact, "check_id": failure.check_id, "detail": failure.detail}
            for failure in failures
        ],
    }

    summary_path = args.summary_out if args.summary_out.is_absolute() else ROOT / args.summary_out
    summary_ok, summary_error = write_summary(summary_path, summary_payload)
    if not summary_ok:
        print(
            f"[M246-D007-SUMMARY-WRITE-01] {display_path(summary_path)}: {summary_error}",
            file=sys.stderr,
        )
        return 1

    if failures:
        for finding in failures:
            print(f"[{finding.check_id}] {finding.artifact}: {finding.detail}", file=sys.stderr)
        return 1
    print(f"[ok] {MODE}: {checks_passed}/{checks_total} checks passed")
    return 0


if __name__ == "__main__":
    raise SystemExit(run(sys.argv[1:]))
