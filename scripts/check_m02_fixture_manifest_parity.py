#!/usr/bin/env python3
"""Fail-closed M02 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M02-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M02-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    LaneSpec(
        ...
        first_issue=1214,
        last_issue=1222,
        required_group_name="m02_lane_a_issue_1214_1222_frontend_scope_freeze",
    ),
    LaneSpec(lane="B", bucket="semantic", first_issue=1223, last_issue=1231),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1232, last_issue=1240),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1241, last_issue=1248),
)
...
def validate_lane_manifest(spec: LaneSpec, bucket_dir: Path, problems: list[str]) -> None:
    ...
    if spec.required_group_name is not None:
        named_groups = [
            group for group in groups if group.get("name") == spec.required_group_name
        ]
        if not named_groups:
            problems.append(
                f"{lane_tag}: missing required manifest group '{spec.required_group_name}'"
            )
        elif len(named_groups) > 1:
            problems.append(
                f"{lane_tag}: duplicate manifest group '{spec.required_group_name}'"
            )
        else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M02 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
