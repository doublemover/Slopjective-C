#!/usr/bin/env python3
"""Fail-closed M11 fixture/manifest parity checker for conformance lanes A-D."""
...
class LaneSpec:
    ...
    def expected_files(self) -> tuple[str, ...]:
        ...
        return tuple(f"M11-{self.lane}{index:03d}.json" for index in range(1, count + 1))

    @property
    def lane_pattern(self) -> re.Pattern[str]:
        return re.compile(rf"^M11-{self.lane}\d{{3}}\.json$")
...
LANE_SPECS: tuple[LaneSpec, ...] = (
    ...
    LaneSpec(lane="B", bucket="semantic", first_issue=1679, last_issue=1689),
    LaneSpec(lane="C", bucket="lowering_abi", first_issue=1690, last_issue=1700),
    LaneSpec(lane="D", bucket="diagnostics", first_issue=1701, last_issue=1711),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check M11 lane A-D fixture/manifest parity fail-closed."
    )
    parser.add_argument(
        "--conformance-root",
        type=Path,
